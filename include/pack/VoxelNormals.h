...
class VoxelNormalTables {
 public:
  ...
  bool isValidTable(const std::uint8_t normalClass) const { return normalClass > 0 && normalClass <= m_tables.size(); }
  ...
  const VoxelNormalTable& getTable(const std::uint8_t normalClass) const {
    if (!isValidTable(normalClass)) throw std::range_error("Table for normal class does not exist");
