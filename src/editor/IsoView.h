...
#  endif
 ...
 public:
  ...
  RECT m_funcRect;
  Vec2<CSProjected, float> GetViewScale() const { return m_viewScale; }
  ProjectedVec GetViewOffset() const { return m_viewOffset; }
  ...
  void __fastcall ChangeTileHeight(DWORD dwPos, DWORD dwNewHeight, BOOL bNonMorphableMove, BOOL bOnlyThisTile = FALSE,
                                   BOOL bNoSlopes = FALSE);
  ...
  void DrawCell(void* dest, int dest_width, int dest_height, int dest_pitch, int x, int y, int w, int h, int col,
                bool dotted = false, bool touchNeighbours = false, int colNeighbour = CLR_INVALID) const;
  ...
  MapCoords GetMapCoordinates(const ProjectedCoords& projCoords, bool bAllowAccessBehindCliffs = false,
                              bool ignoreHideFlags = false) const;
  ...
  /// <returns>Logical map coordinates</returns>
  MapCoords GetMapCoordinatesFromRenderTargetCoordinates(const ProjectedCoords& screenViewCoords,
                                                         bool bAllowAccessBehindCliffs = false,
                                                         bool ignoreHideFlags = false) const;

  /// <summary>
  /// Converts from Win32 window client coordinates (0/0 at the top left corner of the view window) to logical map
  /// coordinates
  /// </summary>
  /// <param name="clientPt">Client coordinates as given e.g. by window messages</param>
  /// <returns>Logical map coordinates</returns>
  MapCoords GetMapCoordinatesFromClientCoordinates(const CPoint& clientPt, bool bAllowAccessBehindCliffs = false,
                                                   bool ignoreHideFlags = false) const;

  /// <summary>
  /// Converts from Win32 window client coordinates (0/0 at the top left corner of the view window) to (world) pixel
  /// coordinates
  /// </summary>
  /// <param name="clientPt">Client coordinates as given e.g. by window messages</param>
  ...
  /// <summary>
  /// Converts from logical map coordinates to view / render target pixel coordinates (screen backbuffer, e.g. currently
  /// not yet scaled by view zoom)
  /// </summary>
  /// <param name="mapCoords">Logical map coordinates</param>
  ...
  /// <summary>
  /// Converts from logical map coordinates to Win32 window client coordinates (0/0 at the top left corner of the view
  /// window)
  /// </summary>
  ...
  /// <summary>
  /// Converts from (world) pixel coordinates to Win32 window client coordinates (0/0 at the top left corner of the view
  /// window)
  /// </summary>
  /// <param name="projCoords">World pixel coordinates</param>
  ...
  /// <summary>
  /// Applies the scaling as it is being done when blitting from back to frontbuffer, required e.g. for drawing calls
  /// directly to the frontbuffer If you use mouse-move coordinates you need to
  /// </summary>
  /// <param name="projCoords">Texel coordinates of the backbuffer render target</param>
