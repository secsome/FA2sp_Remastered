...
class CMapData {
 public:
  ...
  DWORD GetMapPos(int mapX, int mapY) const { return mapX + mapY * m_IsoSize; }
  DWORD GetMapPos(const MapCoords& coords) const { return coords.x + coords.y * m_IsoSize; }
  ...
  void CreateSlopesAt(DWORD dwPos) {
    ...
    if (!theApp.m_Options.bDisableSlopeCorrection && d.bMorphable) {
      if ((h[0][1] < 0 && h[2][1] < 0) || (h[1][0] < 0 && h[1][2] < 0)
          ...
          || (h[1][0] < 0 && h[0][2] < 0 && h[0][1] >= 0) || (h[1][0] < 0 && h[2][2] < 0 && h[2][1] >= 0)

          || (h[0][1] < 0 && h[2][0] < 0 && h[1][0] >= 0) || (h[0][1] < 0 && h[2][2] < 0 && h[1][2] >= 0)

          || (h[1][2] < 0 && h[0][0] < 0 && h[0][1] >= 0) || (h[1][2] < 0 && h[2][0] < 0 && h[2][1] >= 0)

          || (h[2][1] < 0 && h[0][0] < 0 && h[1][0] >= 0) || (h[2][1] < 0 && h[0][2] < 0 && h[1][2] >= 0)

          || (h[1][0] < 0 && h[0][1] < 0 && h[0][0] >= 0) || (h[0][1] < 0 && h[1][2] < 0 && h[0][2] >= 0) ||
          (h[1][2] < 0 && h[2][1] < 0 && h[2][2] >= 0) || (h[2][1] < 0 && h[1][0] < 0 && h[2][0] >= 0)

      ) {
        SetHeightAt(dwPos, m.bHeight + 1);
        for (i = -1; i < 2; i++)
          for (e = -1; e < 2; e++) CreateSlopesAt(dwPos + i + e * m_IsoSize);
    ...
    if (h[0][0] == -1 && h[2][2] == -1 && h[2][0] >= 0 && h[0][2] >= 0 && h[1][0] >= 0 && h[1][2] >= 0 &&
        h[0][1] >= 0 && h[2][1] >= 0)
      ns = SLOPE_UP_LEFTTOP_AND_RIGHTBOTTOM;
    if (h[0][2] == -1 && h[2][0] == -1 && h[0][0] >= 0 && h[2][2] >= 0 && h[0][1] >= 0 && h[1][0] >= 0 &&
        h[1][2] >= 0 && h[2][1] >= 0)
    ...
    if (d.bMorphable && ns != -1) {
      if (tiledata == &un_tiledata)  // NEW URBAN FIX FOR URBAN PAVEMENT
      {
        ...
        int r = ramp2set;
        int m = pave2set;
        if (d.wTileSet == r || d.wTileSet == m) rampbase = ramp2set_start;
      }
  ...
  BOOL SetTileAt(DWORD dwPos, DWORD dwID, DWORD dwTile) {
    ...
    if ((*tiledata)[dwID].bReplacementCount &&
        atoi((*tiles).sections["General"].AccessValueByName("BridgeSet")) != (*tiledata)[dwID].wTileSet) {
  ...
  BOOL AddAircraft(AIRCRAFT* lpAircraft, LPCTSTR lpType = NULL, LPCTSTR lpHouse = NULL, DWORD dwPos = 0,
                   CString suggestedID = "");
  ...
  BOOL AddStructure(STRUCTURE* lpStructure, LPCTSTR lpType = NULL, LPCTSTR lpHouse = NULL, DWORD dwPos = 0,
                    CString suggestedID = "");
  BOOL AddInfantry(INFANTRY* lpInfantry, LPCTSTR lpType = NULL, LPCTSTR lpHouse = NULL, DWORD dwPos = 0,
  ...
  INT GetCelltagAt(DWORD dwPos) const { return fielddata[dwPos].celltag; }
  INT GetCelltagAt(MapCoords pos) const { return GetCelltagAt(GetMapPos(pos)); }
  INT GetWaypointAt(DWORD dwPos) const { return fielddata[dwPos].waypoint; }
  INT GetWaypointAt(MapCoords pos) const { return GetWaypointAt(GetMapPos(pos)); }
  INT GetTerrainAt(DWORD dwPos) const { return fielddata[dwPos].terrain; }
  INT GetTerrainAt(MapCoords pos) const { return GetTerrainAt(GetMapPos(pos)); }
  INT GetAirAt(DWORD dwPos) const { return fielddata[dwPos].aircraft; }
  INT GetAirAt(MapCoords pos) const { return GetAirAt(GetMapPos(pos)); }
  INT GetStructureAt(DWORD dwPos) const {
    if (fielddata[dwPos].structure > -1) return fielddata[dwPos].structure;
    return -1;
  }
  INT GetStructureAt(MapCoords pos) const { return GetStructureAt(GetMapPos(pos)); }
  INT GetUnitAt(DWORD dwPos) const { return fielddata[dwPos].unit; }
  INT GetUnitAt(MapCoords pos) const { return GetUnitAt(GetMapPos(pos)); }
  INT GetInfantryAt(DWORD dwPos, DWORD dwSubPos = 0xFFFFFFFF) const {
    if (dwSubPos == 0xFFFFFFFF) {
      int i;
      for (i = 0; i < SUBPOS_COUNT; i++)
        if (fielddata[dwPos].infantry[i] != -1) return fielddata[dwPos].infantry[i];
      return -1;
    }
    return fielddata[dwPos].infantry[dwSubPos];
  }
  INT GetInfantryAt(MapCoords pos, DWORD dwSubPos = 0xFFFFFFFF) { return GetInfantryAt(GetMapPos(pos), dwSubPos); }
  INT GetNodeAt(DWORD dwPos, CString& lpHouse) const;
  INT GetNodeAt(MapCoords pos, CString& lpHouse) const { return GetNodeAt(GetMapPos(pos), lpHouse); }
  INT GetHeightAt(DWORD dwPos) const { return fielddata[dwPos].bHeight; }
  INT GetHeightAt(const MapCoords& coords) const { return GetHeightAt(GetMapPos(coords)); }
  ...
  const std::vector<std::unique_ptr<CTube>>& GetTubes() const { return m_tubes; }

  DWORD GetIsoSize() const { return m_IsoSize; }
  ...
  WORD GetHeight() const { return m_maprect.bottom; };
  WORD GetWidth() const { return m_maprect.right; };
  ...
  MapCoords ToMapCoords3d(ProjectedCoords xy, bool bAllowAccessBehindCliffs = false,
                          bool ignoreHideFlagsAndOutside = false) const;
...
#  endif
  ...
  __forceinline void Mini_UpdatePos(const int i, const int e, bool isMultiplayer) {
    ...
    if (m_mini_colors.empty() || !tiledata) return;
    ...
    if (xiso >= m_IsoSize) xiso = m_IsoSize - 1;
    if (yiso >= m_IsoSize) yiso = m_IsoSize - 1;
    if (xiso < 0) xiso = 0;
    if (yiso < 0) yiso = 0;
    ...
    RGBTRIPLE& col_r =
        (RGBTRIPLE&)m_mini_colors[(dwDrawPos + sizeof(RGBTRIPLE)) < size ? dwDrawPos + sizeof(RGBTRIPLE) : dwDrawPos];
...
#  endif
  ...
  __forceinline void SmoothAt(DWORD dwPos, int iSmoothSet, int iLatSet, int iTargetSet, BOOL bIgnoreShore = TRUE) {
    ...
    if (ts[1][1] == ils) {
      // single lat
      if (ts[0][1] != ils && ts[1][0] != ils && ts[1][2] != ils && ts[2][1] != ils)
        needed = 16;
      else if (ts[0][1] == ils && ts[1][0] == ils && ts[1][2] == ils && ts[2][1] == ils)
        needed = 0;
      else if (ts[0][1] == ils && ts[2][1] == ils && ts[1][0] != ils && ts[1][2] != ils)
        needed = 11;
      else if (ts[1][0] == ils && ts[1][2] == ils && ts[0][1] != ils && ts[2][1] != ils)
        needed = 6;
      else if (ts[1][0] != ils && ts[0][1] == ils && ts[2][1] == ils)
        needed = 9;
      else if (ts[2][1] != ils && ts[1][0] == ils && ts[1][2] == ils)
        needed = 5;
      else if (ts[1][2] != ils && ts[0][1] == ils && ts[2][1] == ils)
        needed = 3;
      else if (ts[0][1] != ils && ts[1][0] == ils && ts[1][2] == ils)
        needed = 2;
      else if (ts[0][1] == ils && ts[1][0] != ils && ts[1][2] != ils && ts[2][1] != ils)
        needed = 15;
      else if (ts[1][2] == ils && ts[1][0] != ils && ts[0][1] != ils && ts[2][1] != ils)
        needed = 14;
      else if (ts[2][1] == ils && ts[1][0] != ils && ts[0][1] != ils && ts[1][2] != ils)
        needed = 12;
      else if (ts[1][0] == ils && ts[0][1] != ils && ts[1][2] != ils && ts[2][1] != ils)
        needed = 8;
...
inline MapCoords CMapData::ToMapCoords(ProjectedCoords xy) const { return ToMapCoords3d(xy, 0); }

inline MapCoords CMapData::ToMapCoords3d(ProjectedCoords xy, int mapZ) const {
  float cx = xy.x, cy = xy.y + mapZ * f_y / 2;
  return MapCoords(cy / (float)f_y - cx / (float)f_x + (float)(m_IsoSize - 2) / 2 + (float)0.5,
                   cy / (float)f_y + cx / (float)f_x - (float)(m_IsoSize - 2) / 2.0f - (float)0.5);
}

inline MapCoords CMapData::ToMapCoords3d(const ProjectedCoords xy, bool bAllowAccessBehindCliffs,
  ...
  for (int i = 15; i >= 0; i--) {
    for (int e = 0; e < 3; e++) {
      ...
      const MapVec off2(
          e == 1 ? -1 : 0,
          e == 2 ? -1
                 : 0);  // increase x or y or neither in map coordinates by one so that we have a broader area to check
      ...
      if (isInside(cur)) {
        ...
        if (ignoreHideFlagsAndOutside || (!mfd.bHide && !(tiledata && (*tiledata)[ground].bHide))) {
          ...
          // if(*x >= m && *x<= m+f_x && *y>=n && *y<=n+f_y)
          {
            ...
            if (d.x >= 0.0f && d.y >= 0.0f && d.x <= 1.0f &&
                d.y <= 1.0f)  // || (!bAllowAccessBehindCliffs && xy.y >= curProj.y)))
                              // if (d == MapCoords(0, 0))
            {
...
inline ProjectedCoords CMapData::ProjectCoords(MapCoords xy) const { return ProjectCoords3d(xy, 0); }

inline ProjectedCoords CMapData::ProjectCoords3d(MapCoords xy, int z) const {
  return ProjectedCoords((m_IsoSize - 2 - xy.x + xy.y) * f_x / 2, (xy.y + xy.x - z) * f_y / 2);
}
