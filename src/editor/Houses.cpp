...
void CHouses::UpdateDialog() {
  ...
  if (ini.sections.find(MAPHOUSES) == ini.sections.end() && ini.sections.size() > 0) {
    // MessageBox("No houses do exist, if you want to use houses, you should use ""Prepare houses"" before doing
    // anything else.");
  } else {
    ...
    if (ini.sections["Basic"].values.find("Player") != ini.sections["Basic"].values.end()) {
      m_HumanPlayer.SetCurSel(
          m_HumanPlayer.FindStringExact(0, TranslateHouse(ini.sections["Basic"].values["Player"], TRUE)));
    }
...
void CHouses::AddHouse(const char* name) {
  ...
  if (ini.sections.find(name) != ini.sections.end()) {
    MessageBox(((CString) "Sorry this name is not available. " + name +
                (CString) " is already used in the map file. You need to use another name."));
    return;
  }
  ...
  for (c = 0; c > -1; c++) {
    char k[50];
    itoa(c, k, 10);
    if (ini.sections[MAPHOUSES].values.find(k) == ini.sections[MAPHOUSES].values.end()) pos = c;
    if (pos != -1) break;
  }
  for (c = 0; c > -1; c++) {
    char k[50];
    itoa(c, k, 10);
    if (ini.sections[HOUSES].values.find(k) == ini.sections[HOUSES].values.end()) pos2 = c;
    if (pos2 != -1) break;
  }
...
void CHouses::OnShowWindow(BOOL bShow, UINT nStatus) {
  ...
  if (bShow) {
    if (ini.sections.find(MAPHOUSES) == ini.sections.end() && ini.sections.size() > 0) {
      MessageBox(
          "No houses do exist, if you want to use houses, you should use "
          "Prepare houses"
...
void CHouses::OnDeletehouse() {
  ...
  if (ini.sections[MAPHOUSES].values.size() == 0) ini.sections.erase(MAPHOUSES);
...
void CHouses::OnSetfocusAllies() { SetMainStatusBar(GetLanguageStringACP("HousesAlliesHelp")); }
