...
CViewObjects::CViewObjects() { m_ready = FALSE; }
...
#ifdef _DEBUG
void CViewObjects::AssertValid() const { CTreeView::AssertValid(); }

void CViewObjects::Dump(CDumpContext& dc) const { CTreeView::Dump(dc); }
#endif  //_DEBUG
  ...
  if (val < valadded) {
    ...
    if (pos == 1) {
      ...
      if (subpos < 100) {
        ...
        if (subpos == 1) {
          AD.data = 31;
          AD.data2 = atoi(InputBox("Please enter the value (0-255) of the overlay. Don´t exceed this range.",
                                   "Set overlay manually"));

        } else if (subpos == 2) {
          AD.data = 32;
          AD.data2 = atoi(InputBox("Please enter the value (0-255) of the overlay-data. Don´t exceed this range.",
                                   "Set overlay manually"));
        }
...
__inline HTREEITEM TV_InsertItemW(HWND hWnd, WCHAR* lpString, int len, HTREEITEM hInsertAfter, HTREEITEM hParent,
                                  int param) {
  ...
  if (!res) {
    ...
    WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK, lpString, len + 1, (LPSTR)lpAscii, len + 1, NULL, &bUsedDefault);
...
void CViewObjects::UpdateDialog() {
  ...
  HTREEITEM first = tree.InsertItem(TVIF_PARAM | TVIF_TEXT, TranslateStringACP(GetLanguageStringACP("NothingObList")),
                                    i, i, 0, 0, -2, TVI_ROOT, TVI_LAST);
  ...
  if (!Map->IsMultiplayer() || !theApp.m_Options.bEasy)
    rootitems[11] =
        tree.InsertItem(TVIF_PARAM | TVIF_TEXT, TranslateStringACP(sTreeRoots[11]), i, i, 0, 0, i, TVI_ROOT, TVI_LAST);

  for (i = 0; i < 10; i++) {
    BOOL bAllow = TRUE;
    if (theApp.m_Options.bEasy) {
      if (i >= 6 && i <= 9) bAllow = FALSE;
    }
    ...
    if (bAllow)
      rootitems[i] = tree.InsertItem(TVIF_PARAM | TVIF_TEXT, sTreeRoots[i], i, i, 0, 0, i, TVI_ROOT, TVI_LAST);
  }

  rootitems[13] =
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, TranslateStringACP(sTreeRoots[13]), 13, 13, 0, 0, 13, TVI_ROOT, first);

  rootitems[12] =
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, TranslateStringACP(sTreeRoots[12]), 12, 12, 0, 0, 12, TVI_ROOT, TVI_LAST);

  rootitems[10] =
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, TranslateStringACP(sTreeRoots[10]), 10, 10, 0, 0, 10, TVI_ROOT, TVI_LAST);

#ifdef SMUDGE_SUPP
  rootitems[14] = tree.InsertItem(TVIF_PARAM | TVIF_TEXT, TranslateStringACP(sTreeRoots[14]), 14, 14, 0, 0, 10,
                                  TVI_ROOT, rootitems[4]);
#endif

  HTREEITEM structhouses[64];
  HTREEITEM hAllied =
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("Allied"), 0, 0, 0, 0, -1, rootitems[3], TVI_LAST);
  HTREEITEM hSoviet =
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("Soviet"), 0, 0, 0, 0, -1, rootitems[3], TVI_LAST);
  HTREEITEM hYuri = NULL;
  if (yuri_mode) {
    hYuri =
        tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("Yuri"), 0, 0, 0, 0, -1, rootitems[3], TVI_LAST);
  }
  ...
  structhouses[sides.size()] =
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("Other"), 0, 0, 0, 0, -1, rootitems[3], TVI_LAST);

  if (!theApp.m_Options.bEasy) {
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("CreateWaypObList"), 0, 0, 0, 0, 20, rootitems[6],
                    TVI_LAST);
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("CreateSpecWaypObList"), 0, 0, 0, 0, 22, rootitems[6],
                    TVI_LAST);
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DelWaypObList"), 0, 0, 0, 0, 21, rootitems[6],
                    TVI_LAST);
  }
  ...
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("StartpointsDelete"), 0, 0, 0, 0, 21, rootitems[12],
                  TVI_LAST);

  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetTheaterLanguageString("GroundClearObList"), 0, 0, 0, 0, 61, rootitems[13],
                  TVI_LAST);
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetTheaterLanguageString("GroundSandObList"), 0, 0, 0, 0, 62, rootitems[13],
                  TVI_LAST);
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetTheaterLanguageString("GroundRoughObList"), 0, 0, 0, 0, 63, rootitems[13],
                  TVI_LAST);
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetTheaterLanguageString("GroundGreenObList"), 0, 0, 0, 0, 65, rootitems[13],
                  TVI_LAST);
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetTheaterLanguageString("GroundPaveObList"), 0, 0, 0, 0, 66, rootitems[13],
                  TVI_LAST);
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetTheaterLanguageString("GroundWaterObList"), 0, 0, 0, 0, 64, rootitems[13],
                  TVI_LAST);
  if (Map->GetTheater() == THEATER3)
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetTheaterLanguageString("GroundPave2ObList"), 0, 0, 0, 0, 67,
                    rootitems[13], TVI_LAST);

  if (!theApp.m_Options.bEasy) {
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("CreateCelltagObList"), 0, 0, 0, 0, 36, rootitems[7],
                    TVI_LAST);
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DelCelltagObList"), 0, 0, 0, 0, 37, rootitems[7],
                    TVI_LAST);
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("CelltagPropObList"), 0, 0, 0, 0, 38, rootitems[7],
                    TVI_LAST);
  }

  if (!theApp.m_Options.bEasy) {
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("CreateNodeNoDelObList"), 0, 0, 0, 0, 40, rootitems[8],
                    TVI_LAST);
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("CreateNodeDelObList"), 0, 0, 0, 0, 41, rootitems[8],
                    TVI_LAST);
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DelNodeObList"), 0, 0, 0, 0, 42, rootitems[8],
                    TVI_LAST);
  }

  HTREEITEM deleteoverlay = tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DelOvrlObList"), 0, 0, 0, 0,
                                            -1, rootitems[5], TVI_LAST);
  HTREEITEM tiberium = tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("GrTibObList"), 0, 0, 0, 0, -1,
                                       rootitems[5], TVI_LAST);
  // HTREEITEM bluetiberium=tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("BlTibObList"), 0,0,0,0, -1,
  // rootitems[5], TVI_LAST );
  HTREEITEM bridges = tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("BridgesObList"), 0, 0, 0, 0, -1,
                                      rootitems[5], TVI_LAST);
  HTREEITEM alloverlay = tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("OthObList"), 0, 0, 0, 0, -1,
                                         rootitems[5], TVI_LAST);
  HTREEITEM everyoverlay = NULL;

  if (!theApp.m_Options.bEasy) {
    everyoverlay = tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("AllObList"), 0, 0, 0, 0, -1,
                                   rootitems[5], TVI_LAST);
  }

  if (!theApp.m_Options.bEasy) {
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("OvrlManuallyObList"), 0, 0, 0, 0, valadded * 6 + 1,
                    rootitems[5], TVI_LAST);
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("OvrlDataManuallyObList"), 0, 0, 0, 0,
                    valadded * 6 + 2, rootitems[5], TVI_LAST);
  }

  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DelOvrl0ObList"), 0, 0, 0, 0, valadded * 6 + 100 + 0,
                  deleteoverlay, TVI_LAST);
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DelOvrl1ObList"), 0, 0, 0, 0, valadded * 6 + 100 + 1,
                  deleteoverlay, TVI_LAST);
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DelOvrl2ObList"), 0, 0, 0, 0, valadded * 6 + 100 + 2,
                  deleteoverlay, TVI_LAST);
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DelOvrl3ObList"), 0, 0, 0, 0, valadded * 6 + 100 + 3,
                  deleteoverlay, TVI_LAST);

  // tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DrawRanTibObList"), 0,0,0,0, valadded*6+200+0,
  // tiberium, TVI_LAST );
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DrawTibObList"), 0, 0, 0, 0, valadded * 6 + 200 + 10,
                  tiberium, TVI_LAST);
  // tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("IncTibSizeObList"), 0,0,0,0, valadded*6+200+20,
  // tiberium, TVI_LAST ); tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DecTibSizeObList"), 0,0,0,0,
  // valadded*6+200+21, tiberium, TVI_LAST );

  // tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DrawRanTibObList"), 0,0,0,0, valadded*6+300+0,
  // bluetiberium, TVI_LAST );
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DrawTib2ObList"), 0, 0, 0, 0, valadded * 6 + 300 + 10,
                  tiberium, TVI_LAST);
  // tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("IncTibSizeObList"), 0,0,0,0, valadded*6+300+20,
  // bluetiberium, TVI_LAST ); tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DecTibSizeObList"),
  // 0,0,0,0, valadded*6+300+21, bluetiberium, TVI_LAST );

  if (Map->GetTheater() != THEATER4 && Map->GetTheater() != THEATER5) {
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("SmallBridgeObList"), 0, 0, 0, 0,
                    valadded * 6 + 500 + 1, bridges, TVI_LAST);
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("BigBridgeObList"), 0, 0, 0, 0, valadded * 6 + 500 + 0,
                    bridges, TVI_LAST);
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("BigTrackBridgeObList"), 0, 0, 0, 0,
                    valadded * 6 + 500 + 2, bridges, TVI_LAST);
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("SmallConcreteBridgeObList"), 0, 0, 0, 0,
                    valadded * 6 + 500 + 3, bridges, TVI_LAST);
  } else {
  ...
  if (!theApp.m_Options.bEasy && isTrue(g_data.sections["Debug"].values["AllowTunnels"])) {
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("NewTunnelObList"), 0, 0, 0, 0, 50, rootitems[9],
                    TVI_LAST);
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("ModifyTunnelObList"), 0, 0, 0, 0, 51, rootitems[9],
                    TVI_LAST);
    if (isTrue(g_data.sections["Debug"].values["AllowUnidirectionalTunnels"])) {
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("NewTunnelSingleObList"), 0, 0, 0, 0, 52,
                      rootitems[9], TVI_LAST);
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("ModifyTunnelSingleObList"), 0, 0, 0, 0, 53,
                      rootitems[9], TVI_LAST);
    }
    tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("DelTunnelObList"), 0, 0, 0, 0, 54, rootitems[9],
                    TVI_LAST);
  }
  ...
  if (!theApp.m_Options.bEasy || !Map->IsMultiplayer()) {
    if (ini.sections.find(MAPHOUSES) != ini.sections.end() && ini.sections[MAPHOUSES].values.size() > 0) {
      for (i = 0; i < ini.sections[MAPHOUSES].values.size(); i++) {
        ...
        tree.InsertItem(TVIF_PARAM | TVIF_TEXT, TranslateHouse(*ini.sections[MAPHOUSES].GetValue(i), TRUE), 0, 0, 0, 0,
                        valadded * 7 + i, rootitems[11], TVI_LAST);
      }
      ...
      for (i = 0; i < rules.sections[HOUSES].values.size(); i++) {
        ...
        j.MakeLower();
        if (j == "nod" || j == "gdi") continue;
        TV_InsertItemW(tree.m_hWnd, CCStrings[*rules.sections[HOUSES].GetValue(i)].wString,
                       CCStrings[*rules.sections[HOUSES].GetValue(i)].len, TVI_LAST, rootitems[11], valadded * 7 + i);
      }
    }
  } else {
  ...
  for (i = 0; i < overlay_count; i++) {
    if (overlay_visible[i] && (!yr_only[i] || yuri_mode)) {
      if (!overlay_trdebug[i] || isTrue(g_data.sections["Debug"].values["EnableTrackLogic"]))
        tree.InsertItem(TVIF_PARAM | TVIF_TEXT, TranslateStringACP(overlay_name[i]), 0, 0, 0, 0,
                        valadded * 6 + 3000 + overlay_number[i], alloverlay, TVI_LAST);
    }
  }
  ...
  for (i = 0; i < ini.sections["InfantryTypes"].values.size(); i++) {
    ...
    if (strlen(ini.sections[*ini.sections["InfantryTypes"].GetValue(i)].values["Name"]) > 0)
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, ini.sections[*ini.sections["InfantryTypes"].GetValue(i)].values["Name"],
                      0, 0, 0, 0, valadded * 1 + rules.sections["InfantryTypes"].values.size() + i, rootitems[0],
                      TVI_LAST);
    else
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, (*ini.sections["InfantryTypes"].GetValue(i) + " NOTDEFINED"), 0, 0, 0, 0,
                      valadded * 1 + rules.sections["InfantryTypes"].values.size() + i, rootitems[0], TVI_LAST);
  }
  ...
  for (i = 0; i < rules.sections["BuildingTypes"].values.size(); i++) {
    ...
    if (Map->GetTheater() == THEATER5 && g_data.sections["IgnoreDesertRA2"].FindValue(unitname) >= 0) continue;
    if (g_data.sections["IgnoreRA2"].FindValue(unitname) >= 0) continue;
    if (!isTrue(g_data.sections["Debug"].GetValueByName("ShowBuildingsWithToTile", "0")) &&
        !rules.sections[unitname].GetValueByName("ToTile").IsEmpty())
      continue;
    ...
    if (g_data.sections.find(unitname) != g_data.sections.end() &&
        g_data.sections[unitname].values.find("AIBasePlanningSide") != g_data.sections[unitname].values.end()) {
    ...
    if (theater == THEATER0 && !buildinginfo[id].bTemp) { /*MessageBox("Ignored", unitname,0);*/
      continue;
    }
    if (theater == THEATER1 && !buildinginfo[id].bSnow) { /*MessageBox("Ignored", unitname,0);*/
      continue;
    }
    if (theater == THEATER2 && !buildinginfo[id].bUrban) { /*MessageBox("Ignored", unitname,0);*/
      continue;
    }
    ...
    if (bAdded == FALSE) {
      ...
      TV_InsertItemW(tree.m_hWnd, addedString, wcslen(addedString), TVI_LAST, structhouses[sides.size()],
                     valadded * 2 + i);
    }
  ...
  for (i = 0; i < ini.sections["BuildingTypes"].values.size(); i++) {
    ...
    if (id < 0 ||
        (buildinginfo[id].pic[0].bTerrain != TheaterChar::None && buildinginfo[id].pic[0].bTerrain != needed_terrain))
      continue;
    ...
    if (strlen(ini.sections[*ini.sections["BuildingTypes"].GetValue(i)].values["Name"]) > 0) {
      ...
      for (e = 0; e < sides.size(); e++) {
        if (isIncluded(owner, sides[e].name)) {
          ...
          tree.InsertItem(TVIF_PARAM | TVIF_TEXT, addedString, 0, 0, 0, 0, valadded * 2 + i, structhouses[e + 1],
                          TVI_LAST);
        }
      }
    } else {
      ...
      for (e = 0; e < sides.size(); e++) {
        if (!addedfor[sides[e].orig_n])
          if (isIncluded(owner, sides[e].name) || (yuri_mode && e == 2 && owner == "YuriCountry")) {
            tree.InsertItem(TVIF_PARAM | TVIF_TEXT, addedString, 0, 0, 0, 0, valadded * 2 + i, structhouses[e],
                            TVI_LAST);
            bAdded = TRUE;
            addedfor[sides[e].orig_n] = TRUE;
  ...
  for (i = 0; i < ini.sections["AircraftTypes"].values.size(); i++) {
    ...
    if (strlen(ini.sections[*ini.sections["AircraftTypes"].GetValue(i)].values["Name"]) > 0)
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, ini.sections[*ini.sections["AircraftTypes"].GetValue(i)].values["Name"],
                      0, 0, 0, 0, valadded * 3 + i + rules.sections["AircraftTypes"].values.size(), rootitems[2],
                      TVI_LAST);
    else
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, (*ini.sections["AircraftTypes"].GetValue(i) + " NOTDEFINED"), 0, 0, 0, 0,
                      valadded * 3 + i + rules.sections["AircraftTypes"].values.size(), rootitems[2], TVI_LAST);
  }
  ...
  for (i = 0; i < ini.sections["VehicleTypes"].values.size(); i++) {
    ...
    if (strlen(ini.sections[*ini.sections["VehicleTypes"].GetValue(i)].values["Name"]) > 0)
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, ini.sections[*ini.sections["VehicleTypes"].GetValue(i)].values["Name"], 0,
                      0, 0, 0, valadded * 4 + i + rules.sections["VehicleTypes"].values.size(), rootitems[1], TVI_LAST);
    else
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, (*ini.sections["VehicleTypes"].GetValue(i) + " NOTDEFINED"), 0, 0, 0, 0,
                      valadded * 4 + i + rules.sections["VehicleTypes"].values.size(), rootitems[1], TVI_LAST);
  }
  ...
  tree.InsertItem(TVIF_PARAM | TVIF_TEXT, GetLanguageStringACP("RndTreeObList"), 0, 0, 0, 0, valadded * 5 + 999, hTrees,
                  TVI_LAST);
  ...
  for (i = 0; i < rules.sections["TerrainTypes"].values.size(); i++) {
    ...
    if (unitname.GetLength() > 0 && unitname != "VEINTREE" && unitname.Find("ICE") < 0 && unitname.Find("BOXES") < 0 &&
        unitname.Find("SPKR") < 0)  // out with it :-)
      tree.InsertItem(TVIF_PARAM | TVIF_TEXT, (addedString + " (" + unitname + ")"), 0, 0, 0, 0, valadded * 5 + i,
                      howner, TVI_LAST);
