...
CLoading::CLoading(CWnd* pParent /*=NULL*/) : CDialog(CLoading::IDD, pParent) {
...
void CLoading::InitPics(CProgressCtrl* prog) {
  ...
  if (!theApp.m_Options.bDoNotLoadBMPs) {
    ...
    if (ff.FindFile(bmps)) {
      ...
      for (k = 0; k < m_bmp_count + 1; k++) {
        ...
        try {
          pics[(LPCTSTR)ff.GetFileName()].pic =
              BitmapToSurface(((CFinalSunDlg*)theApp.m_pMainWnd)->m_view.m_isoview->dd,
                              *BitmapFromFile(ff.GetFilePath()))
                  .Detach();
  ...
  try {
    pics["SCROLLCURSOR"].pic =
        BitmapToSurface(((CFinalSunDlg*)theApp.m_pMainWnd)->m_view.m_isoview->dd, *BitmapFromResource(IDB_SCROLLCURSOR))
            .Detach();
  ...
  try {
    pics["CELLTAG"].pic =
        BitmapToSurface(((CFinalSunDlg*)theApp.m_pMainWnd)->m_view.m_isoview->dd, *BitmapFromResource(IDB_CELLTAG))
            .Detach();
  ...
  try {
    pics["FLAG"].pic =
        BitmapToSurface(((CFinalSunDlg*)theApp.m_pMainWnd)->m_view.m_isoview->dd, *BitmapFromResource(IDB_FLAG))
            .Detach();
...
// TODO: this was made for Win9x. It does not work anymore on modern operating systems if you don't run the editor as
// administrator (which you should not do)
...
void CLoading::LoadTSIni(LPCTSTR lpFilename, CIniFile* lpIniFile, BOOL bIsExpansion, BOOL bCheckEditorDir) {
  ...
  if (theApp.m_Options.bSearchLikeTS) {
    ...
    for(i=1;i<101;i++)
    {
            ...
            {
                    if(FSunPackLib::XCC_ExtractFile(lpFilename, (CString)TSPath+(CString)"FinalSun"+lpFilename,
    m_hExpand[i].hExpand))
                    {
...
__forceinline void Blit_PalD(BYTE* dst, RECT blrect, const BYTE* src, RECT srcRect, int swidth, int dwidth, int sheight,
                             int dheight, const BYTE* const srcMask = nullptr) {
  if (src == NULL || dst == NULL) return;
...
std::optional<FindShpResult> CLoading::FindUnitShp(const CString& image, char preferred_theat,
                                                   const CIniFileSection& artSection) {
  if (image.IsEmpty()) return std::nullopt;
  ...
  if (isTheater) {
    for (int t = 0; !(curSuffix = kSuffixesToTry[t]).IsEmpty(); ++t) {
      ...
      if (curMixFile)
        return FindShpResult(curMixFile, curMixTheater, curFilename, toTheaterChar(suffixToTheat(curSuffix)),
                             forcedPalette ? forcedPalette : GetIsoPalette(suffixToTheat(curSuffix)));
    }
  }
  // Phase 1: if NewTheater is enabled and first character indicates support, try with real theater, then in order of
  // kTheatersToTry Otherwise,
  if (isNewTheater) {
    if (firstCharSupportsTheater) {
      /*curFilename.SetAt(1, preferred_theat);
      curMixFile = FindFileInMix(curFilename, &curMixTheater);
      if (curMixFile)
              return FindShpResult(curMixFile, curMixTheater, curFilename, toTheaterChar(preferred_theat),
      GetUnitPalette(preferred_theat));*/
      for (int t = 0; curTheater = kTheatersToTry[t]; ++t) {
        curFilename.SetAt(1, curTheater);
        curMixFile = FindFileInMix(curFilename, &curMixTheater);
        if (curMixFile)
          return FindShpResult(curMixFile, curMixTheater, curFilename, toTheaterChar(curTheater),
                               forcedPalette ? forcedPalette : unitOrIsoPalette);
      }
    }
  }
  ...
  for (int t = 0; curTheater = kTheatersToTry[t]; ++t) {
    ...
    if (curMixFile)
      return FindShpResult(curMixFile, curMixTheater, curFilename, toTheaterChar(curTheater),
                           forcedPalette ? forcedPalette : GetIsoPalette(curTheater));
  }

  return std::nullopt;
}

int lepton_to_screen_y(int leptons) { return leptons * f_y / 256; }
...
BOOL CLoading::LoadUnitGraphic(LPCTSTR lpUnittype) {
  ...
  WORD wStep = 1;  // step is 1 for infantry, buildings, etc, and for shp vehicles it specifies the step rate between
                   // every direction
  ...
  if (!isTrue(art.sections[image].values["Voxel"]))  // is it a shp graphic?
  {
    try {
      ...
      auto limited_to_theater =
          isTrue(artSection.GetValueByName("TerrainPalette")) ? shp->mixfile_theater : TheaterChar::None;
      ...
      if (hShpMix > 0) {
        ...
        if (!isTrue(g_data.sections["IgnoreSuperAnim3"].values[image]))
          LoadBuildingSubGraphic("SuperAnimThree", artSection, bAlwaysSetChar, theat, hShpMix, superanim3_h,
                                 superanim3);
        ...
        LoadBuildingSubGraphic("SpecialAnimTwo", artSection, bAlwaysSetChar, theat, hShpMix, specialanim2_h,
                               specialanim2);
        LoadBuildingSubGraphic("SpecialAnimThree", artSection, bAlwaysSetChar, theat, hShpMix, specialanim3_h,
                               specialanim3);
        LoadBuildingSubGraphic("SpecialAnimFour", artSection, bAlwaysSetChar, theat, hShpMix, specialanim4_h,
                               specialanim4);
        ...
        if (isTrue(rules.sections[image].values["Turret"])) {
          ...
          if (bStructure && turretanim_name.GetLength() > 0 &&
              isFalse(rules.sections[image].values["TurretAnimIsVoxel"])) {
            turretanim_filename = turretanim_name + ".shp";
            if (art.sections[turretanim_name].values.find("Image") != art.sections[turretanim_name].values.end())
              turretanim_filename = art.sections[turretanim_name].values["Image"] + ".shp";

            if (isTrue(artSection.GetValueByName("NewTheater"))) {
              auto tmp = turretanim_filename;
              tmp.SetAt(1, theat);
              if (FSunPackLib::XCC_DoesFileExist(tmp, hShpMix)) turretanim_filename = tmp;
            }
          ...
          } else if ((bStructure && turretanim_name.GetLength() > 0 &&
                      isTrue(rules.sections[image].values["TurretAnimIsVoxel"])) ||
                     (!bStructure &&
                      (FindFileInMix(vxl_turretanim_filename) || FindFileInMix(vxl_barrelanim_filename)))) {
            ...
            if (hVXL) {
              ...
              if (FSunPackLib::SetCurrentVXL(turretanim_filename, hVXL)) {
                ...
                for (i = 0; i < 8; i++) {
                  ...
                  if (!FSunPackLib::LoadVXLImage(*m_voxelNormalTables, lightDirection, rotation, turretModelOffset,
                                                 turretColors[i], turretLighting[i], &center_x, &center_y, 0, 0, 0, 0,
                                                 0, &r)) {
                  } else {
            ...
            if (hBarl) {
              ...
              if (FSunPackLib::SetCurrentVXL(barrelanim_filename, hBarl)) {
                // we assume the voxel normal class is always the same for the combined voxels
                FSunPackLib::GetVXLSectionInfo(0, vnc);
                ...
                for (i = 0; i < 8; i++) {
                  ...
                  int center_x, center_y;
                  if (!FSunPackLib::LoadVXLImage(*m_voxelNormalTables, lightDirection, rotation, turretModelOffset,
                                                 vxlBarrelColors[i], vxlBarrelLighting[i], &center_x, &center_y,
                                                 atoi(rules.sections[image].values["TurretAnimZAdjust"]), 0, 0, 0, 0,
                                                 &r)) {
                  } else {
        ...
        if (art.sections[image].values["Palette"] == "lib") hPalette = m_hPalLib;
        ...
        if (bSuccess) {
          ...
          } else if (wStep == 1 && (rules.sections[lpUnittype].values["PowersUpBuilding"].GetLength() == 0 ||
                                    !isTrue(rules.sections[lpUnittype].values["Turret"]))) {  // standard case...
          ...
          } else if (rules.sections[lpUnittype].values["PowersUpBuilding"].GetLength() != 0 &&
                     isTrue(rules.sections[lpUnittype].values["Turret"])) {  // a "real" turret (vulcan cannon, etc...)
            for (i = 0; i < maxPics; i++) {
          ...
          for (i = 0; i < maxPics; i++) {
            ...
            if (!vxlBarrelColors[i].empty() && (i == 1 || i == 0 || i == 7)) {
              ...
              XMover +=
                  atoi(g_data.sections["BuildingVoxelBarrelsRA2"].values[(CString)lpUnittype + (CString) "X" + c]);
              YMover +=
                  atoi(g_data.sections["BuildingVoxelBarrelsRA2"].values[(CString)lpUnittype + (CString) "Y" + c]);
              ...
              errstream << "vxl barrel: " << i << " size: " << ddsd.dwWidth << " " << ddsd.dwHeight << " at "
                        << destRect.left << " " << destRect.top << endl;
              errstream.flush();
              Blit_PalD(lpT[i], destRect, vxlBarrelColors[i].data(), srcRect, ddsd.dwWidth, head.cx, ddsd.dwHeight,
                        head.cy);
              Blit_PalD(lighting[i].data(), destRect, vxlBarrelLighting[i].data(), srcRect, ddsd.dwWidth, head.cx,
                        ddsd.dwHeight, head.cy, vxlBarrelColors[i].data());
            }
            ...
            if (!turretColors[i].empty()) {
              ...
              XMover +=
                  atoi(g_data.sections["BuildingVoxelTurretsRA2"].values[(CString)lpUnittype + (CString) "X" + c]);
              YMover +=
                  atoi(g_data.sections["BuildingVoxelTurretsRA2"].values[(CString)lpUnittype + (CString) "Y" + c]);
              ...
              } else  // !bVoxelTurret
              {
                ...
                int my = atoi(rules.sections[image]
                                  .values["TurretAnimY"]);  //+atoi(rules.sections[image].values["barrelAnimZAdjust"]);
              ...
              errstream << "vxl turret: " << i << " size: " << ddsd.dwWidth << " " << ddsd.dwHeight << " at "
                        << destRect.left << " " << destRect.top << endl;
              errstream.flush();
              Blit_PalD(lpT[i], destRect, turretColors[i].data(), srcRect, ddsd.dwWidth, head.cx, ddsd.dwHeight,
                        head.cy);
              Blit_PalD(lighting[i].data(), destRect, turretLighting[i].data(), srcRect, ddsd.dwWidth, head.cx,
                        ddsd.dwHeight, head.cy, turretColors[i].data());
            }
            ...
            if (!bPowerUp && i != 0 &&
                (imghead.unknown == 0 && !isTrue(g_data.sections["Debug"].values["IgnoreSHPImageHeadUnused"])) &&
                bStructure) {
              ...
              PICDATA p;
              p.pic = lpT[i];
              if (std::find_if(lighting[i].begin(), lighting[i].end(), [](const BYTE b) { return b != 255; }) !=
                  lighting[i].end())
              ...
              if (hPalette == m_hPalIsoTemp || hPalette == m_hPalIsoUrb || hPalette == m_hPalIsoSnow ||
                  hPalette == m_hPalIsoUbn || hPalette == m_hPalIsoDes || hPalette == m_hPalIsoLun)
                p.pal = iPalIso;
              if (hPalette == m_hPalTemp || hPalette == m_hPalUrb || hPalette == m_hPalSnow || hPalette == m_hPalUbn ||
                  hPalette == m_hPalLun || hPalette == m_hPalDes)
                p.pal = iPalTheater;
              if (hPalette == m_hPalUnitTemp || hPalette == m_hPalUnitUrb || hPalette == m_hPalUnitSnow ||
                  hPalette == m_hPalUnitDes || hPalette == m_hPalUnitLun || hPalette == m_hPalUnitUbn)
                p.pal = iPalUnit;
    ...
    for (i = 0; i < 8; i++) {
      ...
      if (isTrue(rules.sections[lpUnittype].values["Turret"])) {
        if (FSunPackLib::SetCurrentVXL(image + "tur.vxl", hMix)) {
          FSunPackLib::LoadVXLImage(*m_voxelNormalTables, lightDirection, rotation, turretModelOffset, turretColors,
                                    turretNormals, &turret_x, &turret_y, 0, &turret_x_zmax, &turret_y_zmax, -1, -1,
                                    &lprT);
        }
        if (FSunPackLib::SetCurrentVXL(image + "barl.vxl", hMix)) {
          FSunPackLib::LoadVXLImage(*m_voxelNormalTables, lightDirection, rotation, turretModelOffset, barrelColors,
                                    barrelNormals, &barrel_x, &barrel_y, 0, NULL, NULL, 0, 0, &lprB);
        }
      }
      ...
      if (!FSunPackLib::LoadVXLImage(*m_voxelNormalTables, lightDirection, rotation, Vec3f(), colors, lighting,
                                     &xcenter, &ycenter, 0, &xcenter_zmax, &ycenter_zmax, -1, -1, &r)) {
        return FALSE;
      }

      FSunPackLib::VoxelNormalClass vnc = FSunPackLib::VoxelNormalClass::Unknown;
      FSunPackLib::GetVXLSectionInfo(
          0, vnc);  // we assume the normal class for all voxels sections and turrets or barrels is the same
      ...
      if (turretColors.size()) {
        ...
        Blit_PalD(colors.data(), destRect, turretColors.data(), srcRect, ddsdT.dwWidth, ddsd.dwWidth, ddsdT.dwHeight,
                  ddsd.dwHeight);
        Blit_PalD(lighting.data(), destRect, turretNormals.data(), srcRect, ddsdT.dwWidth, ddsd.dwWidth, ddsdT.dwHeight,
                  ddsd.dwHeight, turretColors.data());
      ...
      if (barrelColors.size()) {
        ...
        Blit_PalD(colors.data(), destRect, barrelColors.data(), srcRect, ddsdB.dwWidth, ddsd.dwWidth, ddsdB.dwHeight,
                  ddsd.dwHeight);
        Blit_PalD(lighting.data(), destRect, barrelNormals.data(), srcRect, ddsdB.dwWidth, ddsd.dwWidth, ddsdB.dwHeight,
                  ddsd.dwHeight, barrelColors.data());
...
void CLoading::LoadBuildingSubGraphic(const CString& subkey, const CIniFileSection& artSection, BOOL bAlwaysSetChar,
                                      char theat, HMIXFILE hShpMix, SHPHEADER& shp_h, BYTE*& shp) {
  ...
  if (subname.GetLength() > 0) {
    ...
    if (isTrue(artSection.GetValueByName("NewTheater")) || bAlwaysSetChar || subfilename.GetAt(0) == 'G' ||
    subfilename.GetAt(0) == 'N' || subfilename.GetAt(0) == 'Y' || subfilename.GetAt(0) == 'C')
    {
  ...
  if (!isTrue(art.sections[image].values["Voxel"]))  // is it a shp graphic?
  {
    try {
      ...
      if (isTrue(art.sections[image].values["NewTheater"]))  //&&
                                                             //isTrue(artSection.GetValueByName("TerrainPalette")))//(filename.GetAt(0)=='G'
                                                             //|| filename.GetAt(0)=='N' || filename.GetAt(0)=='C') &&
                                                             //filename.GetAt(1)=='A')
      {
      ...
      if (filename == "tibtre01.tem" || filename == "tibtre02.tem" || filename == "tibtre03.tem" ||
          filename == "veinhole.tem") {
        hPalette = m_hPalUnitTemp;
      }
      ...
      if (hShpMix > 0) {
        ...
        if (rules.sections[lpUnittype].values["Bib"] != "no")  // seems to be ignored by TS, art.ini overwrites???
        {
          ...
          if (bibname.GetLength() > 0) {
            ...
            if (isTrue(art.sections[image].values["NewTheater"])) bibfilename.SetAt(1, theat);
        ...
        if (activeanim_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) activeanim_filename.SetAt(1, theat);
        ...
        if (idleanim_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) idleanim_filename.SetAt(1, theat);
        ...
        if (activeanim2_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) activeanim2_filename.SetAt(1, theat);
        ...
        if (activeanim3_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) activeanim3_filename.SetAt(1, theat);
        ...
        if (superanim1_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) superanim1_filename.SetAt(1, theat);
        ...
        if (superanim2_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) superanim2_filename.SetAt(1, theat);
        ...
        if (superanim3_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) superanim3_filename.SetAt(1, theat);
        ...
        if (superanim4_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) superanim4_filename.SetAt(1, theat);
        ...
        if (specialanim1_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) specialanim1_filename.SetAt(1, theat);
        ...
        if (specialanim2_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) specialanim2_filename.SetAt(1, theat);
        ...
        if (specialanim3_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) specialanim3_filename.SetAt(1, theat);
        ...
        if (specialanim4_name.GetLength() > 0) {
          ...
          if (isTrue(art.sections[image].values["NewTheater"])) specialanim4_filename.SetAt(1, theat);
        ...
        turretanim_name = rules.sections[image].values["TurretAnim"];
        if (bStructure && rules.sections[image].values["Turret"] == "yes" && turretanim_name.GetLength() > 0 &&
            rules.sections[image].values["TurretAnimIsVoxel"] != "true") {
          turretanim_filename = turretanim_name + ".shp";
          if (art.sections[turretanim_name].values.find("Image") != art.sections[turretanim_name].values.end())
            turretanim_filename = art.sections[turretanim_name].values["Image"] + ".shp";

          if (isTrue(art.sections[image].values["NewTheater"])) turretanim_filename.SetAt(1, theat);
        ...
        } else if (bStructure && rules.sections[image].values["Turret"] == "yes" && turretanim_name.GetLength() > 0 &&
                   rules.sections[image].values["TurretAnimIsVoxel"] == "true") {
          turretanim_filename = turretanim_name + ".vxl";
          if (art.sections[turretanim_name].values.find("Image") != art.sections[turretanim_name].values.end())
            turretanim_filename = art.sections[turretanim_name].values["Image"] + ".vxl";
          ...
          if (hVXL) {
            ...
            if (FSunPackLib::SetCurrentVXL(turretanim_filename, hVXL)) {
              ...
              for (i = 0; i < 8; i++) {
                ...
                int center_x, center_y;
                if (!FSunPackLib::LoadVXLImageInSurface(*m_voxelNormalTables, lightDirection, v.dd, 0, 1, r_x, r_y, r_z,
                                                        &turrets[i], hPalette, &center_x, &center_y,
                                                        atoi(rules.sections[image].values["TurretAnimZAdjust"]))) {
                } else {
                  DDSURFACEDESC2 ddsd;
                  memset(&ddsd, 0, sizeof(DDSURFACEDESC2));
                  ddsd.dwSize = sizeof(DDSURFACEDESC2);
          ...
          for (i = 0; i < maxPics; i++) {
            ...
            if (turrets[i] != NULL) {
              ...
              if (iMove < 0) {
                ...
                int mx = imghead.x / 2 + imghead.cx / 2 + (-turretinfo[i].x / 2 - turretinfo[i].cx) +
                         atoi(rules.sections[image].values["TurretAnimX"]);
                int my = imghead.y / 2 + imghead.cy / 2 + (-turretinfo[i].y / 2 - turretinfo[i].cy) +
                         atoi(rules.sections[image]
                                  .values["TurretAnimY"]);  //+atoi(rules.sections[image].values["TurretAnimZAdjust"]);
                ...
                if (ddsd.dwWidth != head.cx || ddsd.dwHeight != head.cy) {
                  ...
                  my = imghead.y + imghead.cy / 2 + turretinfo[i].y +
                       atoi(rules.sections[image]
                                .values["TurretAnimY"]);  //+atoi(rules.sections[image].values["TurretAnimZAdjust"])/2;
              ...
              if (lpT[i]->Blt(&destRect, turrets[i], &srcRect, DDBLT_KEYSRC | DDBLT_WAIT, &fx) != DD_OK) {
                errstream << "vxl turret: " << i << " size: " << ddsd.dwWidth << " " << ddsd.dwHeight << "  failed"
                          << endl;
                errstream.flush();
                // exit(-99);
              }
    ...
    try {
      for (i = 0; i < 8; i++) {
        ...
        if (isTrue(rules.sections[lpUnittype].values["Turret"])) {
          if (FSunPackLib::SetCurrentVXL(image + "tur.vxl", hMix)) {
            FSunPackLib::LoadVXLImageInSurface(*m_voxelNormalTables, lightDirection, v.dd, 0, 1, r_x, r_y, r_z,
                                               &lpTurret, m_hPalUnitTemp, &turret_x, &turret_y, 0, &turret_x_zmax,
                                               &turret_y_zmax, -1, -1);
          }
          if (FSunPackLib::SetCurrentVXL(image + "barl.vxl", hMix)) {
            FSunPackLib::LoadVXLImageInSurface(*m_voxelNormalTables, lightDirection, v.dd, 0, 1, r_x, r_y, r_z,
                                               &lpBarrel, m_hPalUnitTemp, &barrel_x, &barrel_y, 0, NULL, NULL, 0, 0);
          }
        }
        ...
        if (!FSunPackLib::LoadVXLImageInSurface(*m_voxelNormalTables, lightDirection, v.dd, 0, 1, r_x, r_y, r_z, &lpT,
                                                m_hPalUnitTemp, &xcenter, &ycenter, 0, &xcenter_zmax, &ycenter_zmax)) {
          return FALSE;
        }
...
BOOL CLoading::InitMixFiles() {
  ...
  if (DoesFileExist((CString)TSPath + "\\ra2md.mix")) yuri_mode = TRUE;  // MW Apr 17th, make it available right here!
...
HMIXFILE CLoading::FindFileInMix(LPCTSTR lpFilename, TheaterChar* pTheaterChar) {
  if (pTheaterChar) *pTheaterChar = TheaterChar::None;
  ...
  for (i = 100; i >= 0; i--) {
    ...
    if (cuExp != NULL) {
      if (FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp)) return cuExp;
    }
  }
  ...
  for (i = 100; i >= 0; i--) {
    ...
    if (cuExp.hExpand != NULL) {
      if (FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hExpand)) return cuExp.hExpand;
      if (cuExp.hECache != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hECache)) return cuExp.hECache;
      if (cuExp.hConquer != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hConquer)) return cuExp.hConquer;
      if (cuExp.hLocal != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hLocal)) return cuExp.hLocal;
      if (cuExp.hTemperat != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hTemperat))
        return cuExp.hTemperat;
      if (strcmp(lpFilename, "yayard.shp") == NULL) FSunPackLib::_DEBUG_EnableLogs = true;
      if (cuExp.hSnow != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hSnow)) return cuExp.hSnow;
      FSunPackLib::_DEBUG_EnableLogs = false;
      if (cuExp.hGeneric != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hGeneric)) return cuExp.hGeneric;
      if (cuExp.hUrban != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hUrban)) return cuExp.hUrban;
      if (cuExp.hUrbanN != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hUrbanN)) return cuExp.hUrbanN;
      if (cuExp.hLunar != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hLunar)) return cuExp.hLunar;
      if (cuExp.hDesert != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hDesert)) return cuExp.hDesert;
      if (cuExp.hBuildings != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hBuildings))
        return cuExp.hBuildings;
      if (cuExp.hIsoGen != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hIsoGen)) return cuExp.hIsoGen;
      ...
      if (cuExp.hSno != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hSno)) return cuExp.hSno;
      if (cuExp.hTem != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hTem)) return cuExp.hTem;
      if (cuExp.hUrb != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hUrb)) return cuExp.hUrb;
      if (cuExp.hUbn != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hUbn)) return cuExp.hUbn;
      if (cuExp.hLun != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hLun)) return cuExp.hLun;
      if (cuExp.hDes != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hDes)) return cuExp.hDes;
      if (cuExp.hMarble != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, cuExp.hMarble)) return cuExp.hMarble;
    }
  }

  if (m_hTibSun != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hTibSun)) return m_hTibSun;
  if (m_hLanguage != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hLanguage)) return m_hLanguage;
  if (m_hLangMD != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hLangMD)) return m_hLangMD;
  if (m_hLocal != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hLocal)) return m_hLocal;
  if (m_hBuildings != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hBuildings)) return m_hBuildings;
  if (m_hCache != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hCache)) return m_hCache;
  if (m_hConquer != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hConquer)) return m_hConquer;
  if (m_hTemperat != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hTemperat)) return m_hTemperat;
  if (m_hSnow != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hSnow)) return m_hSnow;
  if (m_hUrban != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hUrban)) return m_hUrban;
  if (m_hUrbanN != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hUrbanN)) return m_hUrbanN;
  if (m_hLunar != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hLunar)) return m_hLunar;
  if (m_hDesert != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hDesert)) return m_hDesert;
  ...
  if (m_hTem != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hTem)) return m_hTem;
  if (m_hSno != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hSno)) return m_hSno;
  if (m_hUrb != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hUrb)) return m_hUrb;
  if (m_hUbn != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hUbn)) return m_hUbn;
  if (m_hLun != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hLun)) return m_hLun;
  if (m_hDes != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hDes)) return m_hDes;

  if (m_hMarble != NULL && FSunPackLib::XCC_DoesFileExist(lpFilename, m_hMarble)) return m_hMarble;
...
void CLoading::InitTMPs(CProgressCtrl* prog) {
  ...
  *tiledata_count = tcount;
  ...
  for (i = 0; i < 10000; i++) {
    ...
    if (tiles->sections[sec].values["AllowTiberium"] == "true") bTib = TRUE;
    tiles->sections[sec].values["Morphable"].MakeLower();
    if (tiles->sections[sec].values["Morphable"] == "true") bMorph = TRUE;
    tiles->sections[sec].values["AllowToPlace"].MakeLower();
    if (tiles->sections[sec].values["AllowToPlace"] == "no") bPlace = FALSE;
    tiles->sections[sec].values["NonMarbleMadness"].MakeLower();
    if (tiles->sections[sec].values["NonMarbleMadness"].GetLength() > 0) bMadness = TRUE;
    ...
    for (e = 0; e < atoi(tiles->sections[sec].values["TilesInSet"]); e++) {
      ...
      for (reps = 0; reps < 5; reps++) {
        ...
        if (reps > 0) {
          ...
          if (!LoadTile(r_filename, FindFileInMix(filename), hPalette, tilecount, TRUE)) break;
        } else {
      ...
      // if(FSunPackLib::LoadTMPImageInSurface(v.dd,lpFilename, 0, tileCount, pics, hPalette, hOwner))
      {
        ...
        for (i = 0; i < tileCount; i++) {
          if (pics[i] != NULL) {
            ...
            POINT p;
            FSunPackLib::XCC_GetTMPTileInfo(i, &p, &cx, &cy, &direction, &height, &terraintype, &td->tiles[i].rgbLeft,
                                            &td->tiles[i].rgbRight);
...
#  endif
            ...
            if ((*tiledata)[dwID].wTileSet == waterset)
              (*tiledata)[dwID].tiles[i].bHackedTerrainType = TERRAINTYPE_WATER;
          } else {
            td->tiles[i].pic = NULL;
            td->tiles[i].sX = 0;
            td->tiles[i].sY = 0;
...
void CLoading::LoadOverlayGraphic(LPCTSTR lpOvrlName_, int iOvrlNum) {
  ...
  if (unitPalettePrefixes.end() !=
      std::find_if(unitPalettePrefixes.begin(), unitPalettePrefixes.end(),
                   [&sOvrlName](const auto& pair) { return sOvrlName.Find(pair.second) == 0; })) {
    forcedPalette = GetUnitPalette(cur_theat);
  }
  if (isoPalettePrefixes.end() !=
      std::find_if(isoPalettePrefixes.begin(), isoPalettePrefixes.end(),
                   [&sOvrlName](const auto& pair) { return sOvrlName.Find(pair.second) == 0; })) {
  ...
  if (hMix == NULL) {
    filename = image + ".shp";
    if (isTrue(artSection.GetValueByName("NewTheater"))) filename.SetAt(1, theat);
  ...
  if (hMix != NULL) {
    ...
    FSunPackLib::SetCurrentSHP(filename, hMix);
    {
      ...
      for (i = 0; i < maxPics; i++) {
        ...
        if (imghead.unknown == 0 &&
            !isTrue(g_data.sections["Debug"].values["IgnoreSHPImageHeadUnused"]))  // is it a shadow or not used image?
          if (lpT[i]) {
            delete[] lpT[i];
            lpT[i] = NULL;
          }
        ...
        if (/*imghead.unknown &&*/ lpT[i]) {
          ...
          if (hPalette == m_hPalIsoTemp || hPalette == m_hPalIsoUrb || hPalette == m_hPalIsoSnow ||
              hPalette == m_hPalIsoDes || hPalette == m_hPalIsoLun || hPalette == m_hPalIsoUbn)
            p.pal = iPalIso;
          if (hPalette == m_hPalTemp || hPalette == m_hPalUrb || hPalette == m_hPalSnow || hPalette == m_hPalDes ||
              hPalette == m_hPalLun || hPalette == m_hPalUbn)
            p.pal = iPalTheater;
          if (hPalette == m_hPalUnitTemp || hPalette == m_hPalUnitUrb || hPalette == m_hPalUnitSnow ||
              hPalette == m_hPalUnitDes || hPalette == m_hPalUnitLun || hPalette == m_hPalUnitUbn)
            p.pal = iPalUnit;
...
void CLoading::LoadOverlayGraphic(LPCTSTR lpOvrlName_, int iOvrlNum) {
  ...
  if (hMix == NULL) {
    filename = image + ".shp";
    if (isTrue(art.sections[image].values["NewTheater"])) filename.SetAt(1, theat);
...
void CLoading::OnDestroy() { CDialog::OnDestroy(); }
...
BOOL CLoading::InitDirectDraw() {
  ...
  if (!pf.dwBBitMask || !pf.dwRBitMask || !pf.dwGBitMask) {
    ShowWindow(SW_HIDE);
    MessageBox(
        "You must not use a palette color mode like 8 bit in order to run FinalSun/FinalAlert 2. Please check "
        "readme.txt",
        "Error", MB_OK);
  ...
  if (v.dd->CreateSurface(&ddsd, &v.lpdsTemp, NULL) != DD_OK) {
    ...
    v.lpdsBack->Release();
    v.lpdsBack = NULL;
    if (v.lpdsBackHighRes) v.lpdsBackHighRes->Release();
...
void CLoading::PrepareHouses() {
  ...
  for (i = 0; i < rules.sections[HOUSES].values.size(); i++) {
    ...
    for (e = 0; e < sides.size(); e++) {
      house.side = NULL;
      if (sides[e].name == rules.sections[house.name].values["Side"]) house.side = &sides[e];
    }
  }
}
    ...
    if (rules.GetSection(i)->FindName("UIName") >= 0) {
      ...
      if (strings.find(rules.GetSection(i)->values["UIName"]) != strings.end()) {
        ...
        if (!strings[rules.GetSection(i)->values["UIName"]].bUsedDefault) {
          // CCStrings[*rules.GetSectionName(i)].cString=strings[rules.GetSection(i)->values["UIName"]].cString;
          // //.SetString(strings[rules.GetSection(i)->values["UIName"]].wString,
          // strings[rules.GetSection(i)->values["UIName"]].len);
          CCStrings[*rules.GetSectionName(i)].SetString(strings[rules.GetSection(i)->values["UIName"]].wString,
                                                        strings[rules.GetSection(i)->values["UIName"]].len);
        } else {
          CCStrings[*rules.GetSectionName(i)].SetString(strings[rules.GetSection(i)->values["UIName"]].wString,
                                                        strings[rules.GetSection(i)->values["UIName"]].len);
...
void CLoading::PrepareUnitGraphic(LPCSTR lpUnittype) {
  ...
  if (!isTrue(art.sections[image].values["Voxel"]))  // is it a shp graphic?
  {
    try {
      ...
      BOOL isNewTerrain=FALSE;
      if(isTrue(artSection.GetValueByName("NewTheater")))//&&
      isTrue(artSection.GetValueByName("TerrainPalette")))//(filename.GetAt(0)=='G' || filename.GetAt(0)=='N' ||
      filename.GetAt(0)=='C') && filename.GetAt(1)=='A')
      {
              hPalette=m_hPalUnitTemp;
              if(theat=='A') hPalette=m_hPalUnitSnow;
              if(theat=='U') hPalette=m_hPalUnitUrb;
