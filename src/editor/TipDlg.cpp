...
void CTipDlg::GetNextTipString(CString& strNext) {
  ...
  while (!bStop) {
    if (_fgetts(lpsz, MAX_BUFLEN, m_pStream) == NULL) {
      if (fseek(m_pStream, 0, SEEK_SET) != 0) AfxMessageBox(GetLanguageStringACP("CG_IDP_FILE_CORRUPT"));
    } else {
      if (*lpsz != ' ' && *lpsz != '\t' && *lpsz != '\n' && *lpsz != ';') {
        bStop = TRUE;
      }
...
HBRUSH CTipDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) {
  if (pWnd->GetDlgCtrlID() == IDC_TIPSTRING) return (HBRUSH)GetStockObject(WHITE_BRUSH);
...
BOOL CTipDlg::OnInitDialog() {
  ...
  if (m_pStream == NULL) GetDlgItem(IDC_NEXTTIP)->EnableWindow(FALSE);
