...
CTriggerEditorDlg::CTriggerEditorDlg(CWnd* pParent /*=NULL*/) : CDialog(CTriggerEditorDlg::IDD, pParent) {
...
void CTriggerEditorDlg::UpdateDialog() {
  ...
  int curData = -1;
  if (curSel != -1) curData = m_Trigger.GetItemData(curSel);
...
void CTriggerEditorDlg::OnNewtrigger() {
  ...
  // if(MessageBox("Trigger created. If you want to create a simple tag now, press Yes. The tag will be called ""New
  // tag"", you should name it like the trigger (after you have set up the trigger).","Trigger created",MB_YESNO))
  {
  ...
  int i;
  for (i = 0; i < m_Trigger.GetCount(); i++) {
    if (m_Trigger.GetItemData(i) == ini.sections["Triggers"].FindName(ID_T)) m_Trigger.SetCurSel(i);
  }
  OnSelchangeTrigger();
}
...
void CTriggerEditorDlg::OnSelchangeTrigger() { OnEditchangeTrigger(); }
...
void CTriggerEditorDlg::OnClone() {
  ...
  ini.sections["Triggers"].values[ID_T] =
      SetParam(ini.sections["Triggers"].values[ID_T], 2, GetParam(ini.sections["Triggers"].values[ID_T], 2) + " Clone");
