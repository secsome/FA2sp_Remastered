...
CComboUInputDlg::CComboUInputDlg(CWnd* pParent /*=NULL*/) : CDialog(CComboUInputDlg::IDD, pParent) {
...
BOOL CComboUInputDlg::OnInitDialog() {
  ...
  switch (m_type) {
    ...
    case COMBOUINPUT_MANUAL:
      int i;
      for (i = 0; i < m_ManualStrings.size(); i++) box->AddString(m_ManualStrings[i]);
  }
...
void CComboUInputDlg::OnOK() {
  ...
  if (m_type == COMBOUINPUT_TRIGGERS || m_type == COMBOUINPUT_TAGS || m_type == COMBOUINPUT_HOUSES_N ||
      m_type == COMBOUINPUT_COUNTRIES_N)
    TruncSpace(m_Combo);
