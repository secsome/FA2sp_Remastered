...
std::wstring utf8ToUtf16(const std::string& utf8) {
  ...
  auto unterminatedCountWChars =
      MultiByteToWideChar(CP_UTF8, MB_PRECOMPOSED | MB_ERR_INVALID_CHARS, utf8.data(), utf8Count, nullptr, 0);
  ...
  if (MultiByteToWideChar(CP_UTF8, MB_PRECOMPOSED | MB_ERR_INVALID_CHARS, utf8.data(), utf8Count, utf16.data(),
                          unterminatedCountWChars) == 0) {
    throw std::runtime_error("UTF8 -> UTF16 conversion failed");
  }
  return utf16;
}
...
std::wstring utf8ToUtf16(const char* utf8) {
  ...
  // utf8.size() instead of -1)
  auto unterminatedCountWChars =
      MultiByteToWideChar(CP_UTF8, MB_PRECOMPOSED | MB_ERR_INVALID_CHARS, utf8, utf8Count, nullptr, 0);
  if (unterminatedCountWChars == 0) {
    throw std::runtime_error("UTF8 -> UTF16 conversion failed");
  }
  ...
  std::wstring utf16;
  utf16.resize(unterminatedCountWChars);
  if (MultiByteToWideChar(CP_UTF8, MB_PRECOMPOSED | MB_ERR_INVALID_CHARS, utf8, utf8Count, utf16.data(),
                          unterminatedCountWChars) == 0) {
    throw std::runtime_error("UTF8 -> UTF16 conversion failed");
  }
  return utf16;
...
std::string utf16ToCP(const std::wstring& utf16, int CP) {
  ...
  auto unterminatedCountChars = WideCharToMultiByte(CP, CP == CP_UTF8 ? WC_ERR_INVALID_CHARS : 0, utf16.data(),
                                                    utf16Count, nullptr, 0, nullptr, nullptr);
  if (unterminatedCountChars == 0) {
    throw std::runtime_error(CP == CP_UTF8 ? "UTF16 -> UTF8 conversion failed"
                                           : "UTF16 -> MultiByte conversion failed");
  }

  std::string cps;
  cps.resize(unterminatedCountChars);
  if (WideCharToMultiByte(CP, CP == CP_UTF8 ? WC_ERR_INVALID_CHARS : 0, utf16.data(), utf16Count, cps.data(),
                          unterminatedCountChars, nullptr, nullptr) == 0) {
    throw std::runtime_error(CP == CP_UTF8 ? "UTF16 -> UTF8 conversion failed"
                                           : "UTF16 -> MultiByte conversion failed");
  }
  return cps;
...
std::string utf16ToUtf8(const std::wstring& utf16) { return utf16ToCP(utf16, CP_UTF8); }

std::string utf16ToACP(const std::wstring& utf16) { return utf16ToCP(utf16, CP_ACP); }
...
bool deleteFile(const std::string& u8FilePath) { return DeleteFileW(utf8ToUtf16(u8FilePath).c_str()) ? true : false; }
...
void ShowOptionsDialog() {
  ...
  if (((bOldPreferLocalTheaterFiles != theApp.m_Options.bPreferLocalTheaterFiles) ||
...
BOOL DoesFileExist(LPCSTR szFile) {
  ...
  HANDLE hFound = CreateFileW(file.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
...
CString GetLanguageStringACP(CString name) {
  ...
  if (language.sections[theApp.m_Options.LanguageName + "-Strings"].values.find(name) ==
      language.sections[theApp.m_Options.LanguageName + "-Strings"].values.end()) {
...
void UnitInfo(const char* data, string& house, string& type, int& strength, int& x, int& y, int& direction,
              string& action, string& actiontrigger, int& u1, int& u2, int& u3, int& u4, int& u5, int& u6) {
  house = GetParam(data, 0);
  type = GetParam(data, 1);
  strength = atoi(GetParam(data, 2));
  y = atoi(GetParam(data, 3));
...
void AirInfo(const char* data, string& house, string& type, int& strength, int& x, int& y, int& direction,
             string& action, string& actiontrigger, int& u1, int& u2, int& u3, int& u4) {
  house = GetParam(data, 0);
  type = GetParam(data, 1);
  strength = atoi(GetParam(data, 2));
  y = atoi(GetParam(data, 3));
...
void InfanteryInfo(const char* data, string& house, string& type, int& strength, int& x, int& y, int& pos,
                   string& action, int& direction, string& actiontrigger, int& u1, int& u2, int& u3, int& u4, int& u5) {
  house = GetParam(data, 0);
  type = GetParam(data, 1);
  strength = atoi(GetParam(data, 2));
  y = atoi(GetParam(data, 3));
...
void StructureInfo(const char* data, string& house, string& type, int& strength, int& x, int& y, int& direction,
                   string& action, int& u1, int& u2, int& energy, int& upgrades, int& u5, string& upgrade1,
                   string& upgrade2, string& upgrade3, int& u9, int& u10) {
  house = GetParam(data, 0);
  type = GetParam(data, 1);
  strength = atoi(GetParam(data, 2));
  y = atoi(GetParam(data, 3));
...
bool HSVToRGB(const float h, const float s, const float v, float& r, float& g, float& b) {
  if (h < 0.0 || h >= 360.0 || s < 0.0 || s > 1.0 || v < 0.0 || v > 1.0) return false;
...
void HSVToRGB(const unsigned char hsv[3], unsigned char rgb[3]) {
  ...
  for (int i = 0; i < 3; ++i) rgb[i] = (frgb[i] < 0.0 ? 0.0 : (frgb[i] > 1.0 ? 1.0 : frgb[i])) * 255.0;
}
...
std::array<unsigned char, 3> HSVToRGB(const float h, const float s, const float v) {
  ...
  for (int i = 0; i < 3; ++i) ret[i] = (frgb[i] < 0.0 ? 0.0 : (frgb[i] > 1.0 ? 1.0 : frgb[i])) * 255.0;
  return ret;
}
...
void ListCrateTypes(CComboBox& cb) { while (cb.DeleteString(0) != CB_ERR); }
void ListSpeechBubbleTypes(CComboBox& cb) { while (cb.DeleteString(0) != CB_ERR); }
...
int RepairRulesHouses() {
  ...
  for (i = 0; i < delcount; i++) rules.sections[HOUSES].values.erase(toDelete[i]);
    ...
    for (i = 0; i < ini.sections[sSection].values.size(); i++) {
      ...
      if (bNumbers) {
        ...
        if (bCountries) {
          ...
          int e;
          for (e = 0; e < i; e++) {
            if (rules.sections[sSection].FindValue(*ini.sections[sSection].GetValue(e)) >= 0) preexisting++;
          }
          if (rules.sections[sSection].FindValue(*ini.sections[sSection].GetValue(i)) >= 0) {
            itoa(rules.sections[sSection].value_orig_pos[*rules.sections[sSection].GetValueName(
                     rules.sections[sSection].FindValue(*ini.sections[sSection].GetValue(i)))],
                 c, 10);
          } else {
...
CString GetHouseSectionName(CString lpHouse) { return lpHouse + " House"; }
              ...
              if (ini.sections["Tags"].values.find(p) == ini.sections["Tags"].values.end()) {
                if (ini.sections["Actions"].values.find(p) == ini.sections["Actions"].values.end()) {
                  if (ini.sections["AITriggerTypes"].values.find(p) == ini.sections["AITriggerTypes"].values.end()) {
                    if (ini.sections.find(p) == ini.sections.end()) return p;
                  }
                }
              }
            }
  ...
  if (ini.sections.find(HOUSES) != ini.sections.end()) {
    for (owners = 0; owners < ini.sections[HOUSES].values.size(); owners++) {
      ...
      for (i = 0; i < c; i++) {
        ...
        for (j = 0; j < h; j++) {
          for (k = 0; k < w; k++) {
            if (atoi(sx) + j == x && atoi(sy) + k == y) return i;
          }
        }
      }
    }
...
std::unique_ptr<CBitmap> BitmapFromResource(int resource_id) {
  ...
  if (!bm->LoadBitmap(resource_id)) throw BitmapNotFound();
  return bm;
}

std::unique_ptr<CBitmap> BitmapFromFile(const CString& filepath) {
  std::unique_ptr<CBitmap> bm(new CBitmap);
  if (!bm->LoadBitmap(filepath)) throw BitmapNotFound();
  return bm;
}
...
CComPtr<IDirectDrawSurface4> BitmapToSurface(IDirectDraw4* pDD, const CBitmap& bitmap) {
  ...
  if (pDD->CreateSurface(&desc, &pSurface, nullptr) != DD_OK) return nullptr;

  pSurface->Restore();

  CDC bitmapDC;
  if (!bitmapDC.CreateCompatibleDC(nullptr)) return nullptr;
  bitmapDC.SelectObject(bitmap);

  HDC hSurfaceDC = nullptr;
  if (pSurface->GetDC(&hSurfaceDC) != DD_OK) return nullptr;
