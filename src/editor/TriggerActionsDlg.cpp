...
CTriggerActionsDlg::CTriggerActionsDlg(CWnd* pParent /*=NULL*/) : CDialog(CTriggerActionsDlg::IDD, pParent) {
...
void CTriggerActionsDlg::OnEditchangeActiontype() {
  ...
  ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger] =
      SetParam(ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger], pos, (LPCTSTR)eventtype);
  ...
  for (i = 0; i < 6; i++) {
    if (atoi(ptype[i]) > 0) {
      ...
      ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger] =
          SetParam(ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger], pos + i + 1, c);
    } else {
      ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger] =
          SetParam(ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger], pos + i + 1, "0");
    }
  }
  ...
  if (IsWaypointFormat(GetParam(ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger], pos + 1 + 6)) && bNoWP) {
    ...
    itoa(number, c, 10);
    ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger] =
        SetParam(ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger], pos + 1 + 6, c);
  } else if (!IsWaypointFormat(GetParam(ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger], pos + 1 + 6)) &&
             !bNoWP) {
    int wp = atoi(GetParam(ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger], pos + 1 + 6));
    CString s = GetWaypoint(wp);
    ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger] =
        SetParam(ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger], pos + 1 + 6, s);
  }
...
void CTriggerActionsDlg::OnEditchangeParamvalue() {
  ...
  } else if (curparam == -1)  // waypoint FIX MW: OR NUMBER!!!
  {
    ...
    ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger] =
        SetParam(ini.sections["Actions"].values[(LPCTSTR)m_currentTrigger], pos, (LPCTSTR)waypoint);
  }
}
...
void CTriggerActionsDlg::OnDeleteaction() {
  ...
  int posc = 1 + v * 8;
  int i;
  for (i = 0; i < 8; i++) data = SetParam(data, pos + i, GetParam(data, posc + i));
