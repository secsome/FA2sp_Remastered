...
CFinalSunDlg::CFinalSunDlg(CWnd* pParent /*=NULL*/) : CDialog(CFinalSunDlg::IDD, pParent) {
...
BOOL CFinalSunDlg::OnInitDialog() {
  ...
  m_maintoolbar.GetToolBarCtrl().SetStyle(m_maintoolbar.GetToolBarCtrl().GetStyle() | TBSTYLE_FLAT |
  ...
  m_terraintoolbar.GetToolBarCtrl().SetStyle(m_terraintoolbar.GetToolBarCtrl().GetStyle() | TBSTYLE_FLAT |
                                             TBSTYLE_TRANSPARENT | TBSTYLE_TOOLTIPS);
  ...
  m_clifftoolbar.GetToolBarCtrl().SetStyle(m_clifftoolbar.GetToolBarCtrl().GetStyle() | TBSTYLE_FLAT |
                                           TBSTYLE_TRANSPARENT | TBSTYLE_TOOLTIPS);
...
HCURSOR CFinalSunDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }

void CFinalSunDlg::OnFileQuit() { UnloadAll(); }
...
void CFinalSunDlg::OnOptionsTiberiansunoptions() { ShowOptionsDialog(); }

void CFinalSunDlg::OnFileOpenmap() {
  // CMapOpenDialog dlg(TRUE, NULL, NULL,  OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_FILEMUSTEXIST, "TS
  // maps|*.mpr;*.map|TS multi maps|*.mpr|TS single maps|*.map|");
  ...
  if ((f.sections["Map"].values["Theater"] == THEATER0 && theApp.m_Options.bDoNotLoadTemperateGraphics) ||
      (f.sections["Map"].values["Theater"] == THEATER1 && theApp.m_Options.bDoNotLoadSnowGraphics)) {
    MessageBox(
        "You have selected to don´t show temperate or snow theater, but this map uses this theater. You cannot load it "
        "without restarting FinalSun/FinalAlert 2 with this theater enabled.",
        "Error");
    return;
  }
  ...
  if (!Map->CheckMapPackData()) {
    int res = MessageBox(
        "This map seems to be corrupt. Do you want to try repairing it? If you click cancel, a empty map will be "
        "created, if you click no, it will load the map as it is",
        "Corrupt", MB_YESNOCANCEL);
    if (res == IDCANCEL) {
...
void CFinalSunDlg::UpdateDialogs(BOOL bOnlyMissionControl, BOOL bNoRepos) {
  ...
  if (!bOnlyMissionControl) {
    ...
    if (tiles != NULL && tiledata != NULL && tiledata_count != NULL) m_view.m_browser->m_bar.Update();
  }
...
void CFinalSunDlg::OnOptionsExportrulesini() {
  int res = MessageBox(
      "This will export the Rules.Ini of Tiberian Sun V1.13 MMX. ou should not modify this rules.ini because you won´t "
      "be able to play online then and ecause this could cause compatibility problems.\nIf you want to modify the "
      "rules.ini, you need to rename it before you play online.",
      "Export Rules.INI", MB_OK);
  ...
  if (dlg.DoModal() != IDCANCEL) {
    ...
    if (r == 0) {
      MessageBox("FindResource() failed to find IDR_RULES", "DEBUG");
      return;
    }
    ...
    if (hres == 0) {
      MessageBox("LoadResource() failed to load IDR_RULES", "DEBUG");
      return;
    }
    ...
    if (hfile == -1) {
      MessageBox("File could not be opened", "DEBUG");
      return;
    }
...
void CFinalSunDlg::SaveMap(CString FileName_) {
  ...
  deleteFile(tempfile);
  HANDLE hFile = CreateFileW(u16tempfile.c_str(), GENERIC_WRITE, 0, NULL, TRUNCATE_EXISTING,
                             FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
  CloseHandle(hFile);
  hFile = CreateFileW(u16tempfile.c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
                      FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
  ...
  if (hidePreview) {
    ...
    for (e = 0; e < ini.sections["Preview"].values.size(); e++) {
      fi += *ini.sections["Preview"].GetValueName(e);
      fi += "=";
    ...
    for (e = 0; e < ini.sections["PreviewPack"].values.size(); e++) {
      fi += *ini.sections["PreviewPack"].GetValueName(e);
      fi += "=";
  ...
  for (i = 0; i < ini.sections.size(); i++) {
    if (rulessections.find(*ini.GetSectionName(i)) != rulessections.end() ||
        (*ini.GetSectionName(i) == "Digest" || *ini.GetSectionName(i) == "PreviewPack" ||
         *ini.GetSectionName(i) == "Preview" || *ini.GetSectionName(i) == "Header")) {
  ...
  for (e = 0; e < ini.sections["Digest"].values.size(); e++) {
    fi += *ini.sections["Digest"].GetValueName(e);
    fi += "=";
  ...
  if (bSaveAsMMX) FileName = utf8ToUtf16(MAPFileName.GetString());
...
void CFinalSunDlg::SetReady() { m_view.m_statbar.GetStatusBarCtrl().SetText(TranslateStringACP("Ready"), 0, 0); }

void CFinalSunDlg::SetText(const char* text) { m_view.m_statbar.GetStatusBarCtrl().SetText(text, 0, 0); }
...
void se_translator(unsigned int e, _EXCEPTION_POINTERS* p) { throw (unsigned int)(e); }
...
void CFinalSunDlg::OnFileRuntiberiansun()  // or RA2
{
  ...
  BOOL success = CreateProcess(exe, NULL, NULL, NULL, FALSE, NORMAL_PRIORITY_CLASS, NULL, NULL, &si, &pi);
}

void CFinalSunDlg::OnFileImportmod() {
  /*CImportModDlg dlg(TRUE, NULL, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_FILEMUSTEXIST, "TS
  maps|*.mpr;*.map|TS multi maps|*.mpr|TS single maps|*.map|");
...
void CFinalSunDlg::OnDebugExportmappacknosections() {
  ...
  HANDLE hFile = CreateFile("c:\\MAPPACKNOSECTIONS.BIN", GENERIC_WRITE, 0, NULL, TRUNCATE_EXISTING,
                            FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
  CloseHandle(hFile);
  hFile = CreateFile("c:\\MAPPACKNOSECTIONS.BIN", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
                     FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
...
void CFinalSunDlg::OnFileNew() {
  ...
  if (!bCreateNew) {
    ...
    if (file.Find(".bmp") >= 0) {
      ...
      HBITMAP hBitmap =
          (HBITMAP)LoadImageW(NULL, utf8ToUtf16(file.GetString()).c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
      ...
      if (!bImportTrees) {
        for (i = 0; i < count; i++) Map->DeleteTerrain(0);
      }

      if (!bImportUnits) {
        count = Map->GetInfantryCount();
        for (i = 0; i < count; i++) Map->DeleteInfantry(0);
        count = Map->GetUnitCount();
        for (i = 0; i < count; i++) Map->DeleteUnit(0);
        count = Map->GetStructureCount();
        for (i = 0; i < count; i++) Map->DeleteStructure(0);
        count = Map->GetAircraftCount();
        for (i = 0; i < count; i++) Map->DeleteAircraft(0);
      }
...
void CFinalSunDlg::UpdateStrings() {
  ...
  for (i = 0; i < my_menu->GetMenuItemCount(); i++) {
    ...
    my_menu->ModifyMenu(i, mii.fState | mii.fType | MF_BYPOSITION | MF_STRING, mii.wID,
                        TranslateStringACP((LPCTSTR)str));
  ...
  if (theApp.m_Options.bDisableAutoShore)
    my_menu->CheckMenuItem(ID_OPTIONS_DISABLEAUTOSHORE, MF_BYCOMMAND | MF_CHECKED);
  if (theApp.m_Options.bDisableAutoLat) my_menu->CheckMenuItem(ID_OPTIONS_DISABLEAUTOLAT, MF_BYCOMMAND | MF_CHECKED);
  if (theApp.m_Options.bDisableSlopeCorrection)
    my_menu->CheckMenuItem(ID_OPTIONS_DISABLESLOPECORRECTION, MF_BYCOMMAND | MF_CHECKED);
  if (theApp.m_Options.bShowCells) my_menu->CheckMenuItem(ID_OPTIONS_SHOWBUILDINGOUTLINE, MF_BYCOMMAND | MF_CHECKED);
  if (theApp.m_Options.useDefaultMouseCursor)
    my_menu->CheckMenuItem(ID_OPTIONS_USEDEFAULTMOUSECURSOR, MF_BYCOMMAND | MF_CHECKED);
  ...
  if (theApp.m_Options.bEasy) {
    ...
    for (i = edit_my_menu->GetMenuItemCount() - 1; i >= 11;
         i--)  // MW 07/17/2001: i>=9 changed to i>=10 so Basic dialog is always available
    {
...
void CFinalSunDlg::UnloadAll() {
  int iQuit = MessageBox(GetLanguageStringACP("MainDialogExitQuestion"),
                         GetLanguageStringACP("MainDialogExitQuestionCap"), MB_ICONQUESTION | MB_YESNO | MB_DEFBUTTON2);
  if (iQuit == IDNO)
    return;
  else {
    try {
...
void CFinalSunDlg::HideAllDialogs() {
  if (m_aitriggertypes) m_aitriggertypes.ShowWindow(SW_HIDE);
  if (m_aitriggertypesenable) m_aitriggertypesenable.ShowWindow(SW_HIDE);
  if (m_all) m_all.ShowWindow(SW_HIDE);
  if (m_basic) m_basic.ShowWindow(SW_HIDE);
  if (m_houses) m_houses.ShowWindow(SW_HIDE);
  if (m_lighting) m_lighting.ShowWindow(SW_HIDE);
  if (m_map) m_map.ShowWindow(SW_HIDE);
  if (m_Scripttypes) m_Scripttypes.ShowWindow(SW_HIDE);
  if (m_singleplayersettings) m_singleplayersettings.ShowWindow(SW_HIDE);
  if (m_specialflags) m_specialflags.ShowWindow(SW_HIDE);
  if (m_tags) m_tags.ShowWindow(SW_HIDE);
  if (m_taskforces) m_taskforces.ShowWindow(SW_HIDE);
  if (m_teamtypes) m_teamtypes.ShowWindow(SW_HIDE);
  if (m_triggers) m_triggers.ShowWindow(SW_HIDE);
}
...
void CFinalSunDlg::OnEnterIdle(UINT nWhy, CWnd* pWho) { CDialog::OnEnterIdle(nWhy, pWho); }
...
void CFinalSunDlg::OnSizing(UINT fwSide, LPRECT pRect) { CDialog::OnSizing(fwSide, pRect); }
...
void CFinalSunDlg::RecalcLayout() {
  ...
  rr.right = r.right;
  m_view.SetWindowPos(NULL, 0, h, r.right, r.bottom - h,
                      SWP_NOOWNERZORDER | SWP_NOZORDER | SWP_SHOWWINDOW | SWP_DRAWFRAME);
  m_bar.SetWindowPos(NULL, 0, 0, rr.right, rr.bottom,
                     SWP_NOOWNERZORDER | SWP_NOZORDER | SWP_SHOWWINDOW | SWP_DRAWFRAME);
...
// DEL 	{
// DEL 		if(!m_TerrainDlg.Create(NULL, "Terrain-Browser", WS_OVERLAPPEDWINDOW, CFrameWnd::rectDefault, NULL,
// NULL, 0)) DEL 		{ DEL 			MessageBox(GetLanguageStringACP("Err_CreateErr"), "Error"); DEL
// } DEL 	} DEL DEL 	if(m_TerrainDlg.m_hWnd!=NULL) DEL 	{ DEL m_TerrainDlg.ShowWindow(SW_SHOW); DEL
// m_TerrainDlg.ActivateFrame(SW_SHOW); DEL 		m_TerrainDlg.m_bar.Update(); DEL 	} DEL }
...
void CFinalSunDlg::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) { CDialog::OnKeyDown(nChar, nRepCnt, nFlags); }
...
void CFinalSunDlg::OnMaptoolsChangemapheight() {
  ...
  for (i = 0; i < max; i++) {
    ...
    // if(dwX==0 || dwY==0 || dwX+dwY<mapwidth+1 || dwX+dwY+1>mapwidth+mapheight*2 || (dwY+1>mapwidth &&
    // dwX<dwY-mapwidth+1) || (dwX+1>mapwidth && dwY+mapwidth<dwX+1)) if( dwX<2|| dwY<2 || dwX+dwY<mapwidth+2 ||
    // dwX+dwY+2>mapwidth+mapheight*2 || (dwY+2>mapwidth && dwX-2<dwY-mapwidth) || (dwX+2>mapwidth &&
    // dwY+mapwidth-2<dwX))
  ...
  CString res = InputBox(p, GetLanguageStringACP("StrChangeHeightCap"));
  ...
  for (i = 0; i < max; i++) {
    ...
    if (dwX < 1 || dwY < 1 || dwX + dwY < mapwidth + 1 || dwX + dwY > mapwidth + mapheight * 2 ||
      ...
      if (v + a < 0 || v + a > MAXHEIGHT) {
        MessageBox(GetLanguageStringACP("StrChangeHeightErr"), GetLanguageStringACP("StrChangeHeightErrCap"),
                   MB_ICONSTOP);
        return;
      }
    }
...
void CFinalSunDlg::OnHelpManual() {
  ...
  if (ShellExecuteW(0, NULL, (utf8ToUtf16(AppPath) + L"\\HelpManual.pdf").c_str(), NULL, NULL, SW_NORMAL) == 0) {
    MessageBox((CString) "Could not open HelpManual.pdf! Try opening " + (CString)AppPath +
               (CString) "\\HelpManual manually.");
  }
}

LONG __stdcall ExceptionHandler(_EXCEPTION_POINTERS* ExceptionInfo  // address of
                                                                    // exception info
) {
  CString s;
  CString s2;
  ...
  CString s3;
  s3 = "INTERNAL APPLICATION ERROR\n\nApplication will now try to free memory, save the current map as "
       "\"fcrash_backup.map\" in the FinalAlert 2 directory and quit.\n\n\n";

  s3 +=
      "Important: If this error has occured while loading graphics, it can very often be fixed by using another system "
      "color resolution (16, 24 or 32 bit).";
...
void CFinalSunDlg::OnMaptoolsAutolevel() {
  ...
  Sound(SOUND_POSITIVE);
  MessageBox(
      "This tool will try to automatically raise the terrain using the cliffs.\nIt may take some seconds to execute, "
      "as there are masses of data to handle.\nAfter this, you should check your map if everything looks fine. If not, "
      "you should use the different height tools, especially flatten ground, to fix any errors. You can use Edit->Undo "
      "to undo anything that has been done by using this function.",
      "Auto Leveler");
...
void CFinalSunDlg::OnUpdateEditHouses(CCmdUI* pCmdUI) { CheckAvail(pCmdUI); }

void CFinalSunDlg::OnUpdateEditIniediting(CCmdUI* pCmdUI) { CheckAvail(pCmdUI); }
...
void CFinalSunDlg::OnUpdateOptionsSounds(CCmdUI* pCmdUI) { pCmdUI->SetCheck(theApp.m_Options.bNoSounds ? 0 : 1); }
...
void CFinalSunDlg::OnHelpShowlogs() {
  ...
  if (ShellExecuteW(0, NULL, f.c_str(), NULL, NULL, SW_NORMAL) == 0) {
    MessageBoxW(nullptr, (std::wstring(L"Could not open log! Try opening ") + f + L" manually").c_str(),
                L"Cannot open log", MB_ICONEXCLAMATION);
  }
}
...
BOOL CFinalSunDlg::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) {
  if (!CDialog::OnSetCursor(pWnd, nHitTest, message)) SetCursor(m_hArrowCursor);
  return TRUE;
}
...
void CFinalSunDlg::OnOptionsUsedefaultmousecursor() {
  ...
  Options.sections["UserInterface"].values["UseDefaultMouseCursor"] =
      theApp.m_Options.useDefaultMouseCursor ? "1" : "0";
  Options.SaveFile(u8AppDataPath + "\\FinalAlert.ini");
}
