...
void CTileSetBrowserView::OnInitialUpdate() { CScrollView::OnInitialUpdate(); }
...
void CTileSetBrowserView::OnDraw(CDC* pDC) {
  ...
  if (((CFinalSunDlg*)theApp.m_pMainWnd)->m_view.m_isoview->b_IsLoading ||
      ((CFinalSunDlg*)theApp.m_pMainWnd)->m_view.m_isoview->lpds == NULL ||
      ((CFinalSunDlg*)theApp.m_pMainWnd)->m_view.m_isoview->lpds->IsLost() != DD_OK)
    return;

  RECT r;
  GetClientRect(&r);

  if (tiledata == NULL || (*tiledata) == NULL) return;

  if (m_tilecount == 0) return;

  if (m_tile_width == 0) return;  // just to make sure I never divide through 0 here...
  ...
  if (m_CurrentMode == 1) {
    ...
    for (i = 0; i < m_tilecount; i++) {
      ...
      if (cur_y + curheight + (m_tile_height - curheight) / 2 >= this->GetScrollPos(SB_VERT) &&
        ...
        BitBlt(pDC->GetSafeHdc(), cur_x + (m_tile_width - curwidth) / 2, cur_y + (m_tile_height - curheight) / 2,
    ...
    for (i = 0; i < max_ovrl_img; i++) {
      ...
      if (p != NULL && p->pic != NULL) {
        ...
        if (AD.mode == ACTIONMODE_PLACE && AD.data2 == m_currentOverlay && AD.data3 == i && AD.data == 33 &&
            AD.type == 6) {
          CPen p;
          CBrush b;
          p.CreatePen(PS_SOLID, 1, RGB(255, 0, 0));
    ...
    for (i = 0; i < max_ovrl_img; i++) {
      ...
      if (p != NULL && p->pic != NULL) {
        ...
        StretchDIBits(pDC->GetSafeHdc(), cur_x + (m_tile_width - curwidth) / 2, cur_y + (m_tile_height - curheight) / 2,
                      curwidth, curheight, 0, 0, curwidth, curheight, colors, &biinfo, DIB_RGB_COLORS, SRCCOPY);
...
#ifdef _DEBUG
void CTileSetBrowserView::AssertValid() const { CScrollView::AssertValid(); }

void CTileSetBrowserView::Dump(CDumpContext& dc) const { CScrollView::Dump(dc); }
#endif  //_DEBUG
...
DWORD CTileSetBrowserView::GetTileID(DWORD dwTileSet, DWORD dwType) {
  ...
  for (i = 0; i < 10000; i++) {
    ...
    for (e = 0; e < atoi(tiles->sections[sec].values["TilesInSet"]); e++) {
      if (i == dwTileSet && e == dwType) return tilecount;
      tilecount++;
    }
  }
...
void CTileSetBrowserView::SetTileSet(DWORD dwTileSet, BOOL bOnlyRedraw) {
  ...
  int e;
  for (e = 0; e < 4 - strlen(currentTileSet); e++) tset += "0";
  ...
  for (i = 0; i < max; i++) {
    ...
    if (dwID < *tiledata_count) {
      ...
      if (GetAddedHeight(dwID) + (*tiledata)[dwID].rect.bottom - (*tiledata)[dwID].rect.top > m_tile_height)
        m_tile_height = GetAddedHeight(dwID) + (*tiledata)[dwID].rect.bottom - (*tiledata)[dwID].rect.top;
    }
  }
...
__forceinline void BlitTerrainTSB(void* dst, int x, int y, int dleft, int dtop, int dpitch, int dright, int dbottom,
                                  SUBTILE& st)  // BYTE* src, int swidth, int sheight)
{
  BYTE* src = st.pic;
  ...
  for (i = 0; i < (*tiledata)[dwID].cx; i++) {
    for (e = 0; e < (*tiledata)[dwID].cy; e++) {
      ...
      if ((*tiledata)[dwID].tiles[p].pic) {
...
#ifndef NOSURFACES
        if (lpdds->Blt(&dest, (*tiledata)[dwID].tiles[p].pic, NULL, DDBLT_KEYSRC, &fx) != DD_OK) TRACE("Blit failed\n");
#else
        DDSURFACEDESC2 ddsd;
        ...
        BlitTerrainTSB(ddsd.lpSurface, drawx, drawy, 0, 0, ddsd.lPitch, ddsd.dwWidth, ddsd.dwHeight,
                       (*tiledata)[dwID].tiles[p]);
        lpdds->Unlock(NULL);
#endif
      }
...
void CTileSetBrowserView::DrawIt() { CPaintDC myDC(this); }
...
void CTileSetBrowserView::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  if (m_CurrentMode == 1) {
    ...
    for (i = 0; i < m_tilecount; i++) {
      ...
      if (point.x > cur_x + posaddedx && point.y > cur_y + posaddedy && point.x < cur_x + tile_width - posaddedx &&
