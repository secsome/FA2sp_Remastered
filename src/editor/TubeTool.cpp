...
std::unique_ptr<CTube> findTubeEndAt(const MapCoords& mapCoords3d, const ProjectedCoords& projCoords,
                                     const CMapData& map) {
  const auto& tubes = map.GetTubes();
  auto it = std::find_if(tubes.begin(), tubes.end(),
                         [mapCoords3d](const auto& tube) { return tube->getEndCoords() == mapCoords3d; });
  if (it != tubes.end()) return std::make_unique<CTube>(**it);
  ...
  it = std::find_if(tubes.begin(), tubes.end(),
                    [backMapCoords](const auto& tube) { return tube->getEndCoords() == backMapCoords; });
  if (it != tubes.end()) return std::make_unique<CTube>(**it);
  return nullptr;
}
...
std::vector<std::uint16_t> findTubesAt(const ProjectedCoords& projCoords, const CMapData& map) {
  ...
  for (auto& t : map.GetTubes()) {
    ...
    if (t->touches(coords)) foundTubes.push_back(t->getId());
  }
  return foundTubes;
}

AddTubeTool::AddTubeTool(CMapData& map, CIsoView& view, bool bidirectional)
    : MapTool(map, view), m_bidirectional(bidirectional) {}
...
bool AddTubeTool::onRButtonUp(const ProjectedCoords& projCoords, const MapCoords& mapCoords3d,
  ...
  m_modified_tubes.clear();
  getView().RedrawWindow(NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW);
  return ret;  // if we were adding or modifying a tube, a right click should just cancel this current tube, not the
               // whole tool
}

void AddTubeTool::onLButtonDblClick(const ProjectedCoords& projCoords, const MapCoords& mapCoords3d,
                                    MapToolMouseFlags flags) {}

void AddTubeTool::onLButtonUp(const ProjectedCoords& projCoords, const MapCoords& mapCoords3d,
                              MapToolMouseFlags flags) {
  if (!m_tube) {
    ...
    if (tube) {
      ...
      if (m_bidirectional) {
        ...
        auto reverseIt = std::find_if(tubes.begin(), tubes.end(),
                                      [&reversed](const auto& tube) { return tube->isEqual(reversed, true); });
...
std::unique_ptr<CTube> AddTubeTool::createNewTube(const MapCoords& mapCoords3d) const {
  return std::make_unique<CTube>(mapCoords3d.x, mapCoords3d.y, ETubeDirection::Undefined, mapCoords3d.x, mapCoords3d.y,
                                 std::vector<ETubeDirection>());
}

std::unique_ptr<CTube> AddTubeTool::getTubeToModify(const MapCoords& mapCoords3d, const ProjectedCoords& projCoords,
...
void AddTubeTool::onMouseMove(const ProjectedCoords& projCoords, const MapCoords& mapCoords3d,
                              MapToolMouseFlags flags) {
  if (m_tube) {
...
void RemoveTubeTool::onMouseMove(const ProjectedCoords& projCoords, const MapCoords& mapCoords3d,
                                 MapToolMouseFlags flags) {
  auto& m = getMap();
  ...
  if ((flags & MapToolMouseFlags::LBUTTON) == MapToolMouseFlags::LBUTTON) {
    ...
    std::transform(tubes.begin(), tubes.end(), std::back_inserter(m_hover_tubes),
                   [this](std::uint16_t tubeId) { return std::make_unique<CTube>(*getMap().GetTube(tubeId)); });
  }
  getView().RedrawWindow(NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW);
  m_hover_tubes.clear();
}
...
ModifyTubeTool::ModifyTubeTool(CMapData& map, CIsoView& view, bool bidirectional)
    : AddTubeTool(map, view, bidirectional) {}

std::unique_ptr<CTube> ModifyTubeTool::getTubeToModify(const MapCoords& mapCoords3d, const ProjectedCoords& projCoords,
                                                       MapToolMouseFlags flags) const {
  return findTubeEndAt(mapCoords3d, projCoords, getMap());
}

std::unique_ptr<CTube> ModifyTubeTool::createNewTube(const MapCoords& mapCoords3d) const { return nullptr; }
