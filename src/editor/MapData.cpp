...
SNAPSHOTDATA::SNAPSHOTDATA() { memset(this, 0, sizeof(SNAPSHOTDATA)); }
...
FIELDDATA::FIELDDATA() {
  ...
  unit = -1;
  int i;
  for (i = 0; i < SUBPOS_COUNT; i++) infantry[i] = -1;
  aircraft = -1;
  structure = -1;
  structuretype = -1;
  terrain = -1;
  ...
  if (tiledata_count && (*tiledata_count))  // only if initialized
  {
    ...
    if ((*tiledata)[ground].bReplacementCount)  //&& atoi((*tiles).sections["General"].values["BridgeSet"]) !=
                                                //(*tiledata)[ground].wTileSet)
    {
...
void CMapData::CalcMapRect() {
  ...
  if (cucomma == NULL) cucomma = (char*)(msize + strlen(msize));
  memcpy_s(custr, custr_size, &msize[cupos], (cucomma - msize) - cupos + 1);
  custr[((cucomma - msize)) - cupos] = 0;
  cupos = cucomma - msize + 1;
...
CString CMapData::GetHouseID(WORD wHouse, BOOL bCountry) {
  if (wHouse >= GetHousesCount()) return CString("");
...
void CMapData::GetAITriggerType(DWORD dwAITriggerType, AITRIGGERTYPE* pAITrg) {
  ...
  if (dwAITriggerType >= GetAITriggerTypeCount()) return;
...
WORD CMapData::GetHouseIndex(LPCTSTR lpHouse) {
  if (m_mapfile.sections.find(HOUSES) != m_mapfile.sections.end())
    if (m_mapfile.sections[HOUSES].values.size() > 0) return m_mapfile.sections[HOUSES].FindValue(lpHouse);
...
void CMapData::UpdateIniFile(DWORD dwFlags) {
  BOOL bSave = TRUE;
  if (dwFlags == MAPDATA_UPDATE_FROM_INI) bSave = FALSE;
...
void CMapData::Pack(BOOL bCreatePreview, BOOL bCompression) {
  ...
  if (m_mapfile.sections["Digest"].values.size() == 0) {
    ...
    for (i = 0; i < 10; i++) vals[i] = rand() * 65536 / RAND_MAX;
    ...
    while (TRUE) {
      ...
      if (strlen(str) > 0) m_mapfile.sections["Digest"].values[cLine] = str;
      if (cpysize < 70) break;
      pos += 70;
    }
  ...
  errstream << "Values allocated." << endl;
  errstream.flush();
  ...
  while (TRUE) {
    ...
    if (pos + cpysize > strlen((char*)base64)) cpysize = strlen((char*)base64) - pos;
    memcpy(str, &base64[pos], cpysize);
    if (strlen(str) > 0) m_mapfile.sections["OverlayPack"].values[cLine] = str;
    if (cpysize < 70) break;
    pos += 70;
  }
  ...
  while (TRUE) {
    ...
    if (pos + cpysize > strlen((char*)base64)) cpysize = strlen((char*)base64) - pos;
    memcpy(str, &base64[pos], cpysize);
    if (strlen(str) > 0) m_mapfile.sections["OverlayDataPack"].values[cLine] = str;
    if (cpysize < 70) break;
    pos += 70;
  }
  ...
  while (TRUE) {
    ...
    memcpy(str, &base64[pos], cpysize);
    if (cpysize) m_mapfile.sections["IsoMapPack5"].values[cLine] = str;
  ...
  if (bCreatePreview) {
    ...
    hexpacked = NULL;
    hexpackedLen =
        FSunPackLib::EncodeIsoMapPack5(lpRAW, biinfo.bmiHeader.biWidth * biinfo.bmiHeader.biHeight * 3, &hexpacked);
    ...
    while (TRUE) {
      ...
      if (pos + cpysize > strlen((char*)base64)) cpysize = strlen((char*)base64) - pos;
      memcpy(str, &base64[pos], cpysize);
      if (strlen(str) > 0) m_mapfile.sections["PreviewPack"].values[cLine] = str;
      if (cpysize < 70) break;
      pos += 70;
    }
...
void CMapData::SetOverlayAt(DWORD dwPos, BYTE bValue) {
  ...
  for (i = -1; i < 2; i++)
    for (e = -1; e < 2; e++)
      if (i + x > 0 && i + x < m_IsoSize && y + e >= 0 && y + e < m_IsoSize) SmoothTiberium(dwPos + i + e * m_IsoSize);
...
BYTE CMapData::GetOverlayDataAt(DWORD dwPos) { return fielddata[dwPos].overlaydata; }
...
void CMapData::UpdateInfantry(BOOL bSave) {
  ...
  if (bSave == FALSE) {
    ...
    for (i = 0; i < GetIsoSize() * GetIsoSize(); i++) {
      int e;
      for (e = 0; e < SUBPOS_COUNT; e++) fielddata[i].infantry[e] = -1;
    }
    ...
    if (m_mapfile.sections.find("Infantry") != m_mapfile.sections.end()) {
      ...
      for (i = 0; i < sec.values.size(); i++) {
        ...
        if (spp < SUBPOS_COUNT)
          if (pos < fielddata_size) fielddata[pos].infantry[spp] = iv.size() - 1;
    ...
    for (i = 0; i < iv.size(); i++) {
      ...
      if (!id.deleted) {
        ...
        CString value;
        value = infantry.house + "," + infantry.type + "," + infantry.strength + "," + infantry.y + "," + infantry.x +
                "," + infantry.pos + "," + infantry.action + "," + infantry.direction + "," + infantry.tag + "," +
                infantry.flag1 + "," + infantry.flag2 + "," + infantry.flag3 + "," + infantry.flag4 + "," +
                infantry.flag5;
...
void CMapData::UpdateTerrain(BOOL bSave, int num) {
  ...
  if (bSave == FALSE) {
    if (m_mapfile.sections.find("Terrain") == m_mapfile.sections.end() ||
        m_mapfile.sections["Terrain"].values.size() <= 0)
      return;
  ...
  if (bSave == FALSE) {
    ...
    if (m_mapfile.sections.find("Waypoints") != m_mapfile.sections.end()) {
      ...
      for (i = 0; i < sec.values.size(); i++) {
        ...
        int k, l;
        for (k = -1; k < 2; k++)
          for (l = -1; l < 2; l++) Mini_UpdatePos(x + k, y + l, IsMultiplayer());
      }
    }
  }
}
...
void CMapData::GetStructureData(DWORD dwIndex, STRUCTURE* lpStructure) const {
  ...
  if (!section || dwIndex >= section->values.size()) return;
...
BOOL CMapData::AddInfantry(INFANTRY* lpInfantry, LPCTSTR lpType, LPCTSTR lpHouse, DWORD dwPos, int suggestedIndex) {
  ...
  if (lpInfantry != NULL) {
    ...
    if (GetInfantryAt(dwPos, atoi(infantry.pos)) >= 0) infantry.pos = "-1";
  } else {
    char cx[10], cy[10];
...
BOOL CMapData::AddStructure(STRUCTURE* lpStructure, LPCTSTR lpType, LPCTSTR lpHouse, DWORD dwPos, CString suggestedID) {
  ...
  CString value;
  value = structure.house + "," + structure.type + "," + structure.strength + "," + structure.y + "," + structure.x +
          "," + structure.direction + "," + structure.tag + "," + structure.flag1 + "," + structure.flag2 + "," +
          structure.energy + "," + structure.upgradecount + "," + structure.spotlight + "," + structure.upgrade1 + "," +
          structure.upgrade2 + "," + structure.upgrade3 + "," + structure.flag3 + "," + structure.flag4;
...
void CMapData::InitializeUnitTypes() {
  ...
  int i;
  m_overlayCredits[OverlayCredits_Riparius] =
      atoi(m_mapfile.GetValueByName("Riparius", "Value", rules.sections["Riparius"].AccessValueByName("Value")));
  m_overlayCredits[OverlayCredits_Cruentus] =
      atoi(m_mapfile.GetValueByName("Cruentus", "Value", rules.sections["Cruentus"].AccessValueByName("Value")));
  m_overlayCredits[OverlayCredits_Vinifera] =
      atoi(m_mapfile.GetValueByName("Vinifera", "Value", rules.sections["Vinifera"].AccessValueByName("Value")));
  m_overlayCredits[OverlayCredits_Aboreus] =
      atoi(m_mapfile.GetValueByName("Aboreus", "Value", rules.sections["Aboreus"].AccessValueByName("Value")));
...
BOOL CMapData::AddAircraft(AIRCRAFT* lpAircraft, LPCTSTR lpType, LPCTSTR lpHouse, DWORD dwPos, CString suggestedID) {
  ...
  CString value;
  value = aircraft.house + "," + aircraft.type + "," + aircraft.strength + "," + aircraft.y + "," + aircraft.x + "," +
          aircraft.direction + "," + aircraft.action + "," + aircraft.tag + "," + aircraft.flag1 + "," +
          aircraft.flag2 + "," + aircraft.flag3 + "," + aircraft.flag4;
...
BOOL CMapData::AddUnit(UNIT* lpUnit, LPCTSTR lpType, LPCTSTR lpHouse, DWORD dwPos, CString suggestedID) {
  ...
  CString value;
  value = unit.house + "," + unit.type + "," + unit.strength + "," + unit.y + "," + unit.x + "," + unit.direction +
          "," + unit.action + "," + unit.tag + "," + unit.flag1 + "," + unit.flag2 + "," + unit.flag3 + "," +
          unit.flag4 + "," + unit.flag5 + "," + unit.flag6;
...
BOOL CMapData::IsGroundObjectAt(DWORD dwPos) const {
  ...
  if (m_id < 0) m_id = GetUnitAt(dwPos);
  if (m_id < 0) m_id = GetAirAt(dwPos);
  if (m_id < 0) m_id = GetStructureAt(dwPos);
  if (m_id < 0) m_id = GetTerrainAt(dwPos);
...
BOOL CMapData::IsRulesSection(LPCTSTR lpSection) {
  ...
  if (m_mapfile.sections.find("InfantryTypes") != m_mapfile.sections.end())
    if (m_mapfile.sections["InfantryTypes"].FindValue(lpSection) >= 0) return TRUE;

  if (m_mapfile.sections.find("VehicleTypes") != m_mapfile.sections.end())
    if (m_mapfile.sections["VehicleTypes"].FindValue(lpSection) >= 0) return TRUE;

  if (m_mapfile.sections.find("AircraftTypes") != m_mapfile.sections.end())
    if (m_mapfile.sections["AircraftTypes"].FindValue(lpSection) >= 0) return TRUE;

  if (m_mapfile.sections.find("BuildingTypes") != m_mapfile.sections.end())
    if (m_mapfile.sections["BuildingTypes"].FindValue(lpSection) >= 0) return TRUE;

  if (m_mapfile.sections.find("TerrainTypes") != m_mapfile.sections.end())
    if (m_mapfile.sections["TerrainTypes"].FindValue(lpSection) >= 0) return TRUE;

  if ((CString) "IsoMapPack5" != lpSection && (CString) "OverlayPack" != lpSection &&
      (CString) "OverlayDataPack" != lpSection && m_mapfile.sections.find(lpSection) != m_mapfile.sections.end()) {
    CIniFileSection& sec = m_mapfile.sections[lpSection];
    if (sec.FindName("ROF") > -1 && sec.FindName("Range") > -1 && sec.FindName("Damage") > -1 &&
        sec.FindName("Warhead") > -1)
      return TRUE;  // a weapon

    if (sec.FindName("Spread") > -1 && sec.FindName("Range") > -1 && sec.FindName("Damage") > -1 &&
        sec.FindName("Warhead") > -1)
    ...
    for (i = 0; i < m_mapfile.sections.size(); i++) {
      CString name = *m_mapfile.GetSectionName(i);
      if ((CString) "IsoMapPack5" != name && (CString) "OverlayPack" != name && (CString) "OverlayDataPack" != name &&
          (m_mapfile.GetSection(i)->FindName("Projectile") > -1 || m_mapfile.GetSection(i)->FindName("Warhead") > -1)) {
...
void CMapData::UpdateMapFieldData(BOOL bSave) {
  if (bSave == FALSE) {
    ...
    /*for(i=0;i<GetIsoSize()*GetIsoSize();i++)
    {
            ...
            if( dwX<1|| dwY<1 || dwX+dwY<mapwidth+k || dwX+dwY>mapwidth+mapheight*2 || (dwY+k>mapwidth &&
    dwX-k<dwY-mapwidth) || (dwX+k>mapwidth && dwY+mapwidth-k<dwX))
            {
    ...
    for (i = 0; i < dwIsoMapSize; i++) {
      ...
      if (pos < (GetIsoSize() + 1) * (GetIsoSize() + 1)) {
        ...
        if ((*tiledata)[ground].bReplacementCount &&
            atoi((*tiles).sections["General"].values["BridgeSet"]) != (*tiledata)[ground].wTileSet) {
          replacement = rand() * (1 + (*tiledata)[ground].bReplacementCount) / RAND_MAX;
        }
    ...
    for (dwX = 0; dwX <= m_IsoSize; dwX++) {
      for (dwY = 0; dwY <= m_IsoSize; dwY++) {
        ...
        // if( dwX<2|| dwY<2 || dwX+dwY<mapwidth+2 || dwX+dwY+2>mapwidth+mapheight*2 || (dwY+2>mapwidth &&
        // dwX-2<dwY-mapwidth) || (dwX+2>mapwidth && dwY+mapwidth-2<dwX)) continue;
    ...
    /*
       ...
       (y+1>mapwidth && x-1<y-mapwidth) || (x+1>mapwidth && y+mapwidth-1<x)) continue;
...
MAPFIELDDATA* CMapData::GetMappackPointer(DWORD dwPos) {
  ...
  for (i = 0; i < dwIsoMapSize; i++) {
    ...
    if (cur->wX == y && cur->wY == x) return cur;
  }
...
void CMapData::CreateMap(DWORD dwWidth, DWORD dwHeight, LPCTSTR lpTerrainType, DWORD dwGroundHeight) {
  ...
  if (theApp.m_loading) {
    ...
    if (theApp.m_pMainWnd) ((CFinalSunDlg*)theApp.m_pMainWnd)->m_view.m_isoview->UpdateOverlayPictures();
    ...
    if (m_mapfile.sections["Map"].values["Theater"] == THEATER0) {
      ...
      tiles = &tiles_t;
      theApp.m_loading->FreeTileSet();
      if (dlg) theApp.m_loading->InitTMPs(&dlg->m_Progress);
      theApp.m_loading->cur_theat = 'T';
...
int CMapData::GetNecessarySlope(DWORD dwPos) { return 0; }
...
void CMapData::SetReserved(DWORD dwPos, BYTE val) { fielddata[dwPos].bReserved = val; }

void CMapData::HideField(DWORD dwPos, BOOL bHide) { fielddata[dwPos].bHide = bHide; }

void CMapData::UpdateTubes(BOOL bSave) {
  if (!bSave) {
    m_tubes.clear();
    if (m_mapfile.sections.find("Tubes") == m_mapfile.sections.end()) return;
    ...
    for (auto& [sTubeId, value] : tubeSection) {
      ...
      if (tubeId < 0 || tubeId >= std::numeric_limits<std::uint16_t>::max()) continue;
...
CTube* CMapData::GetTube(std::uint16_t wID) {
  ...
  if (it == m_tubes.end()) return nullptr;
...
void CMapData::DeleteTube(std::uint16_t wID) {
  ...
  for (i = 0;i < m_tubes.size();i++)
  {
          ...
          if ((curi.getEndX() == i1.getStartX() && curi.getEndY() == i1.getStartY()) || (curi.getStartX() ==
  i1.getStartX() && curi.getStartY() == i1.getStartY()))
          {
...
BOOL CMapData::CheckMapPackData() {
  ...
  for (i = 0; i < fielddata_size; i++) {
    ...
    if (gr != 0xFFFF && gr >= (*tiledata_count)) return FALSE;
    if (gr == 0xFFFF) gr = 0;
...
static const CString tile_to_lat[tile_to_lat_count][3] = {
    {"SandTile", "ClearToSandLat", "ClearTile"},       {"GreenTile", "ClearToGreenLat", "ClearTile"},
    {"RoughTile", "ClearToRoughLat", "ClearTile"},     {"PaveTile", "ClearToPaveLat", "ClearTile"},
    {"BlueMoldTile", "ClearToBlueMoldLat", ""},  //"ClearTile"}, // TODO: fix
    {"CrystalTile", "ClearToCrystalLat", "ClearTile"}, {"SwampTile", "WaterToSwampLat", "WaterSet"}};

bool CMapData::hasLat(WORD wGround) const {
  if (wGround >= *tiledata_count) return false;
  ...
  for (int i = 0; i < tile_to_lat_count; ++i) {
    ...
    if (lat && (set == tile || set == lat || set == target_tile)) return true;
  }
...
void CMapData::SmoothAllAt(DWORD dwPos) {
  ...
  for (int i = 0; i < tile_to_lat_count; ++i) {
    ...
    const int lat = atoi(sec.GetValueByName(tile_to_lat[i][1], empty));
    const int target_tile = atoi(sec.GetValueByName(tile_to_lat[i][2], empty));
    if (strlen(tile_to_lat[i][2]) && lat && (set == tile || set == lat)) SmoothAt(dwPos, tile, lat, target_tile);
  }
}
...
void CMapData::CreateShore(int left, int top, int right, int bottom, BOOL bRemoveUseless) {
  ...
  for (xx = left - 2; xx < right + 2; xx++) {
    for (yy = top - 2; yy < bottom + 2; yy++) {
      if (xx < 1 || yy < 1 || xx + yy < mapwidth + 1 || xx + yy > mapwidth + mapheight * 2 ||
          (yy + 1 > mapwidth && xx - 1 < yy - mapwidth) || (xx + 1 > mapwidth && yy + mapwidth - 1 < xx))
        continue;
  ...
  if (bRemoveUseless) {
    for (xx = left; xx < right; xx++) {
      for (yy = top; yy < bottom; yy++) {
        ...
        if (td.tiles[fd->bSubTile].bHackedTerrainType != TERRAINTYPE_WATER &&
            td.tiles[fd->bSubTile].bHackedTerrainType != TERRAINTYPE_GROUND)
          continue;
        ...
        if ((ts[1][0] != ts[1][1] && ts[1][2] != ts[1][1]) || (ts[0][1] != ts[1][1] && ts[2][1] != ts[1][1])) {
          if (ts[1][1] == TERRAINTYPE_WATER) {
            SetTileAt(dwPos, 0, 0);
            // replaced[dwPos]=TRUE;
          } else if (ts[1][1] == TERRAINTYPE_GROUND) {
            if ((ts[1][0] == TERRAINTYPE_WATER && ts[1][2] == TERRAINTYPE_WATER) ||
                (ts[0][1] == TERRAINTYPE_WATER && ts[2][1] == TERRAINTYPE_WATER)) {
  ...
  for (xx = left; xx < right; xx++) {
    for (yy = top; yy < bottom; yy++) {
      ...
      if (xx >= left && xx < right && yy >= top && yy < bottom) {
        if (softsets.find((*tiledata)[ground].wTileSet) ==
            softsets.end() /*(*tiledata)[ground].wTileSet==cliffset || (*tiledata)[ground].wTileSet==watercliffset*/) {
          noChange[pos] = TRUE;
          continue;
        }
  ...
  for (i = tStart; i <= tEnd; i++) {
    ...
    if (td.wTileSet == shoreset) {
      ...
      if (pos != 4 && pos != 5 && pos != 12 && pos != 13 && pos != 20 && pos != 21 && pos != 28 && pos != 29 &&
          pos != 6 && pos != 7 && pos != 14 && pos != 15 && pos != 22 && pos != 23 && pos != 30 && pos != 31 &&
          (pos < 32 || pos > 39))
        continue;
      ...
      for (x = 0; x < td.cx; x++) {
        for (y = 0; y < td.cy; y++) {
          if (td.tiles[p].bHackedTerrainType == TERRAINTYPE_WATER) water_count++;
          p++;
        }
      }
        ...
        for (y = top; y < bottom; y++) {
          ...
          for (xx = x; xx < x + max_x; xx++) {
            for (yy = y; yy < y + max_y; yy++) {
              ...
              if (tile[pos_data] <= tEnd && tile[pos_data] >= tEnd - 2) bFits = FALSE;
          ...
          if (bFits)  // ok, place shore (later we need to do random choose of the different tiles here
          {
            ...
            for (k = 0; k < *tiledata_count; k++) {
              ...
              for (xx = 0; xx < t.cx; xx++) {
                for (yy = 0; yy < t.cy; yy++) {
                  if (t.tiles[p].bHackedTerrainType != t_orig.tiles[p].bHackedTerrainType) bSame = FALSE;
                  p++;
                  if (!bSame) break;
                }
                if (!bSame) break;
  ...
  for (x = left; x < right; x++) {
    for (y = top; y < bottom; y++) {
      ...
      if (noChange[pos]) continue;
      if (terrain[pos] == TERRAINTYPE_GROUND && tsets[pos] != shoreset && tsets[pos] != cliffset &&
          tsets[pos] != watercliffset) {
        int i, e;
...
CString CMapData::GetTheater() { return m_mapfile.sections["Map"].values["Theater"]; }
...
void CMapData::Copy(int left, int top, int right, int bottom) {
  ...
  HGLOBAL hGlob = GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE,
                              sizeof(CLIPBOARD_DATA) + cd.iWidth * cd.iHeight * sizeof(CLIPBOARD_MAPCOPY_ENTRY));
  ...
  for (i = 0; i < cd.iWidth; i++) {
    for (e = 0; e < cd.iHeight; e++) {
      ...
      if (fielddata[pos_r].bHeight - (*tiledata)[ground].tiles[fielddata[pos_r].bSubTile].bZHeight < lowestheight)
        lowestheight = fielddata[pos_r].bHeight - (*tiledata)[ground].tiles[fielddata[pos_r].bSubTile].bZHeight;
    }
  }
...
void CMapData::Paste(int x, int y, int z_mod) {
  ...
  if (ground == 0xFFFF) ground = 0;
  int startheight =
      lowestheight + z_mod;  //-(*tiledata)[ground].tiles[GetFielddataAt(x+y*m_IsoSize)->bSubTile].bZHeight;
...
int CMapData::GetMoneyOnMap() const { return m_money; }
...
void CMapData::SmoothTiberium(DWORD dwPos) {
  ...
  if (!(ovrl >= RIPARIUS_BEGIN && ovrl <= RIPARIUS_END) && !(ovrl >= CRUENTUS_BEGIN && ovrl <= CRUENTUS_END) &&
      !(ovrl >= VINIFERA_BEGIN && ovrl <= VINIFERA_END) && !(ovrl >= ABOREUS_BEGIN && ovrl <= ABOREUS_END))
    return;
...
void CMapData::ResizeMap(int iLeft, int iTop, DWORD dwNewWidth, DWORD dwNewHeight) {
  ...
  for (i = inf_count - 1; i >= 0; i--) DeleteInfantry(i);
  ...
  for (i = air_count - 1; i >= 0; i--) DeleteAircraft(i);
  ...
  for (i = str_count - 1; i >= 0; i--) DeleteStructure(i);
  ...
  for (i = unit_count - 1; i >= 0; i--) DeleteUnit(i);

  for (i = 0; i < terrain_count; i++) {
    terrain[i] = m_terrain[i];
  }
  for (i = 0; i < terrain_count; i++) DeleteTerrain(i);
  ...
  int allcount = GetInfantryCount() + GetAircraftCount() + GetUnitCount() + GetStructureCount() + GetTerrainCount() +
                 GetWaypointCount() + GetCelltagCount();
  int curcount = 0;
...
BOOL CMapData::IsMapSection(LPCSTR lpSectionName) {
  ...
  if (str == "IsoMapPack5" || str == "OverlayPack" || str == "OverlayDataPack" || str == "Preview" ||
      str == "PreviewPack" || str == "Map" || str == "Structures" || str == "Terrain" || str == "Units" ||
      str == "Aircraft" || str == "Infantry" || str == "Variables")
    return TRUE;
...
BOOL CMapData::IsYRMap() {
  {
    if (Map->GetTheater() == THEATER3 || Map->GetTheater() == THEATER4 || Map->GetTheater() == THEATER5) return TRUE;
    ...
    if (tiledata == &u_tiledata) {
      ...
      max = atoi(g_data.sections["RA2TileMax"].values["Snow"]);
    else if (tiledata == &t_tiledata)
      max = atoi(g_data.sections["RA2TileMax"].values["Temperat"]);
    ...
    for (i = 0; i < fielddata_size; i++) {
      ...
      if (fielddata[i].overlay >= yroverlay && fielddata[i].overlay != 0xFF) return TRUE;
    }
    ...
    for (i = 0; i < count; i++) {
      ...
      for (e = 0; e < eventcount; e++) {
        ...
        if (g_data.sections["EventsRA2"].values.find(type) != g_data.sections["EventsRA2"].values.end()) {
          if (isTrue(GetParam(g_data.sections["EventsRA2"].values[type], 9))) return TRUE;
        }
      }
      ...
      for (e = 0; e < actioncount; e++) {
        ...
        if (g_data.sections["ActionsRA2"].values.find(type) != g_data.sections["ActionsRA2"].values.end()) {
          if (isTrue(GetParam(g_data.sections["ActionsRA2"].values[type], 14))) return TRUE;
        }
      }
    }
  }
...
void CMapData::UpdateSmudges(BOOL bSave, int num) {
  ...
  if (bSave == FALSE) {
    if (m_mapfile.sections.find("Smudge") == m_mapfile.sections.end() ||
        m_mapfile.sections["Smudge"].values.size() <= 0)
      return;
...
BOOL CMapData::GetInfantryINIData(int index, CString* lpINI) {
  ...
  CString value;
  value = infantry.house + "," + infantry.type + "," + infantry.strength + "," + infantry.y + "," + infantry.x + "," +
          infantry.pos + "," + infantry.action + "," + infantry.direction + "," + infantry.tag + "," + infantry.flag1 +
          "," + infantry.flag2 + "," + infantry.flag3 + "," + infantry.flag4 + "," + infantry.flag5;
...
void CMapData::RedrawMinimap() {
  ...
  int i, e;
  for (i = 0; i < m_IsoSize; i++)
    for (e = 0; e < m_IsoSize; e++) Mini_UpdatePos(i, e, mp);
}
