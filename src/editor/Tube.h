...
inline ETubeDirection ToTubeDirection(const std::underlying_type_t<ETubeDirection> dir) {
  if (dir > 7 || dir < -1) return ETubeDirection::Undefined;
  return static_cast<ETubeDirection>(dir);
}

inline auto to_int(const ETubeDirection dir) { return static_cast<std::underlying_type_t<ETubeDirection>>(dir); }
...
class CTube {
 ...
 public:
  ...
  CTube(std::uint16_t tubeID, std::uint16_t startX, std::uint16_t startY, ETubeDirection direction, std::uint16_t endX,
        std::uint16_t endY, const std::vector<ETubeDirection>& tubeParts);
  CTube(std::uint16_t startX, std::uint16_t startY, ETubeDirection direction, std::uint16_t endX, std::uint16_t endY,
  ...
  static CTube autocreate(std::uint16_t startX, std::uint16_t startY, std::uint16_t endX, std::uint16_t endY,
                          int straightStartParts = 1);

  const std::vector<ETubeDirection>& GetTubeParts() const { return m_tubeParts; }

  void setId(std::uint16_t id) { m_tubeId = id; }

  bool isCounterpart(const CTube& other) const {
    return getStartCoords() == other.getEndCoords() && getEndCoords() == other.getStartCoords();
  }

  bool hasId() const { return m_tubeId != 0xFFFF; }
