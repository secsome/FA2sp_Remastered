...
CTerrainDlg::CTerrainDlg(CWnd* pParent /*=NULL*/) : CDialogBar() {
...
void CTerrainDlg::OnCancel() { DestroyWindow(); }
...
BOOL CTerrainDlg::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect,
...
void CTerrainDlg::Update() {
  ...
  if (tiles) {
    ...
    for (i = 0; i < 10000; i++) {
      ...
      if (atoi(tiles->sections[sec].values["TilesInSet"]) == 0) continue;
      ...
      if (g_data.sections[datsec].FindValue(tsetc) >= 0) bForced = TRUE;

      // force no
      datsec = (CString) "IgnoreSet" + Map->GetTheater();
      if (g_data.sections[datsec].FindValue(tsetc) >= 0) bForcedNot = TRUE;
    ...
    if (id.GetLength() > 0) {
      if (rules.sections.find(id) != rules.sections.end() && rules.sections[id].FindName("Name") >= 0) {
        ...
        for (p = 0; p < max_ovrl_img; p++)
          if (ovrlpics[i][p] != NULL && ovrlpics[i][p]->pic != NULL) bListIt = TRUE;
