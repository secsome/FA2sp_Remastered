...
char const* TMissionsHelp[TMISSION_COUNT] = {
    ...
    "Duh",
    "Unloads all loaded units.  The command parameter specifies which units should stay a part of the team, and which "
    "should be severed from the team.",
    ...
    "Record a team as having successfully accomplished its mission.  Used for AI trigger weighting.  Put this at the "
    "end of every AITrigger script.",
...
void CScriptTypes::OnEditchangeType() {
  ...
  switch (type) {
    ...
    case 6:
      ...
      while (m_Param.DeleteString(0) != CB_ERR);
      for (i = 1; i <= ini.sections[(LPCTSTR)Scripttype].values.size() - 1; i++) m_Param.AddString(itoa(i, tmp, 10));
      break;
    case 8:
  ...
  char types[50];
  itoa(type, types, 10);
  ini.sections[(LPCTSTR)Scripttype].values[action] =
      SetParam(ini.sections[(LPCTSTR)Scripttype].values[action], 0, (LPCTSTR)types);
}
...
void CScriptTypes::OnEditchangeParam() {
  ...
  itoa(m_Action.GetCurSel(), action, 10);
  ini.sections[(LPCTSTR)Scripttype].values[action] =
      SetParam(ini.sections[(LPCTSTR)Scripttype].values[action], 1, (LPCTSTR)param);
}
...
void CScriptTypes::OnDelete() {
  ...
  int res =
      MessageBox("Are you sure to delete this ScriptType? Don´t forget to delete any references to this ScriptType",
                 "Delete ScriptType", MB_YESNO | MB_ICONQUESTION);
  if (res != IDYES) return;

  ini.sections.erase((LPCTSTR)Scripttype);
  ini.sections["ScriptTypes"].values.erase(
      *ini.sections["ScriptTypes"].GetValueName(ini.sections["ScriptTypes"].FindValue((LPCTSTR)Scripttype)));
  // UpdateDialog();
  ((CFinalSunDlg*)theApp.m_pMainWnd)->UpdateDialogs(TRUE);
}
