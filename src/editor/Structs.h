...
enum class TheaterChar : char { T = 'T', A = 'A', U = 'U', N = 'N', D = 'D', L = 'L', Generic = 'G', None = 0 };
...
struct PICDATA {
  ...
  inline ProjectedVec drawOffset() const { return ProjectedVec(x, y); }
...
#  ifdef NOSURFACES_EXTRACT
  BOOL bNotExtracted;  // pic is not extracted, even if NOSURFACES_EXTRACT is specifed. Probably there is inner
                       // transparency!
#  endif
  BYTE* pic;  // optionally use direct blitting? This is either a palette index array or non-palette data (if
              // NOSURFACES_EXTRACT is specified)
  VBORDER* vborder;
#endif
  ...
  inline ProjectedVec drawOffset() const { return ProjectedVec(sX, sY); }
};
struct TILEDATA {
  TILEDATA() = default;
  ~TILEDATA();
...
struct XCString {
  ...
  void SetString(const WCHAR* wString, int len) {
    ...
    auto bufferSize =
        WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK, this->wString, len + 1, nullptr, 0, NULL, &bUsedDefault);
    if (bufferSize == 0) {
      cString = "";
      return;  // failed
    }
    ...
    if (WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK, this->wString, len + 1, (LPSTR)bByte.data(), bufferSize, NULL,
                            &bUsedDefault) == 0) {
      cString = "";
      return;  // failed
    }
