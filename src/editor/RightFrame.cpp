...
BOOL CRightFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext) {
  ...
  if (!m_Splitter.CreateView(0, 0, RUNTIME_CLASS(CIsoView), z, pContext)) return FALSE;

  z.cy = 100;

  if (!m_Splitter.CreateView(1, 0, RUNTIME_CLASS(CTileSetBrowserFrame), z, pContext)) return FALSE;
...
void CRightFrame::OnSize(UINT nType, int cx, int cy) { CFrameWnd::OnSize(nType, cx, cy); }
...
void CRightFrame::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) { CFrameWnd::OnChar(nChar, nRepCnt, nFlags); }
