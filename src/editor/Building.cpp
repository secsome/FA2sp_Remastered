...
CBuilding::CBuilding(CWnd* pParent /*=NULL*/) : CDialog(CBuilding::IDD, pParent) {
...
CString GetName(CString id) {
  ...
  if (ini.sections.find(id) != ini.sections.end()) {
    if (ini.sections[id].values.find("Name") != ini.sections[id].values.end()) return ini.sections[id].values["Name"];
  }
...
BOOL CBuilding::OnInitDialog() {
  ...
  int upgradecount = 0;
  if (strcmp(m_type, "GACTWR") == NULL) upgradecount = 1;
  ...
  if (upgradecount < 3) GetDlgItem(IDC_P8)->EnableWindow(FALSE);
  if (upgradecount < 2) GetDlgItem(IDC_P7)->EnableWindow(FALSE);
  if (upgradecount < 1) {
...
void CBuilding::Init(CString house, CString strength, CString direction, CString tag, CString flag1, CString flag2,
                     CString energy, CString upgradecount, CString spotlight, CString upgrade1, CString upgrade2,
                     CString upgrade3, CString flag3, CString flag4) {
  CIniFile& ini = Map->GetIniFile();
