...
CFinalSunApp::CFinalSunApp() {
  ...
  *(strrchr(AppPath, '\\') + 1) = 0;
  ...
  if ((GetFileAttributesW(AppDataPathFolder.c_str()) & FILE_ATTRIBUTE_DIRECTORY) != FILE_ATTRIBUTE_DIRECTORY) {
    MessageBoxW(NULL, (AppDataPathFolder + L" must be a directory, not a file").c_str(),
                (std::wstring(L"Failed to open ") + AppDataPathFolder).c_str(), 0);
    exit(1);
  }
...
BOOL CFinalSunApp::InitInstance() {
  ...
  if (GetDeviceCaps(GetDC(GetDesktopWindow()), BITSPIXEL) <= 8) {
    MessageBox(0,
               "You currently only have 8 bit color mode enabled. This is not recommended. You can continue, but this "
               "will cause a significant slowdown while loading graphics, and result in poor graphics quality",
               "Error", 0);
    // exit(0);
  }
#else
  if (GetDeviceCaps(GetDC(GetDesktopWindow()), BITSPIXEL) <= 8) {
    MessageBox(0,
               "You currently only have 8 bit color mode enabled. FinalSun/FinalAlert 2 will not work in 8 bit color "
               "mode. See readme.txt for further information!",
               "Error", 0);
    exit(0);
  }
#endif
  ...
  if (!DoesFileExist(iniFile.c_str())) {
    if (CopyFileW(utf8ToUtf16(templateIniFile).c_str(), utf8ToUtf16(iniFile).c_str(), TRUE)) copiedDefaultFile = true;
  }
  ...
  if (copiedDefaultFile || optini.sections.size() == 0 || opts.TSExe.GetLength() == 0 ||
  ...
  opts.bPreferLocalTheaterFiles =
      optini.sections[app]
          .values.emplace("PreferLocalTheaterFiles", opts.bPreferLocalTheaterFiles ? "1" : "0")
          .first->second == "1";
  ...
  opts.fLoadScreenDelayInSeconds = static_cast<float>(
      atof(optini.sections["UserInterface"]
               .values.emplace("LoadScreenDelay", std::to_string(opts.fLoadScreenDelayInSeconds).c_str())
               .first->second));
  opts.bShowStats =
      optini.sections["UserInterface"].values.emplace("ShowStats", opts.bShowStats ? "1" : "0").first->second == "1";
  opts.bHighResUI =
      optini.sections["UserInterface"].values.emplace("HighRes", opts.bHighResUI ? "1" : "0").first->second == "1";
  opts.useDefaultMouseCursor = optini.sections["UserInterface"]
                                   .values.emplace("UseDefaultMouseCursor", opts.useDefaultMouseCursor ? "1" : "0")
                                   .first->second == "1";

  opts.fMiniMapScale = static_cast<float>(atof(
      optini.sections["MiniMap"].values.emplace("Scale", std::to_string(opts.fMiniMapScale).c_str()).first->second));

  auto defaultViewSteps =
      CString(Join(",", opts.viewScaleSteps | std::views::transform([](auto v) { return std::to_string(v); })).c_str());
  auto viewScaleStepsRange =
      SplitParams(optini.sections["UserInterface"].values.emplace("ViewScaleSteps", defaultViewSteps).first->second) |
      std::views::transform([](auto v) { return static_cast<float>(std::atof(v)); });
  opts.viewScaleSteps.assign(viewScaleStepsRange.begin(), viewScaleStepsRange.end());
  opts.viewScaleUseSteps = optini.sections["UserInterface"]
                               .values.emplace("ViewScaleUseSteps", opts.viewScaleUseSteps ? "1" : "0")
                               .first->second == "1";
  opts.viewScaleSpeed =
      static_cast<float>(atof(optini.sections["UserInterface"]
                                  .values.emplace("ViewScaleSpeed", std::to_string(opts.viewScaleSpeed).c_str())
                                  .first->second));
  ...
  if (opts.bDoNotLoadTemperateGraphics && opts.bDoNotLoadSnowGraphics) {
    MessageBox(0,
               "You have turned off loading of both snow and temperate terrain in 'FinalAlert.ini'. At least one of "
               "these must be loaded. The application will now quit.",
               "Error", 0);
    exit(-982);
  }
  ...
  // MW 01/23/2013: changed the global CMapData Map to a global CMapData* to get rid of static initialization/shutdown
  // problems
  {
...
int CFinalSunApp::Run() { return CWinApp::Run(); }
...
BOOL CFinalSunApp::ProcessMessageFilter(int code, LPMSG lpMsg) {
  if (lpMsg->message == WM_KEYDOWN) {
    /*if(lpMsg->wParam==VK_F1)
    {
            ...
            {
                    MessageBox(0,(CString)"Could not open manual! Try opening
    "+(CString)AppPath+(CString)"/help/index.htm manually","Error",0);
            }
            return TRUE;
    }*/
    ...
    if (AD.mode == ACTIONMODE_SETTILE) {
      ...
      if (isox > r.right - r.left || isoy > r.bottom - r.top) {
        ...
        switch (lpMsg->wParam) {
          case VK_LEFT:
            if (AD.type > 0 && (*tiledata)[AD.type].wTileSet == (*tiledata)[AD.type - 1].wTileSet) {
              AD.type -= 1;
              AD.z_data = 0;
            }
            v->SendMessage(WM_MOUSEMOVE, 0, lParam);
            ...
            break;
          case VK_RIGHT:
            if (AD.type < (*tiledata_count) - 1 && (*tiledata)[AD.type].wTileSet == (*tiledata)[AD.type + 1].wTileSet) {
              AD.type += 1;
              AD.z_data = 0;
            }
            v->SendMessage(WM_MOUSEMOVE, 0, lParam);
          ...
          case VK_DOWN:
            if (lpMsg->wParam == VK_DOWN) searchedTileSet = (*tiledata)[AD.type].wTileSet + 1;
...
BOOL CFinalSunApp::PreTranslateMessage(MSG* pMsg) {
  if (!TranslateAccelerator(*this->m_pMainWnd,  // handle to receiving window
