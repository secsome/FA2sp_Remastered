...
CAll::CAll(CWnd* pParent /*=NULL*/) : CDialog(CAll::IDD, pParent) {
  //{{AFX_DATA_INIT(CAll)
...
void CAll::UpdateDialog() {
  ...
  for (i = ini.sections.size() - 1; i >= 0; i--) {
    ...
    if (!Map->IsMapSection(*name)) m_Sections.InsertString(0, *name);
  }
...
void CAll::OnDeletesection() {
  ...
  if (MessageBox(CString((CString) "Are you sure you want to delete " + str +
                         "? You should be really careful, you may not be able to use the map afterwards."),
                 "Delete section", MB_YESNO) == IDNO)
    return;
...
void CAll::OnAddkey() {
  ...
  CString key, value;
  key = InputBox(
      "Please set the name and value for the current key here: (for example, setting a new key "
      "Strength"
      " with the value 200 can be written as "
      "Strength=200"
      ". You don´t need to specify a value.)",
      "Create key");
