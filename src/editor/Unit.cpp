...
CUnit::CUnit(CWnd* pParent /*=NULL*/) : CDialog(CUnit::IDD, pParent) {
  //{{AFX_DATA_INIT(CUnit)
  m_direction = _T("");
  m_flag1 = _T("");
  m_flag2 = _T("");
...
void CUnit::Init(CString house, CString strength, CString direction, CString action, CString tag, CString flag1,
                 CString flag2, CString flag3, CString flag4, CString flag5, CString flag6) {
  CIniFile& ini = Map->GetIniFile();

  if (house == "") {
    m_house = *rules.sections["Houses"].GetValue(0);
    if (ini.sections.find("Houses") != ini.sections.end())
      if (ini.sections["Houses"].values.size() > 0) m_house = TranslateHouse(*ini.sections["Houses"].GetValue(0), TRUE);
  } else
