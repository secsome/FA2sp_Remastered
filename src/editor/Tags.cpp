...
void CTags::UpdateDialog() {
  ...
  if (m_Tag.SetCurSel(0) != CB_ERR) OnSelchangeTag();
...
void CTags::OnDelete() {
  ...
  int res = MessageBox(
      "Are you sure to delete the selected tag? This may cause the attached trigger to don´t work anymore, if no other "
      "tag has the trigger attached.",
      "Delete tag", MB_YESNO);
  if (res == IDNO) return;
