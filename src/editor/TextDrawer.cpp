...
TextDrawer::TextDrawer(IDirectDraw4* pDirectDraw, int fontSizeInPoints, COLORREF col, COLORREF shadowCol)
    : m_fontSizeInPoints(fontSizeInPoints), m_col(col), m_shadowCol(shadowCol) {
  ...
  CFont f;
  f.CreateFont(fontSizeInPixels, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_OUTLINE_PRECIS,
               CLIP_DEFAULT_PRECIS, NONANTIALIASED_QUALITY, VARIABLE_PITCH, "COURIER NEW");

  // Build a string that contains all required characters in order
  std::string s;
  for (char c = 32; c <= 126; ++c) s.push_back(c);
  ...
  if (pDirectDraw->CreateSurface(&desc, &pSurface, nullptr) != DD_OK) return;
  ...
  if (shadowCol != CLR_INVALID) {
    ...
    if (!TextOutA(hDC, 0, extent.cy, s.c_str(), s.size())) return;
  }
  SetTextColor(hDC, col);
  if (!TextOutA(hDC, 0, 0, s.c_str(), s.size())) return;

  if (pSurface->ReleaseDC(hDC) != DD_OK) return;
...
bool TextDrawer::isValid() const { return m_fontSurface != nullptr; }
  ...
  for (const auto c : text) {
    if (c == '\n') {
      ...
      if (m_shadowCol != CLR_INVALID) {
        ...
        target->BltFast(cur.x + 0 * shadowOffset, cur.y + 1 * shadowOffset, m_fontSurface, &s_shadow,
                        DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);
        target->BltFast(cur.x + 0 * shadowOffset, cur.y - 1 * shadowOffset, m_fontSurface, &s_shadow,
                        DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);
        target->BltFast(cur.x + 1 * shadowOffset, cur.y + 0 * shadowOffset, m_fontSurface, &s_shadow,
                        DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);
        target->BltFast(cur.x - 1 * shadowOffset, cur.y + 0 * shadowOffset, m_fontSurface, &s_shadow,
                        DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);

        target->BltFast(cur.x + 1 * shadowOffset, cur.y + 1 * shadowOffset, m_fontSurface, &s_shadow,
                        DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);
        target->BltFast(cur.x - 1 * shadowOffset, cur.y + 1 * shadowOffset, m_fontSurface, &s_shadow,
                        DDBLTFAST_SRCCOLORKEY | DDBLTFAST_WAIT);
        target->BltFast(cur.x + 1 * shadowOffset, cur.y - 1 * shadowOffset, m_fontSurface, &s_shadow,
