  ...
  for (parsepos = 0; parsepos < filesize; parsepos++) {
    if (inComment == TRUE || inNewOrder == FALSE) {
      ...
      if (data[parsepos] == '\\') {
        ...
        inParam = FALSE;
        // MessageBox(0, functiondata[functioncount-1].param[functiondata[functioncount-1].paramcount-1].data(),
        // functiondata[functioncount-1].name->data(), 0);
...
CUserScriptsDlg::CUserScriptsDlg(CWnd* pParent /*=NULL*/) : CDialog(CUserScriptsDlg::IDD, pParent) {
...
void CUserScriptsDlg::OnOK() {
  ...
  for (i = 0; i < s.functioncount; i++) {
    ...
    if (name == ASK_CONTINUE) {
    ...
    } else if (name == ADD_VEHICLE)
      info.type = ID_ADD_VEHICLE;
    else if (name == ADD_AIRCRAFT)
      info.type = ID_ADD_AIRCRAFT;
    else if (name == ADD_STRUCTURE)
      info.type = ID_ADD_STRUCTURE;
    else if (name == GET_INFANTRY)
      info.type = ID_GET_INFANTRY;
    else if (name == GET_AIRCRAFT)
      info.type = ID_GET_AIRCRAFT;
    else if (name == GET_STRUCTURE)
      info.type = ID_GET_STRUCTURE;
    else if (name == GET_VEHICLE)
      info.type = ID_GET_VEHICLE;
    else if (name == UINPUT_GET_HOUSE)
      info.type = ID_UINPUT_GET_HOUSE;
    else if (name == UINPUT_GET_COUNTRY)
      info.type = ID_UINPUT_GET_COUNTRY;
    else if (name == UINPUT_GET_TRIGGER)
      info.type = ID_UINPUT_GET_TRIGGER;
    else if (name == UINPUT_GET_TAG)
      info.type = ID_UINPUT_GET_TAG;
    else if (name == MESSAGE_YES_NO)
      info.type = ID_MESSAGE_YES_NO;
    else if (name == GET_HOUSE)
      info.type = ID_GET_HOUSE;
    else if (name == GET_COUNTRY)
      info.type = ID_GET_COUNTRY;
    else if (name == GET_HOUSE_INDEX)
      info.type = ID_GET_HOUSE_INDEX;
    else if (name == OR)
      info.type = ID_OR;
    else if (name == AND)
      info.type = ID_AND;
      ...
      info.type = ID_NOT;
    else if (name == UINPUT_GET_MANUAL)
      info.type = ID_UINPUT_GET_MANUAL;
  ...
  for (i = 0; i < s.functioncount; i++) {
    ...
    if (paramcount) {
      params = new (CString[paramcount]);
      int e;
      for (e = 0; e < paramcount; e++) params[e] = functions[i].params[e];
    }
    ...
    if (name == ID_SET_VARIABLE || name == ID_ADD || name == ID_SUBSTRACT || name == ID_MULTI || name == ID_DIVIDE ||
        ...
        name == ID_UINPUT_GET_INTEGER || name == ID_UINPUT_GET_STRING || name == ID_GET_RANDOM ||
        name == ID_ADD_TRIGGER || name == ID_ADD_AI_TRIGGER || name == ID_GET_INI_KEY || name == ID_MODULO ||
        name == ID_INSERT || name == ID_LENGTH || name == ID_TRIM || name == ID_GETCHAR || name == ID_REPLACE ||
        name == ID_REMOVE || name == ID_GET_PARAM || name == ID_SET_PARAM || name == ID_GET_PARAM_COUNT ||
        name == ID_IS_INFANTRY_DELETED || name == ID_IS_TERRAIN_DELETED || name == ID_GET_INFANTRY ||
        name == ID_GET_AIRCRAFT || name == ID_GET_STRUCTURE || name == ID_GET_VEHICLE ||
        name == ID_UINPUT_GET_TRIGGER || name == ID_UINPUT_GET_TAG || name == ID_UINPUT_GET_HOUSE ||
        name == ID_UINPUT_GET_COUNTRY || name == ID_MESSAGE_YES_NO || name == ID_OR || name == ID_AND ||
        name == ID_NOT || name == ID_UINPUT_GET_MANUAL) {
    ...
    if (name == ID_ASK_CONTINUE) {
      ...
      if (!enabled) {
        CString s;
        s = "This script wants to disable INI protection. For some scripts this may be necessary, but it can seriously "
            "damage your Map-> Reason why script wants to disable INI protection:\n";
        s += params[1];
    ...
    /*else if(name==ID_UINPUT_GET_MANUAL)
    {
      ...
      if (!bAllowLoop) {
        int res = MessageBox(
            "This script tries to use loops. Some scripts may require this. However, this holds the risk of infinite "
            "loops, which may cause the FinalSun/FinalAlert2 to crash. Do you want to allow looping for this script?",
            "Allow looping?", MB_YESNO);
        if (res == IDYES) {
          bAllowLoop = TRUE;
        } else
      ...
      if (!bIgnoreLoopCounts) {
        ...
        if (loop_count > 300) {
          int res = MessageBox(
              "This script has exceeded the 300 loops limit. Do you want to remove the loop limit (not recommended, "
              "inherits risk of infinite loops if script has bugs)? If you press no, the script will stop after "
              "another 300 loops to ask you again. If you press cancel, the script will be cancelled.",
              "Loop Limit exceeded", MB_YESNOCANCEL | MB_DEFBUTTON2);
      ...
      {
        CString s;
        s = "This script wants to delete objects or triggers from your map. For some scripts this may be necessary, "
            "but it can seriously damage your map. Reason why script wants to delete objects:\n";
        s += params[0];
      ...
      {
        CString s;
        s = "This script wants to add objects or triggers to your map. For some scripts this may be necessary, but it "
            "can seriously damage your map. Reason why script wants to add objects:\n";
        s += params[0];
        s += "\n\nDo you want to allow the script to do this?";
...
void CUserScriptsDlg::ReportScriptError(int line) {
  ...
  MessageBox("Script error in line " + (CString)c + ". Probably wrong parameter count or unknown function call.",
             "Error");
}
