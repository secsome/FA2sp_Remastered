...
void LineDrawer::DrawLine(int from_x, int from_y, int to_x, int to_y, int color, LineStyle style) {
  ...
  m_last_y = to_y;
  if (abs(w) >= abs(h)) {
    return w >= 0 ? DrawLineImplX(from_x, from_y, to_x, to_y, color, style)
                  : DrawLineImplX(to_x, to_y, from_x, from_y, color, style);
  } else {
    return h >= 0 ? DrawLineImplY(from_x, from_y, to_x, to_y, color, style)
                  : DrawLineImplY(to_x, to_y, from_x, from_y, color, style);
  }
}
...
void LineDrawer::DrawLineImplX(const int from_x, const int from_y, const int to_x, const int to_y, const int color,
  ...
  for (; cur_x < to_x; ++cur_x) {
    ...
    if (mod == 1 || (cur_x - from_x) % mod == 0) SetPixel(dest, cur_x, cur_y, color);
  }
}

void LineDrawer::DrawLineImplY(const int from_x, const int from_y, const int to_x, const int to_y, const int color,
                               LineStyle style) {
  ...
  for (; cur_y < to_y; ++cur_y) {
    ...
    if (mod == 1 || (cur_y - from_y) % mod == 0) SetPixel(dest, cur_x, cur_y, color);
  }
}
