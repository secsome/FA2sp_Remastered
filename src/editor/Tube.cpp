...
CTube::CTube(std::uint16_t tubeID, std::uint16_t startX, std::uint16_t startY, ETubeDirection direction,
             std::uint16_t endX, std::uint16_t endY, const std::vector<ETubeDirection>& tubeParts)
    : CTube(startX, startY, direction, endX, endY, tubeParts) {
  m_tubeId = tubeID;
}

CTube::CTube(const std::uint16_t startX, const std::uint16_t startY, const ETubeDirection direction,
             const std::uint16_t endX, const std::uint16_t endY, const std::vector<ETubeDirection>& tubeParts)
    : CTube() {
  m_startX = startX;
  m_startY = startY;
...
CTube::CTube(std::uint16_t tubeId, const std::string& value) : m_tubeId(tubeId) {
...
bool CTube::isValid() const { return m_direction != ETubeDirection::Undefined && m_tubeParts.size() > 1; }
...
CTube CTube::reverse(std::uint16_t newTubeID) const {
  ...
  auto reversed = m_tubeParts | std::views::reverse |
                  std::views::transform([](auto dir) { return opposite_dir(dir); }) | std::views::common;

  // find the first defined direction in the reversed list
  auto reversedFirst = std::find_if(reversed.begin(), reversed.end(),
                                    [](ETubeDirection dir) { return dir != ETubeDirection::Undefined; });
...
bool CTube::isEqual(const CTube& r, bool ignoreId) const {
  ...
  for (auto i = 0; i < smallerTubeParts.size(); ++i) {
    if (largerTubeParts[i] != smallerTubeParts[i]) return false;
  }
  for (auto i = smallerTubeParts.size(); i < largerTubeParts.size(); ++i) {
    if (largerTubeParts[i] != ETubeDirection::Undefined) return false;
  }

  return m_direction == r.m_direction && m_startX == r.m_startX && m_startY == r.m_startY && r.m_endX == r.m_endX &&
         r.m_endY == r.m_endY;
}

CTube CTube::autocreate(std::uint16_t startX, std::uint16_t startY, std::uint16_t endX, std::uint16_t endY,
                        int straightStartParts) {
  CTube ti;
  ti.m_startX = startX;
  ti.m_startY = startY;
  ti.m_endX = startX;
...
ETubeDirection opposite_dir(const ETubeDirection dir) {
  static const ETubeDirection opposite_dir_tbl[8] = {
      ETubeDirection::Bottom, ETubeDirection::BottomLeft, ETubeDirection::Left,  ETubeDirection::TopLeft,
      ETubeDirection::Top,    ETubeDirection::TopRight,   ETubeDirection::Right, ETubeDirection::BottomRight};
  const auto iDir = to_int(dir);
  if (iDir < 0 || iDir > 7) return ETubeDirection::Undefined;
...
ETubeDirection CTube::getLastDirection() const {
  if (m_tubeParts.empty()) return m_direction;
  auto last = std::find_if(m_tubeParts.crbegin(), m_tubeParts.crend(),
                           [](ETubeDirection dir) { return dir != ETubeDirection::Undefined; });
...
bool CTube::append(std::uint16_t endX, std::uint16_t endY, int forceStraightParts) {
  ...
  if (endOnExistingIt != existingPositions.end()) {
    if (endOnExistingIt == existingPositions.begin()) return false;  // zero length remaining
  ...
  while (end != cur) {
    ...
    cur += add;
    // if (std::find(existingPositions.begin(), existingPositions.end(), std::make_pair(curX, curY)) !=
    // existingPositions.end()) 	return false; // intersection - maybe shorten? maybe we can even allow it if TS accepts
    //it?
...
bool CTube::walk(const std::function<bool(const WalkInfo&)>& walker) const {
  ...
  for (auto dir : m_tubeParts) {
    ...
    if (!walker(wi)) return false;
    if (wi.direction == ETubeDirection::Undefined) break;
    wi.pos = wi.next_pos;
  }
  return true;
}
