...
void CTaskForce::UpdateDialog() {
  ...
  for (i = 0; i < rules.sections[ss].values.size(); i++) {
    ...
    s = type;
    s += " (";
    /*if(ini.sections.find((char*)(LPCTSTR)type)!=ini.sections.end() &&
    ini.sections[(char*)(LPCTSTR)type].values.find("Name")!=ini.sections[(char*)(LPCTSTR)type].values.end())
...
*/
  ...
  if (sel < 0) {
    if (m_TaskForces.SetCurSel(0) != CB_ERR) OnSelchangeTaskforces();
  } else {
    if (m_TaskForces.SetCurSel(sel) != CB_ERR) OnSelchangeTaskforces();
  }

  if (!(sel2 < 0)) {
    if (m_Units.SetCurSel(sel2) != LB_ERR) OnSelchangeUnits();
  }
}
...
void CTaskForce::OnDeletetaskforce() {
  ...
  int res;
  res = MessageBox(
      "Are you sure to delete the selected task force? If you delete it, make sure to eliminate ANY references to this "
      "task force in team-types.",
      "Delete task force", MB_YESNO);
  if (res == IDNO) return;
