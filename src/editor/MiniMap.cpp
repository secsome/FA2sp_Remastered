...
void CMiniMap::OnDraw(CDC* pDC) {
  ...
  // set the bits
  // SetDIBitsToDevice(pDC->m_hDC, 0, 0, Map->GetWidth()*2, Map->GetHeight(),0,0,0, Map->GetHeight(), colors, &biinfo,
  // DIB_RGB_COLORS);
  RECT r;
  GetClientRect(&r);
  StretchDIBits(pDC->m_hDC, 0, 0, r.right, r.bottom, 0, 0, Map->GetWidth() * 2, Map->GetHeight(), colors, &biinfo,
  ...
  auto bottomLeft =
      Map->GetMiniMapPos(isoview.GetMapCoordinatesFromClientCoordinates(CPoint(0, cr.bottom), false, true));
  auto bottomRight =
      Map->GetMiniMapPos(isoview.GetMapCoordinatesFromClientCoordinates(CPoint(cr.right, cr.bottom), false, true));
...
#ifdef _DEBUG
void CMiniMap::AssertValid() const { CView::AssertValid(); }

void CMiniMap::Dump(CDumpContext& dc) const { CView::Dump(dc); }
#endif  //_DEBUG
...
BOOL CMiniMap::PreCreateWindow(CREATESTRUCT& cs) {
  ...
  cs.style = WS_POPUPWINDOW | WS_CAPTION /*| WS_DLGFRAME */ | WS_THICKFRAME | WS_OVERLAPPED | DS_3DLOOK |
             WS_MINIMIZEBOX;  // WS_CAPTION | WS_OVERLAPPED | WS_SYSMENU | WS_MINIMIZEBOX;
...
void CMiniMap::UpdateView() {
  ...
  if (Map->GetIsoSize() == 0) {
    ...
    SetWindowPos(&wndTopMost, r.left, r.top, axissizex * m_scale + 2 * (GetSystemMetrics(SM_CXFIXEDFRAME)),
                 axissizey * m_scale + 2 * GetSystemMetrics(SM_CYFIXEDFRAME) + GetSystemMetrics(SM_CYCAPTION),
                 SWP_SHOWWINDOW);
...
void CMiniMap::OnLButtonDown(UINT nFlags, CPoint point) { OnMouseMove(nFlags, point); }
...
void CMiniMap::DrawMinimap(BYTE** lpData, BITMAPINFO& biinfo, int& Pitch) {
  ...
  if (data) {
    ...
    memcpy(&data[firstLine + (biinfo.bmiHeader.biWidth - 1) * sizeof(RGBTRIPLE)],
           &data[firstLine + (biinfo.bmiHeader.biWidth - 2) * sizeof(RGBTRIPLE)], sizeof(RGBTRIPLE));
  }
}
...
void CMiniMap::OnSizing(UINT fwSide, LPRECT pRect) {
  ...
  if (pRect) {
    ...
    if (fwSide == WMSZ_LEFT || fwSide == WMSZ_TOPLEFT || fwSide == WMSZ_BOTTOMLEFT || fwSide == WMSZ_RIGHT ||
...
BOOL CMiniMap::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) {
  if (!CView::OnSetCursor(pWnd, nHitTest, message)) SetCursor(m_hArrowCursor);
  return TRUE;
}
