...
DWORD CCliffModifier::GetTileToPlace(DWORD dwPos, BOOL* bSmall) {
  ...
  {
          if( (!m_bAlternative && (*tiledata)[i].wTileSet==cliffset) || (m_bAlternative &&
  (*tiledata)[i].wTileSet==cliff2set))
          {
                  dwStartSet=i;
                  break;
          }
  ...
  *bSmall = FALSE;
  ...
  for (i = 0; i < t1.cx; i++) {
    for (e = 0; e < t1.cy; e++) {
      ...
      int ground = fd->wGround;
      if (ground == 0xFFFF) ground = 0;
      if ((i == t1.cx - 1 || e == t1.cy - 1) &&
          (*tiledata)[ground].tiles[fd->bSubTile].bHackedTerrainType == TERRAINTYPE_WATER) {
        bWater = TRUE;
        break;
      }
    }
