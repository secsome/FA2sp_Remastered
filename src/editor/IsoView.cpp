...
/*UINT PaintThreadProc( LPVOID pParam )
{
        while(!bNoThreadDraw)
        {
                if(((CFinalSunDlg*)theApp.m_pMainWnd)->m_view.m_isoview!=NULL)
((CFinalSunDlg*)theApp.m_pMainWnd)->m_view.m_isoview->UpdateWindow();
        }
...
class SurfaceLocker {
 public:
  SurfaceLocker(IDirectDrawSurface4* pDDS, LPRECT rect = nullptr) : SurfaceLocker() {
  ...
  DDSURFACEDESC2* ensure_locked() {
    if (m_locked) return &m_ddsd;

    if (m_pDDS->GetSurfaceDesc(&m_ddsd) != DD_OK) return nullptr;

    if (m_pDDS->Lock(m_hasRect ? &m_rect : nullptr, &m_ddsd, DDLOCK_SURFACEMEMORYPTR | DDLOCK_WAIT | DDLOCK_NOSYSLOCK,
                     NULL) != DD_OK)
      return nullptr;

    m_locked = true;

    if (!m_ddsd.lpSurface) return nullptr;  // probably does not happen in reality
...
void CIsoView::OnInitialUpdate() { CView::OnInitialUpdate(); }
...
DWORD WINAPI ChangeHeightThread(LPVOID lpParameter  // thread data
) {
...
__forceinline void BlitTerrain(void* dst, int x, int y, int dleft, int dtop, int dpitch, int dright, int dbottom,
                               const SUBTILE& st)  // BYTE* src, int swidth, int sheight)
{
  BYTE* src = st.pic;
...
__forceinline void BlitTerrainHalfTransp(void* dst, int x, int y, int dleft, int dtop, int dpitch, int dright,
                                         int dbottom, const SUBTILE& st)  // BYTE* src, int swidth, int sheight)
{
  BYTE* src = st.pic;
  const unsigned short swidth = st.wWidth;
  const unsigned short sheight = st.wHeight;
...
inline void CalculateHouseColorPalette(int house_pal[houseColorRelMax + 1], const int* default_pal,
                                       const int* color = NULL) {
  if (color) {
    if (bpp == 4) {
...
__forceinline void BlitPic(void* dst, int x, int y, int dleft, int dtop, int dpitch, int dright, int dbottom,
                           PICDATA& pd, int* color = NULL, int* newPal = NULL)  // BYTE* src, int swidth, int sheight)
{
      ...
      if (val) {
        ...
        if (dest >= dst) {
          ...
          if (pLighting) {
            ...
            for (int i = 0; i < 4; ++i)
              bc[i] = min(255, bc[i] * (200 + l * 300 / 255) /
                                   255);  // game seems to overbrighten and have a lot of ambient - if you change this,
                                          // also change Loading.cpp shp lighting value so that shp light stays at 1.0
...
__forceinline void BlitPicHalfTransp(void* dst, int x, int y, int dleft, int dtop, int dpitch, int dright, int dbottom,
                                     PICDATA& pd, int* color = NULL,
                                     int* newPal = NULL)  // BYTE* src, int swidth, int sheight)
{
  ASSERT(pd.bType != PICDATA_TYPE_BMP);
...
#ifdef _DEBUG
void CIsoView::AssertValid() const { CView::AssertValid(); }

void CIsoView::Dump(CDumpContext& dc) const { CView::Dump(dc); }
#endif  //_DEBUG
...
BOOL CIsoView::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd,
...
void CIsoView::OnMouseMove(UINT nFlags, CPoint point) {
  ...
  if ((nFlags & MK_MBUTTON) == 0) m_zooming = false;
  ...
  if (AD.mode != ACTIONMODE_COPY && AD.mode != ACTIONMODE_PASTE) {
    if (x < 1 || y < 1 || x + y < mapwidth + 1 || x + y > mapwidth + mapheight * 2 ||
        (y + 1 > mapwidth && x - 1 < y - mapwidth) || (x + 1 > mapwidth && y + mapwidth - 1 < x)) {
      isMoving = FALSE;
      return;
    }
  } else {
...

  {
    ...
    if (AD.mode == ACTIONMODE_SETTILE && !((nFlags & MK_LBUTTON) && !(nFlags & MK_CONTROL) && (nFlags & MK_SHIFT))) {
      ...
      Map->TakeSnapshot(TRUE, x - width - 4, y - height - 4, x - width + m_BrushSize_x * width + 7,
                        y - height + m_BrushSize_y * height + 7);
      int cur_pos = pos;
      ...
      if (!((nFlags & MK_CONTROL) && (nFlags & MK_SHIFT))) {
        if (!theApp.m_Options.bDisableAutoShore)
          Map->CreateShore(x - width - 2, y - height - 2, x - width + td.cx * m_BrushSize_x + 5,
                           y - height + td.cy * m_BrushSize_y + 5, FALSE);
      ...
      RECT affect;
      if (m_mapx < x)
        affect.left = m_mapx - 2;
      else
        affect.left = x - 2;
      if (m_mapx < x)
        affect.right = x + 2;
      else
        affect.right = m_mapx + 2;
      if (m_mapy < y)
        affect.top = m_mapy - 2;
      else
        affect.top = y - 2;
      ...
      for (i = 0; i < x2 - x1 + 1; i++) {
        for (e = 0; e < y2 - y1 + 1; e++) {
          ...
          DrawCell(ddsd.lpSurface, ddsd.dwWidth, ddsd.dwHeight, ddsd.lPitch, x_s, y_s, 1, 1,
                   m_color_converter->GetColor(255, 0, 0));
      ...
      RedrawWindow(NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW);
      Map->Undo();
    } else if ((AD.mode == ACTIONMODE_PLACE || AD.mode == ACTIONMODE_RANDOMTERRAIN) && (nFlags & ~MK_CONTROL) == 0 &&
               AD.type != 7 &&
               (AD.type != 6 || (AD.type == 6 && ((AD.data >= 30 && AD.data <= 33) || AD.data == 2 ||
                                                  AD.data == 3))))  // everything placing but not overlay!
    {
      ...
      // if(AD.type!=1 || Map->GetInfantryCountAt(x+y*Map->GetIsoSize())==0)
      {
        for (i = 0; i < 32; i++) {
          for (e = 0; e < 32; e++) {
            ...
            for (z = 0; z < SUBPOS_COUNT; z++)
              if (oldData[i][e].infantry[z] > -1) Map->GetInfantryData(oldData[i][e].infantry[z], &infData[z][i][e]);
          }
        }
        ...
        for (i = 0; i < 32; i++) {
          for (e = 0; e < 32; e++) {
            ...
            if (cur_field.aircraft != oldData[i][e].aircraft) Map->DeleteAircraft(cur_field.aircraft);
            int z;
            for (z = 0; z < SUBPOS_COUNT; z++)
            ...
            if (cur_field.structure != oldData[i][e].structure) Map->DeleteStructure(cur_field.structure);
            if (cur_field.terrain != oldData[i][e].terrain) Map->DeleteTerrain(cur_field.terrain);
#ifdef SMUDGE_SUPP
            if (cur_field.smudge != oldData[i][e].smudge) Map->DeleteSmudge(cur_field.smudge);
#endif
            if (cur_field.unit != oldData[i][e].unit) Map->DeleteUnit(cur_field.unit);

            if (cur_field.overlay != oldData[i][e].overlay) Map->SetOverlayAt(dwPos, oldData[i][e].overlay);
      ...
      if (desc) DrawCellCursor(mapCoords, *desc);
  ...
  if (m_drag && AD.mode == 0) {
    ...
    dc.LineTo(to.x, to.y);
  } else if (AD.mode == ACTIONMODE_SETTILE && (nFlags & MK_LBUTTON) && ((nFlags & MK_SHIFT) || Map->hasLat(AD.type)) &&
             !(nFlags & MK_CONTROL)) {
    ...
    // if(abs(m_FlattenLastX-x)<2 && abs(m_FlattenLastY-y)<2) //ReachableFrom(x+y*Map->GetIsoSize(),
    // m_mapx+m_mapy*Map->GetIsoSize()))
    int ground = Map->GetFielddataAt(x + (y)*Map->GetIsoSize())->wGround;
    if (ground == 0xFFFF) ground = 0;
    ...
    // if((*tiledata)[ground].bMorphable)
    {
      ...
      for (m = left; m < right; m++) {
        for (n = top; n < bottom; n++) {
          ...
          if ((*tiledata)[ground].bMorphable) {
            ...
            for (i = 0; i < max; i++) Map->SetReserved(i, 0);
      ...
      */
      ...
      for (m = m_funcRect.left - 1; m <= m_funcRect.right + 1; m++)
        for (n = m_funcRect.top - 1; n <= m_funcRect.bottom + 1; n++) Map->CreateSlopesAt(m + n * isosize);
  ...
  } else if ((nFlags == MK_LBUTTON) && AD.mode == ACTIONMODE_NODE)  // nodes
  {
    ...
    if (AD.tool) AD.tool->onMouseMove(projCoords, mapCoords, MapToolMouseFlagsFromWin32(nFlags));
  } else if ((nFlags & MK_LBUTTON) == MK_LBUTTON &&
             (AD.mode == ACTIONMODE_PLACE || AD.mode == ACTIONMODE_RANDOMTERRAIN)) {
    // ADD OBJECTS
...
void CIsoView::DrawCellCursor(const MapCoords& mapCoords, const DDSURFACEDESC2& desc) {
  ...
  DrawCell(desc.lpSurface, desc.dwWidth, desc.dwHeight, desc.lPitch, drawCoords.x, drawCoords.y, 1, 1,
           m_color_converter->GetColor(_cell_hilight_colors[new_tileheight]), false, true,
           m_color_converter->GetColor(60, 60, 255));
...
void CIsoView::OnRButtonUp(UINT nFlags, CPoint point) {
  ...
  if (AD.mode != 0 && !rscroll) {
    ...
    if (AD.mode == ACTIONMODE_MAPTOOL) {
      if (AD.tool) ignoreClick = AD.tool->onRButtonUp(projCoords, mapCoords, MapToolMouseFlagsFromWin32(nFlags));
    }
    ...
    switch (wID) {
      case 4: {
        ...
        if (Map->GetAirAt(m_mapx + m_mapy * Map->GetIsoSize()) != -1) {
          ...
          int z;
          for (z = 0; z < SUBPOS_COUNT; z++)
            if (Map->GetInfantryAt(m_mapx + m_mapy * Map->GetIsoSize(), z) != -1)
              HandleProperties(Map->GetInfantryAt(m_mapx + m_mapy * Map->GetIsoSize(), z), 0);
...
void CIsoView::HandleProperties(int n, int type) {
  ...
  switch (type) {
    case 0: {
      ...
      CInfantrie dlg(this);
      char tmp[255];
      dlg.Init((LPCTSTR)data.house, (LPCTSTR)data.strength, (LPCTSTR)data.action, (LPCTSTR)data.direction,
               (LPCTSTR)data.tag, (LPCTSTR)data.flag1, (LPCTSTR)data.flag2, (LPCTSTR)data.flag3, (LPCTSTR)data.flag4,
               (LPCTSTR)data.flag5);
    ...
    case 1: {
      ...
      dlg.Init((LPCTSTR)data.house, (LPCTSTR)data.strength, (LPCTSTR)data.direction, (LPCTSTR)data.tag,
               (LPCTSTR)data.flag1, (LPCTSTR)data.flag2, (LPCTSTR)data.energy, (LPCTSTR)data.upgradecount,
               (LPCTSTR)data.spotlight, (LPCTSTR)data.upgrade1, (LPCTSTR)data.upgrade2, (LPCTSTR)data.upgrade3,
               (LPCTSTR)data.flag3, (LPCTSTR)data.flag4);
    ...
    case 2: {
      ...
      CAircraft dlg(this);
      char tmp[255];
      dlg.Init(data.house, data.strength, data.direction, data.action, data.tag, data.flag1, data.flag2, data.flag3,
               data.flag4);
...
void CIsoView::OnLButtonDblClk(UINT nFlags, CPoint point) {
  ...
  if (AD.mode == ACTIONMODE_MAPTOOL) {
    if (AD.tool) AD.tool->onLButtonDblClick(projCoords, mapCoords, MapToolMouseFlagsFromWin32(nFlags));
  } else {
    if (!Map->isInside(mapCoords)) return;
...
void CIsoView::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  if (AD.mode == ACTIONMODE_CLIFFFRONT || AD.mode == ACTIONMODE_CLIFFBACK) {
    ...
    if ((nFlags & MK_CONTROL) && !(nFlags & MK_SHIFT)) {
      ...
      Map->TakeSnapshot();
      int i;
      for (i = 0; i < Map->GetIsoSize() * Map->GetIsoSize(); i++) Map->SetReserved(i, 0);
      ...
      if ((*tiledata)[ground].bMorphable) {
        ...
        for (f = -m_BrushSize_x / 2; f < m_BrushSize_x / 2 + 1; f++) {
          for (n = -m_BrushSize_y / 2; n < m_BrushSize_y / 2 + 1; n++) {
            ...
            if ((*tiledata)[ground].bMorphable &&
                Map->GetHeightAt(x + f + (y + n) * Map->GetIsoSize()) == oheight + 1) {
              int i;
              for (i = 0; i < Map->GetIsoSize() * Map->GetIsoSize(); i++) Map->SetReserved(i, 0);
        ...
        try {
          ChangeTileHeight(x + (y)*Map->GetIsoSize(), Map->GetHeightAt(x + y * Map->GetIsoSize()) + 1, FALSE,
                           !(nFlags & MK_CONTROL));
        } catch (...) {
    ...
    for (m = -m_BrushSize_x / 2; m < m_BrushSize_x / 2 + 1; m++) {
      for (n = -m_BrushSize_y / 2; n < m_BrushSize_y / 2 + 1; n++) {
        Map->SetHeightAt(x + m + (y + n) * Map->GetIsoSize(),
                         Map->GetHeightAt(x + m + (y + n) * Map->GetIsoSize()) + 1);
      }
    }
    ...
    m_type = 5;
  } else if ((AD.mode < 3 || AD.mode > 4) || (AD.mode == 3 && AD.type == 0) ||
             (AD.mode >= 3 && AD.mode <= 4 && AD.type == 1)) {
...
void CIsoView::PlaceTile(const int x, const int y, const UINT nMouseFlags) {
  Map->TakeSnapshot(TRUE, x - 6, y - 6, x + (*tiledata)[AD.type].cx * m_BrushSize_x + 7,
                    y + (*tiledata)[AD.type].cy * m_BrushSize_y + 7);
  int i, e, f, n;
  int p = 0;
  int width = (*tiledata)[AD.type].cx;
  int height = (*tiledata)[AD.type].cy;
    ...
    for (n = 0; n < m_BrushSize_y; n++) {
      ...
      for (i = 0; i < (*tiledata)[AD.type].cx; i++) {
        for (e = 0; e < (*tiledata)[AD.type].cy; e++) {
          if (x - width + 1 + f * width + i >= Map->GetIsoSize() ||
              y - height + 1 + n * height + e >= Map->GetIsoSize()) {
          } else if ((*tiledata)[AD.type].tiles[p].pic != NULL) {
            Map->SetHeightAt(pos + i + f * width + (e + n * height) * Map->GetIsoSize(),
                             startheight + (*tiledata)[AD.type].tiles[p].bZHeight);
  ...
  if (!((nMouseFlags & MK_CONTROL) && (nMouseFlags & MK_SHIFT))) {
    if (!theApp.m_Options.bDisableAutoShore)
      Map->CreateShore(x - 5, y - 5, x + (*tiledata)[AD.type].cx * m_BrushSize_x + 5,
                       y + (*tiledata)[AD.type].cy * m_BrushSize_y + 5, FALSE);
  ...
  Map->TakeSnapshot(TRUE, x - 6, y - 6, x + (*tiledata)[AD.type].cx * m_BrushSize_x + 6,
                    y + (*tiledata)[AD.type].cy * m_BrushSize_y + 6);
  Map->Undo();
}
...
void CIsoView::OnLButtonUp(UINT nFlags, CPoint point) {
  ...
  if (AD.mode != ACTIONMODE_COPY && AD.mode != ACTIONMODE_PASTE && AD.mode != ACTIONMODE_MAPTOOL) {
    if (!Map->isInside(mapCoords)) return;
    if (dx < 1 || dy < 1 || dx + dy < mapwidth + 1 || dx + dy > mapwidth + mapheight * 2 ||
        (dy + 1 > mapwidth && dx - 1 < dy - mapwidth) || (dx + 1 > mapwidth && dy + mapwidth - 1 < dx))
      return;
  }
  ...
  if (m_drag && m_moved) {
    ...
    if (AD.tool) AD.tool->onLButtonUp(projCoords, mapCoords, MapToolMouseFlagsFromWin32(nFlags));
...
COLORREF CIsoView::GetColor(const char* house, const char* vcolor) {
  ...
  if (color) {
    ...
    if (colorArray.size() == 3) {
      unsigned char hsv[3] = {static_cast<unsigned char>(std::atoi(colorArray[0])),
                              static_cast<unsigned char>(std::atoi(colorArray[1])),
                              static_cast<unsigned char>(std::atoi(colorArray[2]))};
      unsigned char rgb[3];
      HSVToRGB(hsv, rgb);
...
void CIsoView::OnDraw(CDC* pDC) { DrawMap(); }
...
void CIsoView::ReInitializeDDraw() {
  ...
  while ((GetDeviceCaps(::GetDC(::GetDesktopWindow()), BITSPIXEL) <= 8)) {
    if (MessageBox(
            "You currently only have 8 bit color mode enabled. FinalAlert 2 does not work in 8 bit color mode. Please "
            "change the color mode and then click on OK. Click Cancel to quit (and save the map as backup.map).",
            "Error", MB_OKCANCEL) == IDCANCEL) {
  ...
  if(res==IDYES)
  {
          CFileDialog dlg(FALSE, ".mpr", "noname.mpr", OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "TS maps|*.mpr;*.map|TS
  multi maps|*.mpr|TS single maps|*.map|"); do
          {
  ...
  dlg.m_ofn.lpstrInitialDir=(char*)(LPCTSTR)theApp.m_Options.TSExe;
...
void CIsoView::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) { CView::OnChar(nChar, nRepCnt, nFlags); }

void CIsoView::OnDeadChar(UINT nChar, UINT nRepCnt, UINT nFlags) { CView::OnDeadChar(nChar, nRepCnt, nFlags); }
...
void CIsoView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  if (nChar == 65) m_bAltCliff = !m_bAltCliff;
...
int CIsoView::GetOverlayDirection(int x, int y) {
  ...
  if (isTrack(type))  // handling a train track
  {
    if (isTrack(Map->GetOverlayAt((x - 1) + (y - 1) * dwIsoSize)) &&
        isTrack(Map->GetOverlayAt((x + 1) + (y + 1) * dwIsoSize)))
      return 0;
    if (isTrack(Map->GetOverlayAt((x + 1) + (y - 1) * dwIsoSize)) &&
        isTrack(Map->GetOverlayAt((x - 1) + (y + 1) * dwIsoSize)))
      return 1;
    if (isTrack(Map->GetOverlayAt((x - 1) + (y - 0) * dwIsoSize)) &&
        isTrack(Map->GetOverlayAt((x + 1) + (y + 0) * dwIsoSize)))
      return 2;
    if (isTrack(Map->GetOverlayAt((x - 0) + (y - 1) * dwIsoSize)) &&
        isTrack(Map->GetOverlayAt((x + 0) + (y + 1) * dwIsoSize)))
      return 3;
    if (isTrack(Map->GetOverlayAt((x - 1) + (y - 0) * dwIsoSize)) &&
        isTrack(Map->GetOverlayAt((x + 1) + (y + 1) * dwIsoSize)))
      return 4;
    ...
    if (isTrack(Map->GetOverlayAt((x - 1) + (y + 1) * dwIsoSize)) &&
        isTrack(Map->GetOverlayAt((x + 1) + (y + 0) * dwIsoSize)))
      return 11;
    if (isTrack(Map->GetOverlayAt((x - 1) + (y - 1) * dwIsoSize))) return 0;
    if (isTrack(Map->GetOverlayAt((x - 1) + (y + 1) * dwIsoSize))) return 1;
    if (isTrack(Map->GetOverlayAt((x - 1) + (y - 0) * dwIsoSize))) return 2;
    if (isTrack(Map->GetOverlayAt((x - 0) + (y - 1) * dwIsoSize))) return 3;
    if (isTrack(Map->GetOverlayAt((x + 1) + (y + 1) * dwIsoSize))) return 0;
    if (isTrack(Map->GetOverlayAt((x + 1) + (y - 1) * dwIsoSize))) return 1;
    if (isTrack(Map->GetOverlayAt((x + 1) + (y + 0) * dwIsoSize))) return 2;
    if (isTrack(Map->GetOverlayAt((x + 0) + (y + 1) * dwIsoSize))) return 3;
...
void CIsoView::DrawCell(int x, int y, int w, int h, COLORREF col, BOOL dotted, HDC hDC_) {
  ...
  if (!hDC_) lpdsBack->ReleaseDC(hDC);

  DeleteObject(p);
}

void CIsoView::DrawCell(void* dest, int dest_width, int dest_height, int dest_pitch, int drawx, int drawy, int w, int h,
                        int col, bool dotted, bool touchNeighbours, int colNeighbour) const {
  if (colNeighbour == CLR_INVALID) colNeighbour = col;
...
void CIsoView::DrawTube(const CTube& tube, const DDSURFACEDESC2* lockedDDSD, const COLORREF* color) const {
  ...
  if (!lockedDDSD) return;

  // TODO: culling if required

  // find out if we are tube #1 or #2 for bidirectional tubes
  bool type0 =
      tube.getStartX() == tube.getEndX() ? (tube.getStartY() < tube.getEndY()) : (tube.getStartX() < tube.getEndX());
  const auto col = color ? m_color_converter->GetColor(*color)
                         : (type0 ? m_color_converter->GetColor(255, 0, 0) : m_color_converter->GetColor(0, 0, 255));
  ...
  tube.walk([this, &tube, lockedDDSD, &ld, lowestHeight, col, lineOffset](const auto& wi) {
    ...
    if (wi.pos == tube.getStartCoords()) {
      ...
      DrawCell(lockedDDSD->lpSurface, lockedDDSD->dwWidth, lockedDDSD->dwHeight, lockedDDSD->lPitch, draw.x, draw.y, 1,
               1, col, true);
    }
...
MapCoords CIsoView::GetMapCoordinates(const ProjectedCoords& projCoords, bool bAllowAccessBehindCliffs,
                                      bool ignoreHideFlagsAndOutside) const {
  return theApp.m_Options.bFlat ? Map->ToMapCoords(projCoords)
                                : Map->ToMapCoords3d(projCoords, bAllowAccessBehindCliffs, ignoreHideFlagsAndOutside);
}

MapCoords CIsoView::GetMapCoordinatesFromRenderTargetCoordinates(const ProjectedCoords& projCoords,
                                                                 bool bAllowAccessBehindCliffs,
                                                                 bool ignoreHideFlagsAndOutside) const {
  return GetMapCoordinates(projCoords + m_viewOffset, bAllowAccessBehindCliffs, ignoreHideFlagsAndOutside);
}

MapCoords CIsoView::GetMapCoordinatesFromClientCoordinates(const CPoint& clientPt, bool bAllowAccessBehindCliffs,
                                                           bool ignoreHideFlagsAndOutside) const {
  return GetMapCoordinates(GetProjectedCoordinatesFromClientCoordinates(clientPt), bAllowAccessBehindCliffs,
                           ignoreHideFlagsAndOutside);
}
...
ProjectedCoords CIsoView::ScaleBackToFrontBuffer(const ProjectedCoords& backBufferCoords) const {
  ...
  GetWindowRect(&r);
  const auto windowPos = ProjectedVec(r.left, r.top);
  return ((ProjectedCoords(backBufferCoords.x, backBufferCoords.y) - windowPos) / m_viewScale)
             .convertT<std::int32_t>() +
         windowPos;
}
...
void CIsoView::UpdateStatusBar(int x, int y) {
  ...
  if (Map->GetOverlayAt(x + y * Map->GetIsoSize()) != 0xFF) {
    ...
    name += ov;
    for (i = 0; i < overlay_count; i++) {
      if (overlay_number[i] == Map->GetOverlayAt(x + y * Map->GetIsoSize())) name = overlay_name[i];
    }
  ...
  if (AD.mode == ACTIONMODE_COPY) statusbar = GetLanguageStringACP("CopyHelp");

  if (statusbar.GetLength() > 0) SetError(statusbar);
}
...
__forceinline void FixTileHeight(DWORD dwPos, int x, int y) {
  ...
  for (i = -1; i < 2; i++) {
    for (e = -1; e < 2; e++) {
      if (x + i < 0 || y + e < 0 || x + i >= isosize || y + e >= isosize)
        bHeightDiff[i + 1 + (e + 1) * 3] = 0;
      else
  ...
  if ((bHeightDiff[1] < 0 && bHeightDiff[6] < 0) || (bHeightDiff[1] < 0 && bHeightDiff[8] < 0) /*||
...
*/
void __fastcall CIsoView::ChangeTileHeight(DWORD dwPos, DWORD dwNewHeight, BOOL bNonMorpheableMove, BOOL bOnlyThisTile,
                                           BOOL bNoSlopes) {
  int isosize = Map->GetIsoSize();
  ...
  // if(dwX==0 || dwY==0 || dwX+dwY<mapwidth || dwX+dwY>mapwidth+mapheight*2 || (dwY>mapwidth && dwX<dwY-mapwidth) ||
  // (dwX>mapwidth && dwY+mapwidth<dwX)) return; if( dwX<2|| dwY<2 || dwX+dwY<mapwidth+2 ||
  // dwX+dwY+2>mapwidth+mapheight*2 || (dwY+2>mapwidth && dwX-2<dwY-mapwidth) || (dwX+2>mapwidth && dwY+mapwidth-2<dwX))
  // return;
  if (dwX < 1 || dwY < 1 || dwX + dwY < mapwidth + 1 || dwX + dwY > mapwidth + mapheight * 2 ||
      (dwY + 1 > mapwidth && dwX - 1 < dwY - mapwidth) || (dwX + 1 > mapwidth && dwY + mapwidth - 1 < dwX))
    return;
      ...
      if (bAllowed && !fd.bReserved && pos > 0 && pos < mapsize) {
        ...
        if (bAllowed && (!bNonMorpheableMove || bDestMorphable)) {
          if (e == 1 && i == 1) {
            ...
            if (!(*tiledata)[leftGround].bMorphable && !(*tiledata)[rightGround].bMorphable) bAllowed = FALSE;
          }
          if (e == -1 && i == -1) {
            int leftGround = Map->GetFielddataAt(dwPos - 1)->wGround;
            int rightGround = Map->GetFielddataAt(dwPos - isosize)->wGround;
        ...
        if (/*(*tiledata)[ground].bMorphable &&*/ bAllowed) {
          int need_diff = (*tiledata)[ground].tiles[fd.bSubTile].bZHeight -
                          (*tiledata)[orig_ground].tiles[orig_fd.bSubTile].bZHeight;
          ...
          // if(Map->GetHeightAt(pos)-dwNewHeight!=need_diff || !bNonMorpheableMove)
          {
            ...
            */
            ...
            if (!bNonMorpheableMove && bDestMorphable) {
              ...
              {
                if (diff < -1) ChangeTileHeight(pos, dwNewHeight - 1, bNonMorpheableMove, bOnlyThisTile, bNoSlopes);
                if (diff > 1) ChangeTileHeight(pos, dwNewHeight + 1, bNonMorpheableMove, bOnlyThisTile, bNoSlopes);
              }
              /*else
              {
              ...
              if (!bDestMorphable /*&& (*tiledata)[fd.wGround].wTileSet==(*tiledata)[orig_fd.wGround].wTileSet*/
                  /*&& need_diff!=fd.bHeight-orig_fd.bHeight*/) {
                if (destTile.tiles[fd.bSubTile].bTerrainType == sourceTile.tiles[orig_fd.bSubTile].bTerrainType ||
                    destTile.wTileSet == sourceTile.wTileSet) {
                ...
                if ((sourceTile.cx > 1 && sourceTile.cy > 1 && (e < 0 && i < 0)) ||
                    ((sourceTile.cx < 2 || sourceTile.cy < 2) && (e > 0 && i > 0))) {
  ...
  if (!bNoSlopes) {
    for (i = -1; i < 2; i++) {
      for (e = -1; e < 2; e++) {
        int pos = dwPos + i + (e)*isosize;
        if (pos > 0 && pos < mapsize) Map->CreateSlopesAt(pos);
      }
    }
  }
}
...
BOOL CIsoView::ReachableFrom(DWORD dwStart, DWORD dwEnd) {
  ...
  for (i = -1; i < 2; i++) {
    for (e = -1; e < 2; e++) {
      ...
      if (!(*tiledata)[new_ground].bMorphable && ReachableFrom(dwStart + i + e * Map->GetIsoSize(), dwEnd)) return TRUE;
    }
  }
...
void CIsoView::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) { CView::OnKeyUp(nChar, nRepCnt, nFlags); }
...
void CIsoView::BlitBackbufferToHighRes() {
  // MW:
  // the primary buffer and the (optional) high-res buffer cover the whole screen (Windows automatic high DPI scaling
  // does not change this) the backbuffer also has the size of the whole screen, however we don't render to the whole
  // screen we only render to the visible window rectangle (which IS affected by Windows' automatic DPI scaling
  // features) Additionally, we have a user-controlled scale factor. If the user zooms in, we only render to a smaller
  // part of the backbuffer and stretch it to the window rectangle (which may then again be stretched by Windows DPI if
  // enabled in the Windows settings). the rectangle we render to has the same left/top border but a lower value for
  // right/bottom.
  //
  // This is a simple scaling and also scales any text that has already been rendered to the backbuffer (which is why
  // we're usually rendering text afterwards to the HighRes backbuffer). Altogether it'd probably be best to move from
  // DirectDraw to Direct3D, support high DPI awareness and do the scaling right when initially drawing the map.
        ...
        if (AD.data2 == 0) {
          ...
          for (i = 0; i < m_BrushSize_x; i++) {
            for (e = 0; e < m_BrushSize_y; e++) {
              ...
              if (ovrl > 0x65 && ovrl < 0x72) {
                if (Map->GetOverlayDataAt(i + e * Map->GetIsoSize() + dwPos) < 9)
                  Map->SetOverlayDataAt(dwPos + i + e * Map->GetIsoSize(),
                                        Map->GetOverlayDataAt(dwPos + i + e * Map->GetIsoSize()) + 1);
              }
            }
          }
        } else if (AD.data2 == 21) {
          ...
          for (i = 0; i < m_BrushSize_x; i++) {
            for (e = 0; e < m_BrushSize_y; e++) {
              int ovrl = Map->GetOverlayAt(dwPos + i + e * Map->GetIsoSize());
              if (ovrl > 0x65 && ovrl < 0x72) {
                if (Map->GetOverlayDataAt(dwPos + i + e * Map->GetIsoSize()) > 0)
                  Map->SetOverlayDataAt(dwPos + i + e * Map->GetIsoSize(),
                                        Map->GetOverlayDataAt(dwPos + i + e * Map->GetIsoSize()) - 1);
              }
            }
        ...
        if (AD.data2 == 0) {
          ...
          for (i = 0; i < m_BrushSize_x; i++) {
            for (e = 0; e < m_BrushSize_y; e++) {
              int ovrl = Map->GetOverlayAt(dwPos + i + e * Map->GetIsoSize());
              if (ovrl == o) {
                if (Map->GetOverlayDataAt(dwPos + i + e * Map->GetIsoSize()) < 9)
                  Map->SetOverlayDataAt(dwPos + i + e * Map->GetIsoSize(),
                                        Map->GetOverlayDataAt(dwPos + i + e * Map->GetIsoSize()) + 1);
              }
            }
...
void CIsoView::FillArea(DWORD dwX, DWORD dwY, DWORD dwID, BYTE bSubTile) {
  ...
  for (i = -1; i < 2; i++) {
    for (e = -1; e < 2; e++) {
      ...
      cur_x = dwX + i;
      cur_y = dwY + e;
      if (cur_x < 1 || cur_y < 1 || cur_x + cur_y < mapwidth + 1 || cur_x + cur_y > mapwidth + mapheight * 2 ||
          (cur_y + 1 > mapwidth && cur_x - 1 < cur_y - mapwidth) ||
          (cur_x + 1 > mapwidth && cur_y + mapwidth - 1 < cur_x))
        continue;
...
void CIsoView::AutoLevel() {
  ...
  for (i = 0; i < mapsize; i++) {
    ...
    for (x = -1; x < 2; x++) {
      for (y = -1; y < 2; y++) {
        ...
        if (x + ox > mapedge || x + ox < 0 || y + oy > mapedge || y + oy < 0) continue;
        ...
        if ((*tiledata)[cur_ground].wTileSet != (*tiledata)[o_ground].wTileSet) {
          if (((*tiledata)[o_ground].wTileSet == cliffset || (*tiledata)[o_ground].wTileSet == slopesetpiecesset) &&
              (*tiledata)[cur_ground].bMorphable) {
            int height_diff = f1->bHeight - cur_f->bHeight;
            if (o_ground >= 8 + iCliffStart && o_ground <= 13 + iCliffStart &&
                (*tiledata)[o_ground].tiles[cur_f->bSubTile].bZHeight == 0)
              height_diff += 4;
            int need_diff =
                height_diff;  //(*tiledata)[o_ground].tiles[f1->bSubTile].bZHeight-(*tiledata)[cur_ground].tiles[cur_f->bSubTile].bZHeight;
  ...
  for (i = 0; i < mapsize; i++) {
    if (bChanged[i]) {
      int e;
      for (e = 0; e < Map->GetIsoSize() * Map->GetIsoSize(); e++) Map->SetReserved(e, 0);
...
void CIsoView::DrawMap() {
  ...
  if (lpds == NULL || b_IsLoading || tiledata == NULL || (*tiledata) == NULL || (*tiledata_count == 0))
  ...
  // we render texts and waypoints last as they should be always visible anyway and because text rendering is currently
  // done using GDI -> getting a DC for every text is too slow nowadays
  ...
  for (u = left; u < right; u++) {
    for (v = top; v < bottom; v++) {
      ...
      // draw terrain
      if (m.wGround >= (*tiledata_count)) m.wGround = 0;
      ...
      if (!m.bRedrawTerrain) {
        ...
        if (td->bReplacementCount) {
          if (m.bRNDImage > 0) {
            m.bRNDImage <= td->bReplacementCount ? td = &td->lpReplacements[m.bRNDImage - 1]
                                                 : td = &td->lpReplacements[td->bReplacementCount - 1];
          }
        }
...
#else
            BlitTerrain(ddsd.lpSurface, stDrawCoords.x, stDrawCoords.y, r.left, r.top, ddsd.lPitch, r.right, r.bottom,
                        st);
#endif
...
#else
            BlitTerrainHalfTransp(ddsd.lpSurface, stDrawCoords.x, stDrawCoords.y, r.left, r.top, ddsd.lPitch, r.right,
                                  r.bottom, st);
#endif
          }
        }
  ...
  for (u = left; u < right; u++) {
    for (v = top; v < bottom; v++) {
          ...
          if (m.bSubTile < td->wTileCount && td->tiles[m.bSubTile].pic != NULL) {
            ...
            if (!m.bHide && (*tiledata)[dwOrigGround].bHide == FALSE) {
              ...
              BlitTerrain(ddsd.lpSurface, stDrawCoords.x, stDrawCoords.y, r.left, r.top, ddsd.lPitch, r.right, r.bottom,
                          st);
              if (st.anim) {
                const auto animDrawCoords =
                    drawCoords + ProjectedVec(f_x / 2 - st.anim->wMaxWidth / 2, f_y / 2 - st.anim->wMaxHeight / 2) +
                    st.anim->drawOffset();
                BlitPic(ddsd.lpSurface, animDrawCoords.x, animDrawCoords.y, r.left, r.top, ddsd.lPitch, r.right,
                        r.bottom, *st.anim);
              }
#endif
      ...
      // tunnel tops might be repainted later so that units do not appear on top - this probably could be done in
      // CLoading so that instead of a back tile a front tile is responsible for drawing the animation
      {
        if (m.wGround < *tiledata_count) {
          TILEDATA* td = &(*tiledata)[m.wGround];
          ...
          if (m.bSubTile < td->wTileCount) {
            ...
            if (!m.bHide && (*tiledata)[dwOrigGround].bHide == FALSE && st.anim) {
              ...
              const auto animDrawCoords =
                  drawCoords + ProjectedVec(f_x / 2 - st.anim->wMaxWidth / 2, f_y / 2 - st.anim->wMaxHeight / 2) +
                  st.anim->drawOffset();
              BlitPic(ddsd.lpSurface, animDrawCoords.x, animDrawCoords.y, r.left, r.top, ddsd.lPitch, r.right, r.bottom,
                      *st.anim);
#endif
            }
          }
      ...
      if (m.overlay != 0xFF) {
        ...
        if (pic.pic == NULL) {
          if (!pic.bTried) {
            ...
            if (ovrlpics[m.overlay][m.overlaydata] != NULL) pic = *ovrlpics[m.overlay][m.overlaydata];
          }
          ...
          if (pic.pic == NULL) {
            if (!(m.overlay >= 0x4a && m.overlay <= 0x65) && !(m.overlay >= 0xcd && m.overlay <= 0xec)) {
              ...
              m_texts_to_render.push_back(
                  {cd, drawCoords.x + f_x / 2, drawCoords.y + f_y / 2, RGB(0, 0, 0), false, true, true});
...
#else
          BlitPic(ddsd.lpSurface, drawCoordsOvrl.x, drawCoordsOvrl.y, r.left, r.top, ddsd.lPitch, r.right, r.bottom,
                  pic);
#endif
        }
      }
      ...
      if (m.structure != -1) {
        ...
        if (Map->GetStructureAt(mapCoords - MapVec(-1, 0)) != m.structure &&
            Map->GetStructureAt(mapCoords - MapVec(0, -1)) != m.structure) {
          STRUCTUREPAINT objp;
          ...
          if (theApp.m_Options.bShowCells) {
            DrawCell(ddsd.lpSurface, ddsd.dwWidth, ddsd.dwHeight, ddsd.lPitch, drawCoordsBld.x, drawCoordsBld.y, w, h,
                     colorref_conv[objp.col]);
          }
#endif
          ...
          if (pic.pic)  // picture
          {
            ...
            const auto drawCoordsBldShp =
                drawCoordsBld + ProjectedVec(f_x / 2 - pic.wMaxWidth / 2, -pic.wMaxHeight / 2);

#ifndef NOSURFACES
            Blit(pic.pic, drawCoordsBldShp.x, drawCoordsBldShp.y, pic.wMaxWidth, pic.wMaxHeight);
#else

            BlitPic(ddsd.lpSurface, drawCoordsBldShp.x, drawCoordsBldShp.y, r.left, r.top, ddsd.lPitch, r.right,
                    r.bottom, pic, &colorref_conv[objp.col]);
#endif
            ...
            for (int upgrade = 0; upgrade < objp.upradecount; ++upgrade) {
              ...
              if (pic.pic != NULL) {
                static const CString LocLookup[3][2] = {{"PowerUp1LocXX", "PowerUp1LocYY"},
                                                        {"PowerUp2LocXX", "PowerUp2LocYY"},
                                                        {"PowerUp3LocXX", "PowerUp3LocYY"}};
                const auto drawCoordsPowerUp =
                    drawCoordsBldShp + ProjectedVec(atoi(art.sections[objp.type].values[LocLookup[upgrade][0]]),
...
#else
                BlitPic(ddsd.lpSurface, drawCoordsPowerUp.x, drawCoordsPowerUp.y, r.left, r.top, ddsd.lPitch, r.right,
                        r.bottom, pic, &colorref_conv[objp.col]);
#endif
              }
            }
      ...
      if (m.node.type > -1) {
        ...
        if (Map->GetNodeAt(mapCoords + MapVec(1, 0), tmp) != m.node.index &&
            Map->GetNodeAt(mapCoords + MapVec(0, 1), tmp) != m.node.index) {
          const auto drawCoordsBld = GetRenderTargetCoordinates(
              mapCoords - MapVec(buildinginfo[m.node.type].h - 1, buildinginfo[m.node.type].w - 1));
...
#ifdef NOSURFACES
          if (m.structure >=
              0)  // only paint cell if we have a structure preplaced, as we have a half transparent image
          {
#endif
            // place it 2 pixels lower so that user can see the dotted lines even if the building itself has the cells
            // drawn
            DrawCell(ddsd.lpSurface, ddsd.dwWidth, ddsd.dwHeight, ddsd.lPitch, drawCoordsBld.x, drawCoordsBld.y + 3, w,
                     h, colorref_conv[c], true);
...
#else
            BlitPicHalfTransp(ddsd.lpSurface, drawCoordsBldShp.x, drawCoordsBldShp.y, r.left, r.top, ddsd.lPitch,
      ...
      if (m.unit != -1) {
        ...
        if (p.pic == NULL || lpPicFile.GetLength() == 0) {
          ...
          if (p.pic == NULL) {
            ...
            m_texts_to_render.push_back(
                {obj.type, drawCoords.x + f_x / 4, drawCoords.y + f_y / 4, m_color_converter->GetColor(c)});
#endif
        ...
        {
          const auto drawCoordsOffset =
              (p.bType == PICDATA_TYPE_BMP)   ? ProjectedVec((f_y / 4) + p.x, (f_y - p.wHeight) + p.y) - p.drawOffset()
              : (p.bType == PICDATA_TYPE_SHP) ? ProjectedVec(f_x / 2 - (p.wMaxWidth / 2), f_y / 2 - (p.wMaxHeight / 2))
...
#else
          BlitPic(ddsd.lpSurface, drawCoordsUnit.x, drawCoordsUnit.y, r.left, r.top, ddsd.lPitch, r.right, r.bottom, p,
                  &colorref_conv[c]);
#endif
        }
      }
      if (m.aircraft != -1) {
        ...
        {
          const auto drawCoordsOffset =
              (p.bType == PICDATA_TYPE_BMP)   ? ProjectedVec(f_x / 2 - p.wWidth / 2, f_y - p.wHeight) - p.drawOffset()
              : (p.bType == PICDATA_TYPE_SHP) ? ProjectedVec(f_x / 2 - (p.wMaxWidth / 2), f_y / 2 - (p.wMaxHeight / 2))
                                              : ProjectedVec(f_x / 2, f_y / 2) + p.drawOffset();
...
#else
          BlitPic(ddsd.lpSurface, drawCoordsAir.x, drawCoordsAir.y, r.left, r.top, ddsd.lPitch, r.right, r.bottom, p,
                  &colorref_conv[c]);
#endif
        }
      }
      ...
      for (ic = 0; ic < SUBPOS_COUNT; ic++)
        if (m.infantry[ic] != -1) {
          ...
          static const ProjectedVec subPosLookup[5] = {ProjectedVec(0, -f_y / 4), ProjectedVec(f_x / 4, 0),
                                                       ProjectedVec(-f_x / 4, 0), ProjectedVec(0, f_y / 4),
                                                       ProjectedVec()};
          ...
          if (p.pic == NULL) {
            ...
            if (p.pic == NULL) {
              ...
              // TextOut(drawx+f_x/4,drawy+f_y/4, obj.type,c);
              m_texts_to_render.push_back(
                  {obj.type, drawCoordsInf.x + f_x / 4, drawCoordsInf.y + f_y / 4, RGB(0, 0, 0)});
#endif
              missingimages[obj.type] = TRUE;
            }
          }
          ...
          if (p.pic)  // we have a picture!
          {
            auto drawCoordsInfShp =
                drawCoordsInf + ProjectedVec(f_x / 2 - (p.wMaxWidth / 2), f_y / 2 - (p.wMaxHeight / 2));

#ifndef NOSURFACES
            Blit(p.pic, drawCoordsInfShp.x, drawCoordsInfShp.y, p.wWidth, p.wHeight);
#else
            BlitPic(ddsd.lpSurface, drawCoordsInfShp.x, drawCoordsInfShp.y, r.left, r.top, ddsd.lPitch, r.right,
                    r.bottom, p, &colorref_conv[c]);
#endif
          }
        }
      if (m.terrain != -1) {
        ...
        if (pic.pic)
          ;
        {
          auto drawCoordsTerrain =
              drawCoords + ProjectedVec(f_x / 2 - (pic.wMaxWidth / 2), f_y / 2 - 3 - (pic.wMaxHeight / 2));

#ifndef NOSURFACES
          Blit(pic.pic, drawCoordsTerrain.x, drawCoordsTerrain.y);
#else
          BlitPic(ddsd.lpSurface, drawCoordsTerrain.x, drawCoordsTerrain.y, r.left, r.top, ddsd.lPitch, r.right,
                  r.bottom, pic);
...
#  endif
        ...
        if (pic.pic) {
          auto drawCoordsSmudge =
              drawCoords + ProjectedVec(f_x / 2 - (pic.wMaxWidth / 2), /*f_y / 2 - 3*/ -(pic.wMaxHeight / 2));

#  ifndef NOSURFACES
          Blit(pic.pic, drawCoordsSmudge.x, drawCoordsSmudge.y);
#  else
          BlitPic(ddsd.lpSurface, drawCoordsSmudge.x, drawCoordsSmudge.y, r.left, r.top, ddsd.lPitch, r.right, r.bottom,
                  pic);
...
#  endif
      ...
      if (m.waypoint != -1) {
        ...
        m_texts_to_render.push_back(
            {ID.GetString(), waypointTextCoords.x, waypointTextCoords.y, RGB(0, 0, 255), false, useFont9, true});
#ifdef NOSURFACES
        lpdsBack->Lock(NULL, &ddsd, DDLOCK_SURFACEMEMORYPTR | DDLOCK_WAIT | DDLOCK_NOSYSLOCK, NULL);
#endif
      }
  ...
  if (m_cellCursor != MapCoords(-1, -1)) {
    SurfaceLocker locker(lpdsBack);
    auto desc = locker.ensure_locked();
    if (desc) DrawCellCursor(m_cellCursor, *desc);
  }

  auto renderDuration = std::chrono::duration_cast<std::chrono::duration<float>>(
      std::chrono::steady_clock::now() -
      startTime);  // not including text output, ReleaseDC, waitForVerticalBlank etc below

  if (bDrawStats) {
    if (theApp.m_Options.bShowStats) {
      auto systemStats = std::format("ms: {0}  Left: {1}  Top: {2}  Right: {3}  Bottom: {4}", renderDuration.count(),
                                     left, top, right, bottom);
      m_texts_to_render.push_back(
          {systemStats.c_str(), r.left + 10, r.top + 10 + (-m_fontDefaultHeight) * 3 / 2, RGB(0, 0, 0), true});
    }
  ...
  if (rscroll) {
    ...
    Blit((LPDIRECTDRAWSURFACE4)sc.pic, rclick_x * m_viewScale.x + r.left - sc.wWidth / 2,
         rclick_y * m_viewScale.y + r.top - sc.wHeight / 2);
  }

  BlitBackbufferToHighRes();  // lpdsBackHighRes contains the same graphic, but scaled to the whole window

  RenderUIOverlay();
  if (theApp.m_Options.bVSync) dd->WaitForVerticalBlank(DDWAITVB_BLOCKBEGIN, NULL);
  FlipHighResBuffer();
  last_succeeded_operation = 10100;
}

void CIsoView::RenderUIOverlay() {
  if (!m_textDefault) updateFontScaled();
  ...
  RECT ls;
  Map->GetLocalSize(&ls);
  ProjectedCoords l_lt(ll + ls.left * f_x - f_x / 2, lt + (ls.top - 4) * f_y);
  ProjectedCoords l_br(ll + ls.left * f_x + ls.right * f_x - f_x / 2, lt + (ls.top - 4 + 4) * f_y + ls.bottom * f_y);
  ...
  for (const auto& s : m_texts_to_render) {
    ...
    if (s.fixedScreenPos || !useHighRes) {
      ...
      auto textRenderer =
          s.useFont9 ? (blue ? *m_textBlue9Scaled : *m_text9Scaled) : (blue ? *m_textBlueScaled : *m_textScaled);
      textRenderer.RenderText(dds, r.left + (s.drawx - r.left) / m_viewScale.x,
                              r.top + (s.drawy - r.top) / m_viewScale.y, s.text, s.centered);
    }
  }
}
...
void CIsoView::FocusWaypoint(int index) {
  ...
  auto pos = ProjectedVec((x - (r.right - r.left) / 2 * m_viewScale.x) - r.left,
                          (y - (r.bottom - r.top) / 2 * m_viewScale.y) - r.top);
...
void CIsoView::Zoom(CPoint& pt, float f) {
  ...
  if (m_viewScaleControl > 1.0f) m_viewScaleControl = 1.0f;
  if (m_viewScaleControl < 0.1f) m_viewScaleControl = 0.1f;

  if (!theApp.m_Options.viewScaleUseSteps)
    m_viewScale.set(m_viewScaleControl, m_viewScaleControl);
  else {
    // find previous step
    auto prevStepIt = std::find_if(theApp.m_Options.viewScaleSteps.rbegin(), theApp.m_Options.viewScaleSteps.rend(),
                                   [oldScale](auto f) { return f >= oldScale.x; });
    auto stepIt = std::find_if(theApp.m_Options.viewScaleSteps.rbegin(), theApp.m_Options.viewScaleSteps.rend(),
                               [this](auto f) { return f >= m_viewScaleControl; });
    if (stepIt > prevStepIt) stepIt = prevStepIt + 1;
    if (stepIt < prevStepIt) stepIt = prevStepIt - 1;
  ...
  m_viewOffset =
      m_viewOffset +
      ((oldScaledMPos - newScaledMPos)).convertT<std::int32_t>();  // this lets the mouse cursor pixel stay constant
