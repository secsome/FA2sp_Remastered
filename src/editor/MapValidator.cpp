...
CMapValidator::CMapValidator(CWnd* pParent /*=NULL*/) : CDialog(CMapValidator::IDD, pParent) {
...
BOOL CMapValidator::CheckMap() {
  ...
  if (ini.sections.find("Basic") == ini.sections.end() || ini.sections["Basic"].values.size() == 0) {
    ...
    if (ini.sections.find(MAPHOUSES) == ini.sections.end()) {
      ...
      if (below8found < 8) {
        ...
        if (below8found < 2) AddItemWithNewLine(m_MapProblemList, GetLanguageStringACP("MV_HousesNoWaypoints"), 1);
      }
  ...
  for (auto& tube : tubes) {
    auto n_reverse =
        std::count_if(tubes.begin(), tubes.end(), [&tube](const auto& other) { return tube->isCounterpart(*other); });
    if (n_reverse == 0) {
      CString error = TranslateTubeString(GetLanguageStringACP("MV_TubeCounterpartMissing"), *tube, n_reverse);
      AddItemWithNewLine(m_MapProblemList, error, 0);
    }
    auto n_same_start = std::count_if(tubes.begin(), tubes.end(), [&tube](const auto& other) {
      return tube->getStartCoords() == other->getStartCoords() && *tube != *other;
    });
    if (n_same_start) {
      CString error = TranslateTubeString(GetLanguageStringACP("MV_TubeStartNotUnique"), *tube, n_same_start);
      AddItemWithNewLine(m_MapProblemList, error, 0);
    }
    auto n_same_end = std::count_if(tubes.begin(), tubes.end(), [&tube](const auto& other) {
      return tube->getEndCoords() == other->getEndCoords() && *tube != *other;
    });
    if (n_same_end) {
      CString error = TranslateTubeString(GetLanguageStringACP("MV_TubeEndNotUnique"), *tube, n_same_end);
      AddItemWithNewLine(m_MapProblemList, error, 0);
    }
    auto n_invalid_counterpart_end = std::count_if(tubes.begin(), tubes.end(), [&tube](const auto& other) {
      return tube->getEndCoords() == other->getStartCoords() && tube->getStartCoords() != other->getEndCoords();
    });
    if (n_invalid_counterpart_end) {
      CString error =
          TranslateTubeString(GetLanguageStringACP("MV_TubeInvalidCounterpartEnd"), *tube, n_invalid_counterpart_end);
      AddItemWithNewLine(m_MapProblemList, error, 0);
    }
    auto n_invalid_counterpart_start = std::count_if(tubes.begin(), tubes.end(), [&tube](const auto& other) {
      return tube->getStartCoords() == other->getEndCoords() && tube->getEndCoords() != other->getStartCoords();
    });
    if (n_invalid_counterpart_start) {
      CString error = TranslateTubeString(GetLanguageStringACP("MV_TubeInvalidCounterpartStart"), *tube,
  ...
  if (bWaypBig) AddItemWithNewLine(m_MapProblemList, GetLanguageStringACP("MV_>100Waypoint"), 1);
