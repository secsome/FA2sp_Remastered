...
inline BOOL isTrue(CString expr) {
  expr.MakeLower();
  if (expr == "yes" || expr == "true" || expr == "1") return TRUE;
  return FALSE;
}

inline BOOL isFalse(CString expr) {
  expr.MakeLower();
  if (expr == "no" || expr == "false" || expr == "0") return TRUE;
  return FALSE;
}

// retrieve the picture filename of a unit (as it is saved in the pics map). The returned file may not exist in the pics
// map (you need to do a check!).
inline CString GetUnitPictureFilename(LPCTSTR lpUnitName, DWORD dwPicIndex) {
  ...
  if (art.sections[UnitName].FindName("NewTheater") >= 0 &&
      art.sections[UnitName].AccessValueByName("DemandLoad") != "yes")
    if (art.sections[UnitName].AccessValueByName("NewTheater") == "yes") filename.SetAt(1, 'T');
...
inline CString GetParam(const CString& data, const int param) {
  ...
  while (curParam--) {
    ...
    if (nextComma < 0) return CString();  // RVO; param not found
...
inline std::string GetParam(const std::string& data, const int param) {
  ...
  while (curParam--) {
    auto nextComma = data.find(',', paramStrPos);
    if (nextComma == std::string::npos) return std::string();  // RVO; param not found
    paramStrPos = nextComma + 1;
  }

  auto nextComma = data.find(',', paramStrPos);
  std::string res =
      data.substr(paramStrPos, nextComma == std::string::npos ? data.size() - paramStrPos : nextComma - paramStrPos);
  return res;  // RVO
}

inline CString GetParam(const char* data, const int param) { return GetParam(CString(data), param); }
...
inline std::vector<CString> SplitParams(const CString& data) { return Split(data, ','); }

inline CString Join(const CString& join, const std::vector<CString>& strings) {
  CString res;
  int len = 0;
  for (auto& s : strings) len += s.GetLength() + join.GetLength();
...
inline BOOL isTrack(int type) { return (type >= OVRL_TRACK_BEGIN && type <= OVRL_TRACK_END); }
