...
CTriggerEventsDlg::CTriggerEventsDlg(CWnd* pParent /*=NULL*/) : CDialog(CTriggerEventsDlg::IDD, pParent) {
...
void CTriggerEventsDlg::OnEditchangeEventtype() {
  ...
  if (atoi(GetParam(ini.sections["Events"].values[(LPCTSTR)m_currentTrigger], pos + 1)) == 2) bAlready4 = TRUE;

  ini.sections["Events"].values[(LPCTSTR)m_currentTrigger] =
      SetParam(ini.sections["Events"].values[(LPCTSTR)m_currentTrigger], pos, (LPCTSTR)eventtype);
  ...
  if (g_data.sections["EventsRA2"].FindName(eventtype) >= 0) eventdata = g_data.sections["EventsRA2"].values[eventtype];
  ...
  if (code == 2) {
    ...
    if (!bAlready4)
      ini.sections["Events"].values[(LPCTSTR)m_currentTrigger] =
          SetParam(ini.sections["Events"].values[(LPCTSTR)m_currentTrigger], pos + 1, "2,0");
    ...
    itoa(code, c, 10);
    ini.sections["Events"].values[(LPCTSTR)m_currentTrigger] =
        SetParam(ini.sections["Events"].values[(LPCTSTR)m_currentTrigger], pos + 1, c);
...
void CTriggerEventsDlg::OnSelchangeParameter() {
  ...
  if (g_data.sections["EventsRA2"].FindName(GetParam(EventData, startpos)) >= 0)
    ParamType = GetParam(g_data.sections["EventsRA2"].values[GetParam(EventData, startpos)], 1 + curparam);
