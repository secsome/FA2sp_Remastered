...
CTriggerOptionsDlg::CTriggerOptionsDlg(CWnd* pParent /*=NULL*/) : CDialog(CTriggerOptionsDlg::IDD, pParent) {
...
void CTriggerOptionsDlg::UpdateDialog() {
  ...
  for (i = 0; i < m_AttachedTrigger.GetCount(); i++) {
    ...
    TruncSpace(tmp);
    if (tmp == attachedTrigger) m_AttachedTrigger.SetCurSel(i);
  }
...
void CTriggerOptionsDlg::OnChangeName() {
  ...
  ini.sections["Triggers"].values[m_currentTrigger] =
      SetParam(ini.sections["Triggers"].values[m_currentTrigger], 2, newName);
...
void CTriggerOptionsDlg::OnEditchangeHouse() {
  ...
  ini.sections["Triggers"].values[m_currentTrigger] =
      SetParam(ini.sections["Triggers"].values[m_currentTrigger], 0, newHouse);
...
void CTriggerOptionsDlg::OnEditchangeAttachedtrigger() {
  ...
  ini.sections["Triggers"].values[m_currentTrigger] =
      SetParam(ini.sections["Triggers"].values[m_currentTrigger], 1, newTrigger);
}

void CTriggerOptionsDlg::OnKillfocusName() { ((CTriggerEditorDlg*)(this->GetOwner()->GetOwner()))->UpdateDialog(); }
...
void CTriggerOptionsDlg::OnDisabled() {
  ...
  if (bDisabled)
    ini.sections["Triggers"].values[m_currentTrigger] =
        SetParam(ini.sections["Triggers"].values[m_currentTrigger], 3, "1");
  else
    ini.sections["Triggers"].values[m_currentTrigger] =
        SetParam(ini.sections["Triggers"].values[m_currentTrigger], 3, "0");
}
...
void CTriggerOptionsDlg::OnEasy() {
  ...
  if (bEasy)
    ini.sections["Triggers"].values[m_currentTrigger] =
        SetParam(ini.sections["Triggers"].values[m_currentTrigger], 4, "1");
  else
    ini.sections["Triggers"].values[m_currentTrigger] =
        SetParam(ini.sections["Triggers"].values[m_currentTrigger], 4, "0");
}
...
void CTriggerOptionsDlg::OnMedium() {
  ...
  if (bMedium)
    ini.sections["Triggers"].values[m_currentTrigger] =
        SetParam(ini.sections["Triggers"].values[m_currentTrigger], 5, "1");
  else
    ini.sections["Triggers"].values[m_currentTrigger] =
        SetParam(ini.sections["Triggers"].values[m_currentTrigger], 5, "0");
}
...
void CTriggerOptionsDlg::OnHard() {
  ...
  if (bHard)
    ini.sections["Triggers"].values[m_currentTrigger] =
        SetParam(ini.sections["Triggers"].values[m_currentTrigger], 6, "1");
  else
    ini.sections["Triggers"].values[m_currentTrigger] =
        SetParam(ini.sections["Triggers"].values[m_currentTrigger], 6, "0");
}
