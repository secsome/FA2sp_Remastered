...
CNewMap::CNewMap(CWnd* pParent /*=NULL*/) : CDialog(CNewMap::IDD, pParent) {
...
void CNewMap::OnBrowse() {
  ...
  CMapOpenDialog dlg(TRUE, NULL, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_FILEMUSTEXIST,
                     "TS maps|*.mpr;*.map|TS multi maps|*.mpr|TS single maps|*.map|");
...
void CNewMap::OnNew() { m_OK.EnableWindow(TRUE); }
