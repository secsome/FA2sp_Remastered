...
CIniFile::CIniFile() { Clear(); }

CIniFile::~CIniFile() { sections.clear(); }
...
void CIniFile::Clear() { sections.clear(); }
...
WORD CIniFile::InsertFile(const std::string& filename, const char* Section, BOOL bNoSpaces) {
  ...
  while (!file.eof()) {
    ...
    cLine.erase(
        std::find_if(cLine.begin(), cLine.end(), [](const char c) { return c == '\r' || c == '\n' || c == ';'; }),
        cLine.end());
    ...
    if (openBracket != npos && closeBracket != npos && openBracket < closeBracket &&
        (equals == npos || equals > openBracket)) {
      if ((Section != nullptr) && cSec == Section) return 0;  // the section we want to insert is finished
...
CString& CIniFileSection::AccessValueByName(const CString& valueName) { return values[valueName]; }
...
BOOL CIniFile::SaveFile(const CString& filename) const { return SaveFile(std::string(filename.GetString())); }
...
CString CIniFile::GetValueByName(const CString& sectionName, const CString& valueName,
...
int CIniFileSection::GetValueOrigPos(int index) const noexcept {
  if (index > value_orig_pos.size() - 1) return -1;

  auto i = value_orig_pos.cbegin();
  for (int e = 0; e < index; e++) i++;
