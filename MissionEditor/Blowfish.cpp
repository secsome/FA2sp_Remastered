...
BlowfishEngine::BlowfishEngine() noexcept
	: IsKeyed{ false }
	, P_Encrypt {}
	, P_Decrypt {}
	, bf_S {}
...
void BlowfishEngine::Submit_Key(const void* key, int length)
{
	...
	unsigned int* p_en = &P_Encrypt[0];
	unsigned int* p_de = &P_Decrypt[ROUNDS + 1];
...
int BlowfishEngine::Encrypt(const void* plaintext, int length, void* cyphertext)
{
	if (plaintext == 0 || length == 0)
		return 0;

	if (cyphertext == 0)
		cyphertext = const_cast<void*>(plaintext);
...
int BlowfishEngine::Decrypt(const void* cyphertext, int length, void* plaintext)
{
	if (cyphertext == 0 || length == 0)
		return 0;

	if (plaintext == 0)
		plaintext = const_cast<void*>(cyphertext);
...
void BlowfishEngine::Sub_Key_Encrypt(unsigned int& left, unsigned int& right)
{
	...
	{
		l.Int ^= P_Encrypt[index];
		r.Int ^= (((bf_S[0][l.Char.C0] + bf_S[1][l.Char.C1]) ^ bf_S[2][l.Char.C2]) + bf_S[3][l.Char.C3]);
		r.Int ^= P_Encrypt[index + 1];
		l.Int ^= (((bf_S[0][r.Char.C0] + bf_S[1][r.Char.C1]) ^ bf_S[2][r.Char.C2]) + bf_S[3][r.Char.C3]);
	}
	left = r.Int ^ P_Encrypt[ROUNDS + 1];
	right = l.Int ^ P_Encrypt[ROUNDS];
}

void BlowfishEngine::Process_Block(const void* plaintext, void* cyphertext, const unsigned int* ptable)
{
	...
	{
		left.Int ^= *ptable++;
		right.Int ^= (((bf_S[0][left.Char.C0] + bf_S[1][left.Char.C1]) ^ bf_S[2][left.Char.C2]) + bf_S[3][left.Char.C3]);
		right.Int ^= *ptable++;
		left.Int ^= (((bf_S[0][right.Char.C0] + bf_S[1][right.Char.C1]) ^ bf_S[2][right.Char.C2]) + bf_S[3][right.Char.C3]);
	}
...
const unsigned int BlowfishEngine::P_Init[BlowfishEngine::ROUNDS + 2] =
{
...
const unsigned int BlowfishEngine::S_Init[4][UCHAR_MAX + 1] =
{
	{
