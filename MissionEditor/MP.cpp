...
void XMP_Mod_Mult_Clear(int precision)
{
	...
	XMP_Init(_mod_quotient, 0, sizeof(_mod_quotient) / sizeof(_mod_quotient[0]));
	XMP_Init(_mod_divisor, 0, sizeof(_mod_divisor) / sizeof(_mod_divisor[0]));
...
bool XMP_Is_Small_Prime(const digit* candidate, int precision)
{
	...
	if (*candidate > MPPrimesHolder::primeTable[sizeof(MPPrimesHolder::primeTable) / sizeof(MPPrimesHolder::primeTable[0]) - 1]) return false;

	unsigned int* ptr = (unsigned int*)bsearch(&candidate, &MPPrimesHolder::primeTable[0], sizeof(MPPrimesHolder::primeTable) / sizeof(MPPrimesHolder::primeTable[0]), sizeof(MPPrimesHolder::primeTable[0]), pfunc);
	return(ptr != NULL);
}
