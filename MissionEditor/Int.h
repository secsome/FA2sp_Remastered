...
private:
	...
	struct RemainderTable
	{
		...
		{
			for (unsigned i = 0; i < sizeof(MPPrimesHolder::primeTable) / sizeof(MPPrimesHolder::primeTable[0]); i++) {
		...
		{
			HasZeroEntry = false;
			for (unsigned int i = 0; i < sizeof(MPPrimesHolder::primeTable) / sizeof(MPPrimesHolder::primeTable[0]); i++) {
				table[i] += increment;
		...
		bool HasZeroEntry;
		unsigned short table[sizeof(MPPrimesHolder::primeTable) / sizeof(MPPrimesHolder::primeTable[0])];
	};
