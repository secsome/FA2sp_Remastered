...
void CIsoView::InitializeGameScene()
{
	if (FAILED(Scene.Initialize(m_hWnd)))
	{
		MessageBox("Failed to setup GameScene!", "Error", MB_OK | MB_ICONERROR);
		exit(114);
	}
}
...
void CIsoView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	CStatusBarCtrl& stat = theApp.GetMainWnd()->m_view.m_statbar.GetStatusBarCtrl();
...
void CIsoView::OnRButtonUp(UINT nFlags, CPoint point)
{
	...
	if (AD.mode != 0 && !rscroll)
	{
		...
		if (!ignoreClick)
		{
			...
			CMyViewFrame& frame = theApp.GetMainWnd()->m_view;
...
{
	CMyViewFrame& v = theApp.GetMainWnd()->m_view;
